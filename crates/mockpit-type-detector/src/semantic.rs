...
/// # Examples
/// ```
/// # use mockpit_type_detector::semantic::matches_field_name;
...
/// # Examples
/// ```
/// # use mockpit_type_detector::semantic::matches_any_field_name;
...
pub fn matches_any_field_name(field_name: &str, patterns: &[&str]) -> bool {
  patterns.iter().any(|pattern| matches_field_name(field_name, pattern))
}
...
/// # Examples
/// ```
/// # use mockpit_type_detector::semantic::contains_field_pattern;
...
/// # Examples
/// ```
/// # use mockpit_type_detector::semantic::contains_any_field_pattern;
...
/// # Examples
/// ```
/// # use mockpit_type_detector::semantic::ends_with_field_pattern;
...
pub fn detect_from_field_name_only(field_name: &str) -> Option<(FieldType, f64)> {
  ...
  ) || (contains_field_pattern(field_name, "description") && !contains_field_pattern(field_name, "short"))
  {
  ...
  if matches_any_field_name(
    ...
    field_name,
    &["avatar", "icon", "thumbnail", "gravatar", "picture", "photo"],
  ) {
  ...
  ) || (contains_field_pattern(field_name, "hash") && !contains_field_pattern(field_name, "tag"))
  {
...
pub fn detect_from_semantic_context(field_name: &str, values: &[&JsonValue]) -> Option<(FieldType, f64)> {
  ...
  let suggests_etag = contains_field_pattern(field_name, "etag")
    || (contains_field_pattern(field_name, "version") && contains_field_pattern(field_name, "hash"))
    || (contains_field_pattern(field_name, "version") && contains_field_pattern(field_name, "tag"))
    || (contains_field_pattern(field_name, "cache") && contains_field_pattern(field_name, "tag"))
    || (contains_field_pattern(field_name, "entity") && contains_field_pattern(field_name, "tag"))
    || (contains_field_pattern(field_name, "checksum") && contains_field_pattern(field_name, "tag"))
    || (contains_field_pattern(field_name, "content") && contains_field_pattern(field_name, "hash"))
    || (contains_field_pattern(field_name, "object") && contains_field_pattern(field_name, "version"))
  ...
  let suggests_physical_measurement =
    contains_any_field_pattern(field_name, &["height", "width", "depth", "distance", "duration"]);
  ...
  let suggests_filesize = suggests_size_measurement
    && (suggests_storage_context
      || !(suggests_physical_measurement || contains_any_field_pattern(field_name, &["array", "list"])));
  ...
  let suggests_constant =
    contains_any_field_pattern(field_name, &["version", "constant", "default", "fixed", "standard"]);
  ...
  if !strs.is_empty() {
    ...
    let suggests_revision = contains_any_field_pattern(field_name, &["revision", "version", "checksum"]);
    ...
    if suggests_revision {
      ...
      let looks_like_etag = strs.iter().any(|s| {
        ...
        if s.starts_with('"') && s.ends_with('"') {
          ...
          (inner.len() == 32 || inner.len() == 40 || inner.len() == 64) && inner.chars().all(|c| c.is_ascii_hexdigit())
        } else {
          false
        }
      });
    ...
    let suggests_mimetype = contains_any_field_pattern(
      field_name,
      &[
        "mime",
        "content_type",
    ...
    if suggests_timezone {
      ...
      let looks_like_timezone = strs
        .iter()
        .any(|s| s.contains('/') && s.chars().next().map(|c| c.is_ascii_uppercase()).unwrap_or(false));
      ...
      })
    {
      ...
      return Some((FieldType::Categorical { values: unique_values }, 0.90));
    }
    ...
    if let Some(pattern) = analyze_pagination_pattern(&strs) {
      ...
      let field_suggests_pagination = contains_any_field_pattern(field_name, &["next", "prev", "page"]);

      let confidence = if field_suggests_pagination { 0.95 } else { 0.85 };
    ...
    if strs.iter().any(|s| {
      URL_REGEX.is_match(s)
        && (s.contains("page=") || s.contains("limit=") || s.contains("offset=") || s.contains("cursor="))
    }) {
      ...
      if contains_any_field_pattern(field_name, &["next", "prev", "page", "link"]) {
        ...
        if let Some(first_url) = strs.first() {
          if let Ok(parsed) = Url::parse(first_url) {
            ...
            let base_url = format!("{}://{}{}", parsed.scheme(), host_with_port, parsed.path());
            let static_params: Vec<(String, String)> = parsed
              .query_pairs()
              .filter(|(k, _)| !PAGE_KEYS.contains(&k.as_ref()) && !LIMIT_KEYS.contains(&k.as_ref()))
    ...
    if (matches_any_field_name(
      ...
      field_name,
      &["avatar", "icon", "thumbnail", "gravatar", "picture", "photo", "image"],
    ...
    if matches_any_field_name(field_name, &["next", "previous", "link", "href", "url", "uri"])
      && strs.iter().any(|s| URL_REGEX.is_match(s))
    {
      let confidence = calculate_url_confidence(&strs);
      if confidence >= CONFIDENCE_URL {
    ...
    if looks_like_api_path || (suggests_api_path_field && strs.iter().any(|s| s.starts_with('/'))) {
    ...
    if strs.iter().any(|s| URL_REGEX.is_match(s)) {
      ...
      {
        let sample_url = strs.iter().find(|s| URL_REGEX.is_match(s)).map(|s| (*s).to_string());
    ...
    let suggests_token = (contains_field_pattern(field_name, "token") && !contains_field_pattern(field_name, "data"))
      || contains_any_field_pattern(field_name, &["jwt", "bearer"])
      || (contains_field_pattern(field_name, "api") && contains_any_field_pattern(field_name, &["key", "secret"]))
      || (contains_field_pattern(field_name, "access") && contains_field_pattern(field_name, "key"))
      || (contains_field_pattern(field_name, "session") && contains_field_pattern(field_name, "key"));
    ...
    if suggests_token || matches_field_name(field_name, "key") {
      let looks_like_token = strs.iter().any(|s| {
        ...
        s.len() >= 32 && !s.starts_with('#') && s.chars().all(|c| c.is_ascii_alphanumeric() || c == '-' || c == '_')
      });
    ...
    let suggests_color_context = contains_any_field_pattern(field_name, &["color", "colour", "hex", "theme", "accent"])
    ...
    // Email fields
    if contains_any_field_pattern(field_name, &["email", "mail"]) && strs.iter().any(|s| EMAIL_REGEX.is_match(s)) {
    ...
    let suggests_path = contains_any_field_pattern(field_name, &["path", "directory", "folder", "location"]);

    let suggests_filename = contains_field_pattern(field_name, "file") && contains_field_pattern(field_name, "name");
    ...
    // Version fields
    if matches_any_field_name(field_name, &["version", "ver"]) && strs.iter().any(|s| SEMVER_REGEX.is_match(s)) {
      return Some((FieldType::Semver, 0.90));
    }

    // ETag fields (Filestore API uses numeric version numbers in "etag" field)
    // This handles vendor-specific ETags which are just numeric version numbers
    if matches_field_name(field_name, "etag") && strs.iter().all(|s| s.chars().all(|c| c.is_ascii_digit())) {
      return Some((FieldType::ETag, 0.95));
    }
    ...
    let suggests_phone = contains_any_field_pattern(field_name, &["phone", "tel", "fax", "mobile", "cell", "hotline"])
      || (contains_field_pattern(field_name, "contact") && contains_field_pattern(field_name, "number"))
      || (contains_field_pattern(field_name, "support") && contains_field_pattern(field_name, "number"));

    if suggests_phone && strs.iter().any(|s| s.len() >= 10 && PHONE_REGEX.is_match(s)) {
    ...
    let suggests_short_text = contains_any_field_pattern(
      field_name,
      &[
        "title", "headline", "subject", "caption", "label", "heading", "tagline", "slogan", "motto", "line",
      ],
    ...
    let suggests_long_text = (contains_field_pattern(field_name, "description")
      && contains_field_pattern(field_name, "long"))
      || (contains_field_pattern(field_name, "body") && !contains_field_pattern(field_name, "size"))
      || contains_any_field_pattern(
        field_name,
    ...
    let suggests_currency = contains_any_field_pattern(
      field_name,
      &["currency", "forex", "denomination", "monetary_unit", "exchange"],
    );
    ...
    let suggests_country = contains_any_field_pattern(field_name, &["country", "nation", "citizenship", "origin"])
      || (contains_field_pattern(field_name, "iso") && contains_field_pattern(field_name, "country"));
...
pub(super) fn calculate_semantic_boost(field_name: &str, field_type: &FieldType) -> f64 {
  ...
  match field_type {
    FieldType::Uuid => {
      ...
      if name_lower.ends_with("_id") || name_lower.ends_with("uuid") || name_lower.contains("uuid") {
        return 0.2;
      }
    ...
    FieldType::Timestamp => {
      if name_lower.contains("date") || name_lower.ends_with("_at") || name_lower.contains("time") {
        return 0.15;
      }
    ...
    FieldType::Url => {
      if name_lower.contains("url") || name_lower.contains("link") || name_lower.contains("href") {
        return 0.15;
      }
      // Penalty if field name suggests email or image
      if name_lower.contains("email") || name_lower.contains("avatar") || name_lower.contains("icon") {
        return -0.2;
      }
      0.0
    },
    FieldType::ImageUrl => {
      ...
      if name_lower.contains("url") && !name_lower.contains("avatar") && !name_lower.contains("icon") {
        return -0.15;
      }
      0.0
    },
    ...
    FieldType::Token if name_lower.contains("token") || name_lower.contains("key") || name_lower.contains("jwt") => 0.2,
    FieldType::Latitude if name_lower == "lat" || name_lower == "latitude" => 0.3,
    FieldType::Longitude if name_lower == "lon" || name_lower == "lng" || name_lower == "longitude" => 0.3,
    FieldType::CountryCode if name_lower.contains("country") && name_lower.contains("code") => 0.25,
