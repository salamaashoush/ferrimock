...
//!
//! ```rust
//! use mockpit_type_detector::TypeDetector;
//! use serde_json::json;
//!
...
pub use types::{ArrayPattern, FieldType, ObjectAnalysis, PaginationDirection, PaginationScheme, PaginationUrlPattern};
...
impl TypeDetector {
  ...
  fn detect_from_patterns(&self, values: &[&JsonValue], features: &features::TypeFeatures) -> (FieldType, f64) {
    ...
    if matches!(field_type, FieldType::DownloadUrl { .. }) {
      let sample_url = strings.iter().find(|s| s.len() > 100).map(|s| (*s).to_string());
