...
pub(super) fn analyze_numbers(values: &[&JsonValue]) -> (FieldType, f64) {
  ...
  if floats.is_empty() {
    return (FieldType::RandomNumber { min: None, max: None }, 0.5);
  }
...
pub(super) fn analyze_integers(numbers: &[i64]) -> (FieldType, f64) {
  if numbers.is_empty() {
    return (FieldType::RandomNumber { min: None, max: None }, 0.8);
  }
  ...
  if numbers.iter().all(|&n| n >= min_timestamp && n <= max_timestamp) {
...
pub(super) fn analyze_floats(floats: &[f64]) -> (FieldType, f64) {
  if floats.is_empty() {
    return (FieldType::RandomFloat { min: None, max: None }, 0.8);
  }
  ...
  if floats.iter().all(|&f| (-180.0..=180.0).contains(&f)) && floats.iter().any(|&f| f.abs() > 0.0) {
...
pub(super) fn analyze_array_pattern<F>(values: &[&JsonValue], detect_type_from_values: F) -> (FieldType, f64)
where
...
{
  let objects: Vec<&serde_json::Map<String, JsonValue>> = values.iter().filter_map(|v| v.as_object()).collect();
  ...
  for field in all_fields {
    let field_values: Vec<&JsonValue> = objects.iter().filter_map(|obj| obj.get(&field)).collect();
