use mockpit_type_detector::{TypeDetector, features};
...
fn bench_single_field_numeric_string_id(c: &mut Criterion) {
  let detector = TypeDetector::new();
  let values = vec![json!("10000000002"), json!("12345678901234"), json!("98765432109876")];
  let value_refs = as_refs(&values);
...
fn bench_multi_sample_url(c: &mut Criterion) {
  ...
  for &size in &[10, 50, 100, 500] {
    let values = generate_sample_values(size, |i| json!(format!("https://example.com/page/{}", i)));
    let value_refs = as_refs(&values);
...
fn bench_multi_sample_timestamp(c: &mut Criterion) {
  ...
  for &size in &[10, 50, 100, 500] {
    let values = generate_sample_values(size, |i| json!(format!("2024-01-{:02}T10:30:00Z", (i % 28) + 1)));
    let value_refs = as_refs(&values);
...
fn bench_feature_extraction_many_samples(c: &mut Criterion) {
  ...
  for &size in &[10, 50, 100, 500] {
    let values: Vec<String> = (0..size).map(|i| format!("https://example.com/page/{}", i)).collect();
...
fn bench_shannon_entropy(c: &mut Criterion) {
  ...
  let high_entropy_values: Vec<&str> = vec!["a1B2c3D4e5F6g7H8", "x9Y8z7W6v5U4t3S2", "m1N2o3P4q5R6s7T8"];
...
fn bench_real_world_api_response(c: &mut Criterion) {
  ...
  let id_values = vec![json!("10000000002"), json!("23930793380")];
...
fn bench_real_world_mixed_types(c: &mut Criterion) {
  ...
  let fields = vec![
    (
      "next",
      vec![json!("https://example.com/page2"), json!("https://example.com/page3")],
    ),
    ("user_id", vec![json!("12345678901234"), json!("98765432109876")]),
    ("email", vec![json!("user@example.com"), json!("admin@test.org")]),
    (
      "created_at",
...
fn bench_bigquery_equivalent(c: &mut Criterion) {
  ...
  let url_values = generate_sample_values(500, |i| json!(format!("https://example.com/api/v1/users/{}", i)));
  let url_refs = as_refs(&url_values);
