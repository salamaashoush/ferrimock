...
use base64::Engine;
use mockpit_engine::{MockMatcher, MockRegistry, ResponseGeneratorExt};
use crate::ui;
...
pub async fn test_mock_match(params: TestMockParams) -> anyhow::Result<()> {
  ...
  if json {
    return test_mock_match_json(method_str, &clean_path, &final_query, headers, body, render, mock_file).await;
  }
  ...
  let body_bytes: Option<Vec<u8>> = match &body {
    Some(b) if b.starts_with('@') => {
      ...
      match std::fs::read(file_path) {
        Ok(content) => {
          println!();
          println!("{}", ui::kv("Body", &format!("(from file: {})", ui::path(file_path))));
          Some(content)
        },
        Err(e) => {
          return Err(anyhow::anyhow!("Failed to read body file '{file_path}': {e}"));
        },
      }
    },
    Some(b) => {
  ...
  let count = if let Some(file) = mock_file {
    ...
    let collections_dir = std::env::var("MOCKS_DIR").unwrap_or_else(|_| "mocks/collections".to_string());
  ...
  if let Some(mock_match) = matcher.find_match(
    ...
    println!("{}", ui::kv("Priority", &ui::number(mock_match.mock.priority)));
    println!(
      "{}",
      ui::kv("Response Status", &ui::number(mock_match.mock.response.status.as_u16()))
    );
    println!(
      "{}",
      ui::kv("Response Mode", &format!("{:?}", mock_match.mock.response.mode))
    );
        ...
        .await
      {
        Ok(dynamic_response) => {
          ...
          if let Some(status) = dynamic_response.status {
            println!("{}", ui::kv("Status Override", &ui::number(status.as_u16())));
          }
          ...
          if let Ok(json) = serde_json::from_str::<serde_json::Value>(&body_str) {
            ...
            if body_str.len() > 2000 {
              ...
              println!("{truncated}...");
              println!("{}", ui::dim(&format!("(truncated, {} total bytes)", body_str.len())));
            } else {
              println!("{body_str}");
            }
          }
    ...
    if !debug {
      println!();
      println!("{}", ui::dim("Tip: Use --debug to see why each mock didn't match"));
    }
  }
...
async fn test_mock_match_json(
  ...
  // Find match
  let mock_result = matcher.find_match(&method, path, query.as_deref(), &header_map, body_bytes.as_deref());
        ...
        .await
      {
        Ok(dynamic_response) => {
          ...
          let (body_value, is_base64) = if is_binary {
            ...
            (
              json!(base64::engine::general_purpose::STANDARD.encode(&dynamic_response.body)),
              true,
            )
          } else {
...
fn show_debug_matching(
  ...
  for mock in all_mocks {
    ...
    if method_match {
      match_details.push(format!(
        ...
        if mock.request.methods.is_empty() {
          ...
          format!(
            "matches {:?}",
            mock.request.methods.iter().map(Method::as_str).collect::<Vec<_>>()
          )
        }
      ));
    } else {
    ...
    if url_match {
      match_details.push(format!("  {} URL: {} matches pattern(s)", CHECK_ICON.green(), path));
    } else {
      all_matched = false;
      ...
      if header_match {
        ...
        for matcher in &mock.request.header_matchers {
          if !matcher.matches(headers) {
            match_details.push(format!("  {} Header matcher failed: {:?}", CROSS_ICON.red(), matcher));
          }
        }
      }
    }
    ...
    if !mock.request.query_matchers.is_empty() {
      let query_match = mock.request.query_matchers.iter().all(|matcher| matcher.matches(query));
