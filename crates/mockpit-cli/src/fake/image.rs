...
pub fn generate_fake_image(
  ...
  let base64_data = match image_type.to_lowercase().as_str() {
    ...
    "gradient" => {
      ...
      fake_image_gradient(Some(width), Some(height), Some(start), Some(end), Some(direction))
    },
    ...
    "stripes" => {
      let c1 = bg_color.unwrap_or("#FFFFFF");
      let c2 = text_color.unwrap_or("#000000");
      fake_image_stripes(Some(width), Some(height), Some(c1), Some(c2), Some(20), Some(direction))
    },
    "text" => {
  ...
  let final_data = if image_format.to_lowercase() == "jpeg" || image_format.to_lowercase() == "jpg" {
  ...
  if let Some(path) = output {
    ...
    let temp_path = std::env::temp_dir().join(format!("fake-image-{}.{}", mockpit_fake_data::fake_short_hash(), ext));
    let bytes = base64::engine::general_purpose::STANDARD
      .decode(&final_data)
      .map_err(|e| anyhow::anyhow!("Failed to decode base64: {e}"))?;
    ...
    println!(
      "{}",
      ui::success(&format!("Generated: {}", ui::path(&temp_path.to_string_lossy())))
    );
