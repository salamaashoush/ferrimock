...
pub fn generate_single_value(
  ...
  let value = match generator.to_lowercase().as_str() {
    ...
    _ => {
      anyhow::bail!("Unknown generator: '{generator}'. Use 'fake list' to see available generators.");
    },
  };
...
pub fn list_generators(
  ...
  for cat in categories {
    ...
    let cat_generators: Vec<&&GeneratorInfo> = filtered.iter().filter(|g| g.category == cat).collect();

    for g in cat_generators {
      if verbose {
        println!("  {} {} {}", ui::action(g.name), g.description, ui::dim(g.params));
