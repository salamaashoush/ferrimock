...
pub async fn convert_har(opts: ConvertHarOptions) -> anyhow::Result<()> {
  ...
  let options = HarLoadOptions {
    ...
    filter_non_allowed_domains: opts.filter_non_allowed_domains,
    ...
    extra_allowed_domains: opts.extra_domains,
  };
  ...
  println!(
    "{}",
    ui::success(&format!("Loaded {} mock definition(s)", ui::number(mocks.len())))
  );
  println!();
  ...
  let final_mocks = if opts.interactive {
    ...
    for (idx, mock) in mocks.iter().enumerate() {
      ...
      if let Some(ref match_config) = mock.match_config {
        println!("{}", ui::kv("Method", &format!("{:?}", match_config.methods)));
  ...
  println!("{}", ui::success("Successfully converted HAR to mock collection"));
  println!();
  println!("{}", ui::kv("Output", &ui::path(&opts.output)));
