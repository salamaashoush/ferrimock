...
const REPO: &str = "salamaashoush/mockpit";
...
pub async fn run(check_only: bool) -> Result<()> {
    ...
    if latest == CURRENT_VERSION {
        println!(
            "{}  mockpit {} is already the latest version",
            "ok:".green().bold(),
            CURRENT_VERSION
        );
        return Ok(());
    ...
    if check_only {
        println!("\n   Run {} to update", "mockpit self-update".green());
        return Ok(());
    }

    // Determine install command based on how mockpit was likely installed
    ...
    if cargo_install {
        ...
        let status = tokio::process::Command::new("cargo")
            .args(["install", "mockpit-cli", "--locked"])
            .status()
            .await
            .context("Failed to run cargo install")?;

        if status.success() {
            println!("{}  Updated to mockpit {}", "ok:".green().bold(), latest);
        } else {
