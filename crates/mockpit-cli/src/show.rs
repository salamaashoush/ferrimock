...
pub async fn show_mock(mock_id: &str) -> anyhow::Result<()> {
  ...
  println!("{}", ui::header(&format!("Mock Definition: {}", mock_def.id)));
  println!();
  ...
  println!("{}", ui::kv("Status", &ui::number(mock_def.response.status)));
  println!("{}", ui::kv("Mode", &format!("{:?}", mock_def.response.mode)));
  ...
  println!("{}", ui::kv("Body Source", &format!("{:?}", mock_def.response.body)));
