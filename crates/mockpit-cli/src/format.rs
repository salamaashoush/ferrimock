...
pub fn format_mocks(path: Option<String>, check: bool, stdin: bool, file_format: Option<&str>) -> anyhow::Result<()> {
  ...
  let input_path =
    path.unwrap_or_else(|| std::env::var("MOCKS_DIR").unwrap_or_else(|_| "mocks/collections".to_string()));
...
fn format_json_file(original: &str) -> anyhow::Result<String> {
  ...
  let mut output = serde_json::to_string_pretty(&config).map_err(|e| anyhow::anyhow!("JSON serialize error: {e}"))?;
  output.push('\n');
  ...
  let serialized = if changed {
    let mut out =
      serde_json::to_string_pretty(&config_mut).map_err(|e| anyhow::anyhow!("JSON re-serialize error: {e}"))?;
    out.push('\n');
    out
  } else {
    output
...
fn format_yaml_file(original: &str) -> anyhow::Result<String> {
  ...
  let structural = serde_yaml::to_string(&config).map_err(|e| anyhow::anyhow!("YAML serialize error: {e}"))?;

  // Format body and template strings
  let mut config_mut: MockCollectionConfig =
    serde_yaml::from_str(&structural).map_err(|e| anyhow::anyhow!("YAML re-parse error: {e}"))?;
  ...
  let serialized = if changed {
    serde_yaml::to_string(&config_mut).map_err(|e| anyhow::anyhow!("YAML re-serialize error: {e}"))?
  } else {
    structural
  };
...
fn format_external_references(
  ...
  let config: MockCollectionConfig = match extension {
    Some("json") => serde_json::from_str(content).map_err(|e| anyhow::anyhow!("JSON parse error: {e}"))?,
    Some("yaml" | "yml") => serde_yaml::from_str(content).map_err(|e| anyhow::anyhow!("YAML parse error: {e}"))?,
    _ => return Ok(()),
  };
    ...
    if let Some(ref response_config) = mock.response_config {
      ...
      if let Some(ref_path) = response_config.file_ref() {
        if let Err(e) = format_external_file(mock_dir, ref_path, false, check) {
          let mock_id = &mock.id;
          println!("  {} formatting file for mock {}: {}", ui::warning("warn"), mock_id, e);
        }
      }
    }
  }
...
fn format_external_file(mock_dir: &Path, ref_path: &str, is_template: bool, check: bool) -> anyhow::Result<bool> {
  ...
  if !check {
    ...
    let filename = full_path.file_name().and_then(|n| n.to_str()).unwrap_or("unknown");
    println!("  {} {}", ui::success("formatted external"), ui::path(filename));
  }
...
fn expand_json_body_strings(json_str: &str) -> anyhow::Result<String> {
  let mut value: serde_json::Value =
    serde_json::from_str(json_str).map_err(|e| anyhow::anyhow!("JSON re-parse for body expansion: {e}"))?;
...
fn expand_yaml_body_strings(yaml_str: &str) -> anyhow::Result<String> {
  let mut value: serde_yaml::Value =
    serde_yaml::from_str(yaml_str).map_err(|e| anyhow::anyhow!("YAML re-parse for body expansion: {e}"))?;

  expand_yaml_body_values(&mut value);
  sort_yaml_keys(&mut value);

  serde_yaml::to_string(&value).map_err(|e| anyhow::anyhow!("YAML re-serialize after body expansion: {e}"))
}
