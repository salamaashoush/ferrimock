...
use crate::{MockAction, MockCommand};
use crate::{
  consolidate, convert, create, export, format, list, recordings, reload, serve, show, test, validate, wizard,
};
  ...
  match cmd.action {
    MockAction::Create {
      ...
      match (interactive, url) {
        ...
        (true, url) | (false, url @ None) => {
          wizard::run_wizard(url, output, &method, status, body, template, id, priority, collection)
        },
    ...
    MockAction::List { collection, verbose } => list::list_mocks(collection, verbose).await,
    ...
    MockAction::Convert {
      ...
      convert::convert_har(convert::ConvertHarOptions {
        ...
        filter_non_allowed_domains: !all_domains,
    ...
    MockAction::Consolidate {
      ...
      no_templates,
      verbose,
    } => consolidate::consolidate_mocks(input, output, format, min_pattern, !no_templates, verbose).await,
    MockAction::Serve {
      port,
      host,
      mocks,
