...
pub fn create_mock(
  ...
  if interactive {
    ...
    println!("{}", ui::kv("Template", if template { "Yes" } else { "No" }));
    println!();
  ...
  let mock_body = if let Some(body_input) = body {
    ...
    if body_input.starts_with('@') {
      ...
      std::fs::read_to_string(file_path).map_err(|e| anyhow::anyhow!("Failed to read body file: {e}"))?
    } else {
      body_input
    }
  } else if template {
  ...
  // Write the mock file
  std::fs::write(&output_path, mock_config).map_err(|e| anyhow::anyhow!("Failed to write mock file: {e}"))?;
...
pub fn generate_template_body(method: &str, url: &str) -> String {
  ...
  let is_list = url.contains("/users") || url.contains("/files") || url.contains("/folders") || url.contains("/items");
...
pub fn generate_json_mock(params: &MockGeneratorParams<'_>) -> anyhow::Result<String> {
  ...
  mock_obj.insert("id".to_string(), serde_json::Value::String(mock_id.to_string()));
  mock_obj.insert("priority".to_string(), serde_json::Value::Number((*priority).into()));
  ...
  match_obj.insert("url".to_string(), serde_json::Value::String(url.to_string()));
  mock_obj.insert("match".to_string(), serde_json::Value::Object(match_obj));

  // Build response object
  let mut response_obj = serde_json::Map::new();
  response_obj.insert("status".to_string(), serde_json::Value::Number((*status).into()));
  ...
  response_obj.insert("headers".to_string(), serde_json::Value::Object(headers_obj));
  let field_name = if *is_template { "template" } else { "body" };
  response_obj.insert(field_name.to_string(), serde_json::Value::String(body.to_string()));
  mock_obj.insert("response".to_string(), serde_json::Value::Object(response_obj));
  ...
  if let Some(coll) = collection {
    config.insert("name".to_string(), serde_json::Value::String(coll.to_string()));
