...
pub fn run_wizard(
  ...
  println!("{}", ui::dim("Press Enter to accept defaults shown in [brackets]."));
  println!();
...
fn step_request_matching(state: &mut WizardState) -> anyhow::Result<()> {
  ...
  print!("{} [{}]: ", ui::emphasis("URL Pattern"), ui::dim(default_url));
  io::stdout().flush()?;
  ...
  println!("  {}", ui::dim(&format!("Auto-detected: {pattern_type} pattern")));
  println!();

  // HTTP Methods
  println!("{}", ui::emphasis("HTTP Method(s):"));
  println!("  {}", ui::dim("Select methods (space-separated, e.g., 'GET POST')"));
  ...
  for (i, method) in methods_list.iter().enumerate() {
    ...
    println!("  {} {} {}", if selected { "[x]" } else { "[ ]" }, i + 1, method);
  }
  let default_methods = state.methods.join(" ");
  print!("{} [{}]: ", ui::emphasis("Methods"), ui::dim(&default_methods));
  io::stdout().flush()?;
  ...
  if input.trim().eq_ignore_ascii_case("y") {
    loop {
      ...
      state.header_matchers.push((name.to_string(), pattern.to_string()));
  ...
  if input.trim().eq_ignore_ascii_case("y") {
    loop {
      ...
      state.query_matchers.push((name.to_string(), pattern.to_string()));
      println!("  {}", ui::success(&format!("Added: {name} = {pattern}")));
    }
  }
  println!();
  ...
  if state.methods.iter().any(|m| m == "POST" || m == "PUT" || m == "PATCH") {
...
fn step_response_config(state: &mut WizardState) -> anyhow::Result<()> {
  ...
  print!("{} [{}]: ", ui::emphasis("Status Code"), ui::dim(&default_status));
  io::stdout().flush()?;
  ...
  print!("{} [{}]: ", ui::emphasis("Select or enter custom"), ui::dim("1"));
  io::stdout().flush()?;
  ...
  match &state.body_source {
    BodySource::Template => {
      ...
      state.template_body = match template_choice {
        ...
        "9" => {
          println!();
          println!("{}", ui::dim("Enter template (press Ctrl+D or empty line to finish):"));
    ...
    BodySource::Static => {
      println!("{}", ui::dim("Enter static body (press Enter twice to finish):"));
      let mut lines = Vec::new();
      loop {
        let mut line = String::new();
...
fn step_metadata(state: &mut WizardState, output: Option<String>) -> anyhow::Result<()> {
  ...
  // Mock ID
  print!("{} [{}]: ", ui::emphasis("Mock ID"), ui::dim(&state.mock_id));
  io::stdout().flush()?;
  ...
  print!("{} [{}]: ", ui::emphasis("Output file"), ui::dim(&default_path));
  io::stdout().flush()?;
  ...
  let path_str = if input.is_empty() { &default_path } else { input };
...
fn step_review_and_save(state: &WizardState) -> anyhow::Result<()> {
  ...
  if !state.query_matchers.is_empty() {
    println!("{}", ui::kv("Query Matchers", &state.query_matchers.len().to_string()));
  }
  if state.body_matcher.is_some() {
    println!("{}", ui::kv("Body Matcher", "Yes"));
  }
  println!();
  println!("{}", ui::kv("Output", &state.output_path.display().to_string()));
  ...
  // Write the mock file
  std::fs::write(&state.output_path, &mock_config).map_err(|e| anyhow::anyhow!("Failed to write mock file: {e}"))?;
  ...
  println!(
    ...
    ui::dim(&format!(
      ...
      state.url_pattern.replace(":id", "123").replace(":userId", "456")
    ))
  );
...
fn detect_url_pattern_type(url: &str) -> &'static str {
  if url.contains(':') && !url.contains("://") {
    ...
    "Glob pattern"
  } else if url.starts_with('^') || url.ends_with('$') || url.contains("\\d") || url.contains("\\w") {
    "Regex"
  } else {
    "Exact match"
  }
