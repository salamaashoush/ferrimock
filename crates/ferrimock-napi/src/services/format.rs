...
pub fn format(input: JsFormatInput) -> Result<JsFormatOutput> {
    let result =
        ferrimock::services::format::format_path(ferrimock::services::format::FormatInput {
            path: input.path,
...
pub fn format_content(content: String, file_format: String) -> Result<String> {
    ferrimock::services::format::format_content(ferrimock::services::format::FormatContentInput {
        content,
        file_format,
    })
    .map_err(|e| Error::from_reason(e.to_string()))
