...
pub(crate) fn msw_params_map(
    ...
    ferrimock::types::msw_params(captures)
        .into_iter()
        .map(|(k, v)| {
            (
                k,
                match v {
                    ferrimock::types::MswParamValue::Single(s) => Either::A(s),
                    ferrimock::types::MswParamValue::List(l) => Either::B(l),
                },
            )
        })
        .collect()
...
impl GraphQLRequestInfo {
    ...
    pub fn operation_name(&self) -> Option<String> {
        ...
        ferrimock::engine::MockMatcher::operation_name_from_query(doc).map(str::to_string)
    }
