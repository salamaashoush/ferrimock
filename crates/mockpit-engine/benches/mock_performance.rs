use mockpit_engine::types::{BodySource, MockDefinition, RequestMatcher, ResponseGenerator, UrlPattern};
...
fn create_complex_mock(id: &str) -> MockDefinition {
  use mockpit_engine::types::{BodyMatcher, HeaderMatcher};
...
fn create_template_mock(id: &str) -> MockDefinition {
  MockDefinition {
    ...
    response: ResponseGenerator::new(
      StatusCode::OK,
      BodySource::template(r#"{"timestamp": "{{ now() }}", "random": "{{ get_random(start=1, end=101) }}"}"#),
    ),
    vars: None,
  }
}
...
fn bench_miss_at_scale(c: &mut Criterion) {
  ...
  for size in [100, 500].iter() {
    ...
    for i in 0..*size {
      ...
      let mock = MockDefinition {
        ...
        request: RequestMatcher {
          ...
          url_patterns: smallvec![UrlPattern::regex(&format!(r"^/api/endpoint{}(/.*)?$", i)).unwrap()],
          header_matchers: smallvec![],
          query_matchers: smallvec![],
          body_matcher: None,
          graphql_matcher: None,
    ...
    group.bench_with_input(BenchmarkId::new("miss_mixed_methods", size), size, |b, _| {
      b.iter(|| {
        matcher.find_match(
          black_box(&Method::GET),
          black_box("/completely/different/path/no/match"),
...
fn bench_isolate_costs(c: &mut Criterion) {
  ...
  for size in [100, 500].iter() {
    ...
    group.bench_with_input(BenchmarkId::new("iter_method_url_check", size), size, |b, _| {
      let method = Method::GET;
...
fn bench_static_response(c: &mut Criterion) {
  ...
  let small_response = ResponseGenerator::new(StatusCode::OK, BodySource::inline(r#"{"status": "ok"}"#));
...
fn bench_file_response(c: &mut Criterion) {
  ...
  std::fs::write(&medium_file, format!(r#"{{"data": [{}]}}"#, "1,".repeat(1000))).unwrap();

  let large_file = temp_dir.path().join("large.json");
  std::fs::write(&large_file, format!(r#"{{"data": [{}]}}"#, "1,".repeat(10000))).unwrap();
...
fn bench_template_response(c: &mut Criterion) {
  use mockpit_engine::RequestContext;
  ...
  group.bench_function("100_concurrent_simple_matches", |b| {
    b.to_async(tokio::runtime::Runtime::new().unwrap()).iter(|| async {
      ...
      for _ in 0..100 {
        let m = matcher.clone();
        let h = headers.clone();
        let handle = tokio::spawn(async move { m.find_match(&Method::GET, "/api/test", None, &h, None) });
        handles.push(handle);
      }
...
fn bench_response_patcher(c: &mut Criterion) {
  use mockpit_engine::ResponsePatcher;
  use mockpit_engine::types::PatchOperation;
  use bytes::Bytes;
  use http::Response;
...
fn bench_structured_response(c: &mut Criterion) {
  ...
  let structured_response = ResponseGenerator::new(
    StatusCode::CREATED,
    BodySource::template(r#"{"status": 201, "headers": {"X-Custom": "value"}, "body": {"id": "{{ captures.id }}"}}"#),
  );
