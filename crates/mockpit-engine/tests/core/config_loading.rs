...
"#;
  ...
  let template_mock = definitions.iter().find(|m| m.id == "templated-user").unwrap();
  assert!(matches!(
    template_mock.response.body,
    mockpit_engine::BodySource::Template { .. }
  ));

  // Validate patch mock exists (patching is handled by ResponsePatcher separately)
  let patch_mock = definitions.iter().find(|m| m.id == "patched-response").unwrap();
  assert_eq!(patch_mock.id, "patched-response");

  // Validate conditional mock
  let cond_mock = definitions.iter().find(|m| m.id == "conditional-mock").unwrap();
...
"#;
  ...
  tokio::fs::write(mocks_dir.join("test.yaml"), mock_yaml).await.unwrap();

  // Load from directory
  let registry = MockRegistry::new();
  let count = registry.load_from_directory(mocks_dir.to_str().unwrap()).await.unwrap();
