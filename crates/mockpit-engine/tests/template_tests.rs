...
fn test_template_error_with_suggestions() {
  let suggestions = vec!["Use valid syntax".to_string(), "Check documentation".to_string()];
...
fn test_request_context_with_header() {
  ...
  assert_eq!(ctx.headers.get("Authorization"), Some(&"Bearer token123".to_string()));
...
fn test_request_context_extract_query_params_with_url_encoding() {
  let params = RequestContext::extract_query_params(Some("name=John%20Doe&email=test%40example.com"));
...
fn test_fake_functions_return_non_empty() {
  ...
  let functions = vec![
    ...
    ("fake_pagination_url_offset", "{{ fake_pagination_url_offset() }}"),
  ...
  for (name, template) in functions {
    ...
    assert!(!result.unwrap().is_empty(), "Function {} returned empty", name);
  }
}
...
fn test_store_incr_multiple_times() {
  ...
  let template =
    r#"{{ store_incr(key="incr_multi") }},{{ store_incr(key="incr_multi") }},{{ store_incr(key="incr_multi") }}"#;
  let result = render_template(template, &ctx).unwrap();
...
fn test_store_has_existing() {
  ...
  let template1 = format!(r#"{{% set _ = store_set(key="{}", value="exists") %}}ok"#, unique_key);
  render_template(&template1, &ctx).unwrap();
...
fn test_store_set_nx_when_not_exists() {
  ...
  let template = format!(r#"{{{{ store_set_nx(key="{}", value="first") }}}}"#, unique_key);
  let result = render_template(&template, &ctx).unwrap();
  assert_eq!(result, "true");
...
fn test_store_set_nx_when_exists() {
  ...
  // Set initial value
  let template1 = format!(r#"{{% set _ = store_set(key="{}", value="original") %}}ok"#, unique_key);
  render_template(&template1, &ctx).unwrap();

  // Try to set with NX (should fail)
  let template2 = format!(r#"{{{{ store_set_nx(key="{}", value="new") }}}}"#, unique_key);
  let result = render_template(&template2, &ctx).unwrap();
  assert_eq!(result, "false");
...
fn test_store_get_or_set_existing() {
  ...
  // Set initial value
  let template1 = format!(r#"{{% set _ = store_set(key="{}", value="existing") %}}ok"#, unique_key);
  render_template(&template1, &ctx).unwrap();

  // get_or_set should return existing value
  let template2 = format!(r#"{{{{ store_get_or_set(key="{}", default="default") }}}}"#, unique_key);
  let result = render_template(&template2, &ctx).unwrap();
...
fn test_store_set_with_ttl() {
  ...
  // Set with TTL
  let template1 = r#"{% set _ = store_set(key="ttl_test", value="expires", ttl_seconds=3600) %}ok"#;
  render_template(template1, &ctx).unwrap();
...
fn test_set_global_persistence_store_custom() {
  ...
  custom_store.set("custom_key".to_string(), Value::String("custom_value".to_string()));
...
fn test_template_with_loops_and_conditionals() {
  let ctx = RequestContext::new();
  let template = r#"[{% for i in range(start=0, end=3) %}{% if i > 0 %},{% endif %}{{ i }}{% endfor %}]"#;
  let result = render_template(template, &ctx).unwrap();
...
fn test_vars_basic_access() {
  ...
  vars.insert("base_url".to_string(), serde_json::json!("https://api.example.com"));
  ctx.vars = Some(vars);
...
fn test_vars_nested_object() {
  let mut ctx = RequestContext::new();
  let mut vars = serde_json::Map::new();
  vars.insert("settings".to_string(), serde_json::json!({"color": "blue", "size": 42}));
  ctx.vars = Some(vars);
...
fn test_vars_empty_map() {
  ...
  let result = render_template("Hello {{ vars.anything | default(value='world') }}", &ctx).unwrap();
  assert_eq!(result, "Hello world");
}
...
// ============================================================================
// MOCK FILE SERVICE URL HELPER TESTS
// ============================================================================

#[test]
fn test_mock_pdf_url_default() {
  let ctx = RequestContext::new();
  let result = render_template("{{ mock_pdf_url() }}", &ctx).unwrap();
  assert!(
    result.starts_with("/__internal_gateway_files/pdf/"),
    "Should start with pdf path, got: {}",
    result
  );
  // The UUID part should be 36 chars
  let uuid_part = result.strip_prefix("/__internal_gateway_files/pdf/").unwrap();
  assert_eq!(uuid_part.len(), 36, "UUID should be 36 chars, got: {}", uuid_part);
}

#[test]
fn test_mock_pdf_url_with_params() {
  let ctx = RequestContext::new();
  let result = render_template(r#"{{ mock_pdf_url(text="hello", pages=3) }}"#, &ctx).unwrap();
  assert!(
    result.starts_with("/__internal_gateway_files/pdf/"),
    "Should start with pdf path"
  );
  assert!(
...
#[test]
fn test_mock_image_url_default() {
  let ctx = RequestContext::new();
  let result = render_template("{{ mock_image_url() }}", &ctx).unwrap();
  assert!(
    result.starts_with("/__internal_gateway_files/image/"),
...
#[test]
fn test_mock_image_url_with_params() {
  let ctx = RequestContext::new();
  let result = render_template(
    r#"{{ mock_image_url(width=800, height=600, type="gradient", format="jpeg") }}"#,
    &ctx,
  )
  .unwrap();
  assert!(result.starts_with("/__internal_gateway_files/image/"));
  assert!(result.contains("width=800"), "Should contain width, got: {}", result);
  assert!(result.contains("height=600"), "Should contain height, got: {}", result);
  assert!(result.contains("type=gradient"), "Should contain type, got: {}", result);
  assert!(result.contains("format=jpeg"), "Should contain format, got: {}", result);
}

#[test]
fn test_mock_avatar_url_default() {
  let ctx = RequestContext::new();
  let result = render_template("{{ mock_avatar_url() }}", &ctx).unwrap();
  assert!(
    result.starts_with("/__internal_gateway_files/avatar/"),
...
#[test]
fn test_mock_avatar_url_with_params() {
  let ctx = RequestContext::new();
  let result = render_template(r#"{{ mock_avatar_url(initials="SA", size=128) }}"#, &ctx).unwrap();
  assert!(result.starts_with("/__internal_gateway_files/avatar/"));
  assert!(
    result.contains("initials=SA"),
    "Should contain initials, got: {}",
    result
  );
  assert!(result.contains("size=128"), "Should contain size, got: {}", result);
}

#[test]
fn test_mock_font_url_default() {
  let ctx = RequestContext::new();
  let result = render_template("{{ mock_font_url() }}", &ctx).unwrap();
  assert_eq!(result, "/__internal_gateway_files/font");
}

#[test]
fn test_mock_font_url_with_family() {
  let ctx = RequestContext::new();
  let result = render_template(r#"{{ mock_font_url(family="NotoSans") }}"#, &ctx).unwrap();
  assert_eq!(result, "/__internal_gateway_files/font?family=NotoSans");
}
