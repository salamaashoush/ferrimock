...
use mockpit_engine::registry::{MockCall, MockRegistry};
use mockpit_engine::types::{BodySource, MockDefinition, RequestMatcher, ResponseGenerator};
...
fn test_call_tracking_limit() {
  ...
  let calls = registry.get_calls("test-mock").unwrap();
  assert!(calls.len() <= 10, "Stored {} calls, expected at most 10", calls.len());
}
...
fn test_clear_all_call_tracking() {
  ...
  // Record some calls
  let call = MockCall::new("GET".to_string(), "/test".to_string(), None, FxHashMap::default(), None);
