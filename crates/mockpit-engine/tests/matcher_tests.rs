...
use mockpit_engine::matcher::{MockAction, MockMatcher};
use mockpit_engine::registry::MockRegistry;
use mockpit_engine::types::{
  BodyMatcher, BodySource, HeaderMatcher, MockDefinition, QueryMatcher, RequestMatcher, ResponseGenerator,
  ResponseMode, UrlPattern,
};
...
fn test_clear_cache() {
  ...
  assert_eq!(result1.unwrap().mock.id, "test", "Should match the correct mock");
  ...
  assert!(result.is_some(), "Should still find match after cache clear");
  assert_eq!(result.unwrap().mock.id, "test", "Should still match the correct mock");
}
...
fn test_custom_cache_size() {
  ...
  assert_eq!(result.unwrap().mock.id, "test", "Should match the correct mock");
}
...
fn test_cache_skipped_with_query_params() {
  ...
  let result = matcher.find_match(&Method::GET, "/test", Some("foo=bar"), &HeaderMap::new(), None);
...
fn test_cache_skipped_for_mocks_with_header_matchers() {
  ...
  let mock = create_test_mock(
    ...
    smallvec![Method::GET],
    smallvec![UrlPattern::exact("/test")],
    smallvec![HeaderMatcher::present(HeaderName::from_static("authorization"))],
    smallvec![],
    None,
  );
  registry.add_mock(mock);
...
fn test_url_match_with_query_string_exact() {
  ...
  let result = matcher.find_match(&Method::GET, "/api/search", Some("q=test"), &HeaderMap::new(), None);
  assert!(result.is_some());

  // Should not match with different query
  let result = matcher.find_match(&Method::GET, "/api/search", Some("q=other"), &HeaderMap::new(), None);
  assert!(result.is_none());
}
...
fn test_url_match_path_only_backwards_compat() {
  ...
  let result = matcher.find_match(&Method::GET, "/api/search", Some("q=anything"), &HeaderMap::new(), None);
  assert!(result.is_some());
}
...
fn test_body_matcher_json_path() {
  ...
  let mock = create_test_mock(
    ...
    smallvec![],
    smallvec![],
    Some(BodyMatcher::json_path("$.user.name", serde_json::json!("Alice"))),
  );
  registry.add_mock(mock);
...
fn test_body_matcher_json_equals() {
  ...
  let result = matcher.find_match(&Method::POST, "/test", None, &HeaderMap::new(), Some(body.as_bytes()));
  assert!(result.is_some());
...
fn test_query_matcher_exact() {
  ...
  let result = matcher.find_match(&Method::GET, "/test", Some("page=1"), &HeaderMap::new(), None);
  assert!(result.is_some());

  // Should not match different value
  let result = matcher.find_match(&Method::GET, "/test", Some("page=2"), &HeaderMap::new(), None);
  assert!(result.is_none());
...
fn test_query_matcher_regex() {
  ...
  let result = matcher.find_match(&Method::GET, "/test", Some("id=123"), &HeaderMap::new(), None);
  assert!(result.is_some());

  // Should not match non-numeric ID
  let result = matcher.find_match(&Method::GET, "/test", Some("id=abc"), &HeaderMap::new(), None);
  assert!(result.is_none());
}
...
fn test_query_matcher_present() {
  ...
  let result = matcher.find_match(&Method::GET, "/test", Some("debug=true"), &HeaderMap::new(), None);
  assert!(result.is_some());

  let result = matcher.find_match(&Method::GET, "/test", Some("debug="), &HeaderMap::new(), None);
  assert!(result.is_some());
...
fn test_query_matcher_absent() {
  ...
  let result = matcher.find_match(&Method::GET, "/test", Some("cache=true"), &HeaderMap::new(), None);
  assert!(result.is_none());
}
...
fn test_header_matcher_regex() {
  ...
  let mock = create_test_mock(
    ...
    smallvec![],
    smallvec![UrlPattern::exact("/test")],
    smallvec![HeaderMatcher::regex(HeaderName::from_static("user-agent"), r"^Mozilla/.*").unwrap()],
    smallvec![],
    None,
  );
  registry.add_mock(mock);
...
fn test_header_matcher_absent() {
  ...
  let mut headers = HeaderMap::new();
  headers.insert(HeaderName::from_static("x-cache"), HeaderValue::from_static("hit"));
  let result = matcher.find_match(&Method::GET, "/test", None, &headers, None);
  assert!(result.is_none());
}
...
fn test_url_captures_extraction() {
  ...
  let result = matcher.find_match(&Method::GET, "/users/42/posts/123", None, &HeaderMap::new(), None);
  assert!(result.is_some());
...
fn test_url_pattern_suffix() {
  ...
  let result = matcher.find_match(&Method::GET, "/documents/file.pdf", None, &HeaderMap::new(), None);
  assert!(result.is_some());

  // Should not match other extensions
  let result = matcher.find_match(&Method::GET, "/documents/file.txt", None, &HeaderMap::new(), None);
  assert!(result.is_none());
}
...
fn test_url_pattern_glob() {
  ...
  let result = matcher.find_match(&Method::GET, "/api/v2/admin/users", None, &HeaderMap::new(), None);
  assert!(result.is_some());
...
fn test_call_tracking_in_matcher() {
  ...
  assert_eq!(registry.get_call_count("test"), 1, "Should track exactly one call");

  let calls = registry.get_calls("test");
  assert!(calls.is_some(), "Should have calls recorded");
  assert_eq!(calls.unwrap().len(), 1, "Should have exactly one recorded call");
}
...
fn test_call_tracking_from_cache() {
  ...
  assert!(result2.is_some(), "Second call should find match from cache");
  ...
  let calls = registry.get_calls("test");
  assert!(calls.is_some(), "Should have calls recorded");
  assert_eq!(calls.unwrap().len(), 2, "Should have exactly two recorded calls");
}
...
fn test_cache_invalidation_on_mock_modification() {
  ...
  headers.insert(HeaderName::from_static("x-test"), HeaderValue::from_static("value"));
  let result = matcher.find_match(&Method::GET, "/test", None, &headers, None);
  assert!(result.is_some(), "Should match with required header present");
}
...
fn test_query_param_url_decoding() {
  ...
  let result = matcher.find_match(&Method::GET, "/test", Some("name=John%20Doe"), &HeaderMap::new(), None);
  assert!(result.is_some(), "Should match URL-encoded query param");
  assert_eq!(result.unwrap().mock.id, "test", "Should match the correct mock");

  // Test that non-encoded version also works
  let result = matcher.find_match(&Method::GET, "/test", Some("name=John Doe"), &HeaderMap::new(), None);
...
fn test_empty_url_patterns_matches_all() {
  let registry = MockRegistry::new();
  let mock = create_test_mock("test", 100, smallvec![], smallvec![], smallvec![], smallvec![], None);
  registry.add_mock(mock);

  let matcher = MockMatcher::new(registry);

  // Should match any URL
  let result = matcher.find_match(&Method::GET, "/any/path/here", None, &HeaderMap::new(), None);
  assert!(result.is_some(), "Should match any URL when patterns are empty");
  assert_eq!(result.unwrap().mock.id, "test", "Should match the correct mock");

  // Test multiple different URLs
  let result = matcher.find_match(&Method::POST, "/completely/different", None, &HeaderMap::new(), None);
...
async fn test_try_match_disabled_registry() {
  ...
  assert!(registry.is_enabled(), "Registry should be enabled initially");
  ...
  let action = matcher
    .try_match_parts(&Method::GET, "/test", None, &HeaderMap::new(), None)
    .await;
  assert!(action.is_none(), "Should not match when registry is disabled");
}
...
async fn test_try_match_with_post_body() {
  ...
  let action = matcher
    .try_match_parts(&Method::POST, "/test", None, &HeaderMap::new(), Some(body.as_slice()))
    .await;
...
async fn test_try_match_template_error() {
  ...
  mock.response.set_body(BodySource::template("{{ invalid syntax"));
  registry.add_mock(mock);
...
async fn test_try_match_static_response() {
  ...
  mock.response.set_body(BodySource::inline(r#"{"static": "response"}"#));
  mock
    .response
    .headers
...
fn test_registry_accessor() {
  ...
  assert!(matcher.registry().is_enabled(), "Registry should be enabled by default");
