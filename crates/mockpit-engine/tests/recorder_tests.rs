...
use mockpit_recorder::{MockRecorder, RecordingFilterOptions, RecordingFormat};
use bytes::Bytes;
...
async fn test_filter_url_pattern() {
  ...
  let recorder = MockRecorder::with_filters("filter-test", temp_dir.path(), RecordingFormat::Json, filter_options);
...
async fn test_capture_errors_only() {
  ...
  let recorder = MockRecorder::with_filters("errors-only", temp_dir.path(), RecordingFormat::Json, filter_options);
...
async fn test_exclude_static_assets() {
  ...
  let recorder = MockRecorder::with_filters("no-static", temp_dir.path(), RecordingFormat::Json, filter_options);
...
async fn test_min_duration_filter() {
  ...
  let recorder = MockRecorder::with_filters("min-duration", temp_dir.path(), RecordingFormat::Json, filter_options);
...
async fn test_auto_export_on_error() {
  ...
  let recorder = MockRecorder::with_filters("auto-export", temp_dir.path(), RecordingFormat::Json, filter_options);
  ...
  assert!(!error_files.is_empty(), "Expected auto-export error file to be created");
  ...
  let collection: serde_json::Value = serde_json::from_str(&error_content).expect("Error file should be valid JSON");
  ...
  let mocks = collection["mocks"].as_array().expect("Should have mocks array");
  ...
  assert!(mocks.len() <= 2, "Should have at most 2 interactions from buffer");
  assert!(!mocks.is_empty(), "Should have at least some interactions exported");
  ...
  assert_eq!(recorder.count(), 3, "Main recorder should still have 3 interactions");
}
...
async fn test_error_context_buffer_circular() {
  ...
  assert!(!error_files.is_empty(), "Expected error export file to be created");
  ...
  // Verify main recorder still has all interactions
  assert_eq!(recorder.count(), 4, "Main recorder should have all 4 interactions");
}
...
async fn test_file_body_storage_threshold() {
  ...
  let body_files: Vec<_> = std::fs::read_dir(&bodies_dir).unwrap().filter_map(|e| e.ok()).collect();
  assert_eq!(
    body_files.len(),
    1,
  ...
  assert_eq!(body_content, large_body, "Body file content should match original data");
...
async fn test_yaml_serialization_format() {
  ...
  assert!(mock.response_config.is_some(), "Mock should have return config");
...
async fn test_file_body_storage_for_html() {
  ...
  let body_file_path = body_files[0].path();
  let body_content = std::fs::read_to_string(&body_file_path).unwrap();
  assert_eq!(body_content, html_body, "Body file should contain the original HTML");
...
async fn test_yaml_format_recording() {
  ...
  assert!(content.contains("/api/users"), "YAML should contain the recorded URL");

  // Verify YAML can be parsed
  let parsed: serde_yaml::Value = serde_yaml::from_str(&content).unwrap();
  assert!(parsed.get("name").is_some(), "Parsed YAML should have name field");
  assert!(parsed.get("mocks").is_some(), "Parsed YAML should have mocks key");

  // Verify recording details
  assert!(content.contains("POST"), "Should contain POST method");
  assert!(content.contains("role=admin"), "Should contain query string");
  assert!(content.contains("Alice"), "Should contain request/response data");
  assert!(
...
async fn test_finalize_and_consolidate() -> Result<()> {
  let temp_dir = TempDir::new().unwrap();
  let recorder = MockRecorder::with_format("consolidate-test", temp_dir.path(), RecordingFormat::Json);
  ...
  let consolidator_options = mockpit_consolidator::ConsolidatorOptions::default();
  let (file_path, stats) = recorder.finalize_and_consolidate(consolidator_options, false).await?;
...
async fn test_finalize_and_consolidate_har_format() -> Result<()> {
  let temp_dir = TempDir::new().unwrap();
  let recorder = MockRecorder::with_format("consolidate-har", temp_dir.path(), RecordingFormat::Har);
  ...
  let consolidator_options = mockpit_consolidator::ConsolidatorOptions::default();
  let (_file_path, stats) = recorder.finalize_and_consolidate(consolidator_options, false).await?;
...
async fn test_binary_data_handling() {
  ...
  assert!(interaction.request.body.as_ref().unwrap().contains("binary data"));
  assert!(interaction.request.body.as_ref().unwrap().contains("8 bytes"));
...
async fn test_large_body_handling() {
  ...
  assert_eq!(interaction.request.body.as_ref().unwrap().len(), 1024 * 1024);
...
async fn test_yaml_streaming_format_multiple() {
  ...
  let parsed: serde_yaml::Value = serde_yaml::from_str(&content).unwrap();
  assert!(parsed.get("name").is_some(), "Parsed YAML should have name field");
  assert!(parsed.get("mocks").is_some(), "Parsed YAML should have mocks array");
...
async fn test_pending_writes_timeout() {
  let temp_dir = TempDir::new().unwrap();
  let recorder = MockRecorder::with_format("pending-writes", temp_dir.path(), RecordingFormat::Json);
  ...
  let collection: mockpit_config::MockCollectionConfig = serde_json::from_str(&content).unwrap();
  assert_eq!(collection.mocks.len(), 100, "Should have all 100 recorded interactions");
  ...
  assert_eq!(recorder.count(), 100, "Recorder should have 100 interactions");
}

#[tokio::test]
async fn test_recording_format_parse() {
  assert!(matches!(RecordingFormat::parse("json").unwrap(), RecordingFormat::Json));
  assert!(matches!(RecordingFormat::parse("yaml").unwrap(), RecordingFormat::Yaml));
  assert!(matches!(RecordingFormat::parse("yml").unwrap(), RecordingFormat::Yaml));
  assert!(matches!(RecordingFormat::parse("har").unwrap(), RecordingFormat::Har));
...
async fn test_combined_filters() {
  ...
  let recorder = MockRecorder::with_filters("combined", temp_dir.path(), RecordingFormat::Json, filter_options);
...
async fn test_neither_capture_filter_set() {
  ...
  let recorder = MockRecorder::with_filters("all-status", temp_dir.path(), RecordingFormat::Json, filter_options);
  ...
  assert_eq!(all.len(), 3, "Should be able to retrieve all 3 interactions");
  ...
  assert!(status_codes.contains(&302), "Should have recorded 302 redirect");
  assert!(status_codes.contains(&500), "Should have recorded 500 error");
...
async fn test_file_body_with_image_content_type() {
  ...
  assert!(content.contains("bodies/"), "Image should be stored in separate file");
...
async fn test_session_name_with_spaces() {
  ...
  let collection: mockpit_config::MockCollectionConfig = serde_json::from_str(&content).unwrap();
  assert_eq!(collection.mocks.len(), 1, "Should have 1 mock in collection");
...
async fn test_finalize_and_consolidate_without_init() {
  ...
  let consolidator_options = mockpit_consolidator::ConsolidatorOptions::default();
  let result = recorder.finalize_and_consolidate(consolidator_options, false).await;
...
async fn test_file_body_storage_for_zip() {
  ...
  assert!(content.contains("bodies/"), "ZIP should be stored in separate file");
}
...
async fn test_file_body_storage_for_video() {
  ...
  assert!(content.contains("bodies/"), "Video should be stored in separate file");
}
...
async fn test_file_body_storage_for_audio() {
  ...
  assert!(content.contains("bodies/"), "Audio should be stored in separate file");
}
...
async fn test_file_body_storage_for_font() {
  ...
  let recorder = MockRecorder::with_filters("font-test", temp_dir.path(), RecordingFormat::Json, filter_options);
  ...
  assert!(content.contains("bodies/"), "Font should be stored in separate file");
}
...
async fn test_file_body_storage_for_css() {
  ...
  let recorder = MockRecorder::with_filters("css-test", temp_dir.path(), RecordingFormat::Json, filter_options);
  ...
  assert!(content.contains("bodies/"), "CSS should be stored in separate file");
}
...
async fn test_har_unknown_status_code() {
  ...
  let recorder = MockRecorder::with_filters("unknown-status", temp_dir.path(), RecordingFormat::Json, filter_options);
...
async fn test_json_streaming_with_special_characters() {
  let temp_dir = TempDir::new().unwrap();
  let recorder = MockRecorder::with_format("json-special", temp_dir.path(), RecordingFormat::Json);
...
async fn test_filter_with_all_static_extensions() {
  ...
  let recorder = MockRecorder::with_filters("static-all", temp_dir.path(), RecordingFormat::Json, filter_options);

  // Test extensions covered by web_static_patterns (JS, CSS, fonts, source maps)
  let filtered_exts = vec![
    ".js", ".mjs", ".jsx", ".ts", ".tsx", ".css", ".scss", ".sass", ".less", ".woff", ".woff2", ".ttf", ".eot", ".otf",
    ".map",
  ];
  ...
  let not_filtered_exts = vec![".png", ".jpg", ".jpeg", ".gif", ".svg", ".ico", ".webp", ".avif"];
...
async fn test_filter_static_case_insensitive() {
  ...
  let recorder = MockRecorder::with_filters("static-case", temp_dir.path(), RecordingFormat::Json, filter_options);
  ...
  assert!(!id3.is_empty(), "PNG should NOT be filtered (could be API content)");
  assert_eq!(recorder.count(), 1);
}
...
async fn test_filter_client_error_status_codes() {
  ...
  let recorder = MockRecorder::with_filters("client-errors", temp_dir.path(), RecordingFormat::Json, filter_options);
  ...
  for code in codes {
    ...
    assert!(!id.is_empty(), "4xx error {} should be captured", code.as_u16());
  }
...
async fn test_filter_server_error_status_codes() {
  ...
  let recorder = MockRecorder::with_filters("server-errors", temp_dir.path(), RecordingFormat::Json, filter_options);
  ...
  for code in codes {
    ...
    assert!(!id.is_empty(), "5xx error {} should be captured", code.as_u16());
  }
...
async fn test_filter_3xx_redirect_codes() {
  ...
  let recorder = MockRecorder::with_filters("redirects", temp_dir.path(), RecordingFormat::Json, filter_options);
  ...
  for code in codes {
    ...
    assert!(!id.is_empty(), "3xx code {} should be captured", code.as_u16());
  }
...
async fn test_filter_regex_pattern_match() {
  ...
  let recorder = MockRecorder::with_filters("regex-filter", temp_dir.path(), RecordingFormat::Json, filter_options);
...
async fn test_auto_export_with_zero_context() {
  ...
  let recorder = MockRecorder::with_filters("zero-context", temp_dir.path(), RecordingFormat::Json, filter_options);
...
async fn test_auto_export_multiple_errors() {
  ...
  let recorder = MockRecorder::with_filters("multi-error", temp_dir.path(), RecordingFormat::Json, filter_options);
...
async fn test_yaml_with_complex_data() {
  let temp_dir = TempDir::new().unwrap();
  let recorder = MockRecorder::with_format("yaml-complex", temp_dir.path(), RecordingFormat::Yaml);

  let complex_json = r#"{"user":{"name":"Test","roles":["admin","user"],"meta":{"created":"2024-01-01"}}}"#;
...
async fn test_yaml_with_multiline_body() {
  let temp_dir = TempDir::new().unwrap();
  let recorder = MockRecorder::with_format("yaml-multiline", temp_dir.path(), RecordingFormat::Yaml);
...
async fn test_finalize_and_consolidate_with_yaml() -> Result<()> {
  let temp_dir = TempDir::new().unwrap();
  let recorder = MockRecorder::with_format("consolidate-yaml", temp_dir.path(), RecordingFormat::Yaml);
  ...
  let content = tokio::fs::read_to_string(&file_path).await?;
  let _collection: mockpit_config::MockCollectionConfig = serde_yaml::from_str(&content)?;
...
async fn test_finalize_and_consolidate_with_yaml_items() -> Result<()> {
  let temp_dir = TempDir::new().unwrap();
  let recorder = MockRecorder::with_format("consolidate-yaml-items", temp_dir.path(), RecordingFormat::Yaml);
...
async fn test_response_headers_preserved() {
  ...
  let header_map: FxHashMap<String, String> = interaction.response.headers.iter().cloned().collect();
...
async fn test_query_string_with_arrays() {
  ...
  let interaction = recorder.get(&id).unwrap();
  assert_eq!(interaction.request.query.as_ref().unwrap(), "ids=1&ids=2&ids=3");
}
