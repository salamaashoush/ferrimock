...
use mockpit_config::{HeaderMatchConfig, MatchConfig, MockCollectionConfig, MockConfig, RequestConfig, ReturnConfig};
...
async fn test_from_file_json_with_log_in_content() {
  ...
  let content = r#"{"mocks": [{"id": "test", "match": {"url": "/log"}, "return": {"status": 200}}]}"#;
...
async fn test_into_mock_definitions_with_dir() {
  ...
  let result = config.into_mock_definitions_with_dir(Some(config_dir), None).await;
  assert!(result.is_ok());
...
async fn test_mock_config_missing_match() {
  ...
  assert!(result.is_err());
  assert!(result.unwrap_err().contains("Missing 'match' configuration"));
}
...
"#;
  ...
  let result = serde_yaml::from_str::<MockCollectionConfig>(yaml);
  assert!(result.is_err());
  assert!(result.unwrap_err().to_string().contains("Cannot mix status shortcuts"));
}
...
"#;
  ...
  let config: MockCollectionConfig = serde_yaml::from_str(yaml).unwrap();
  let mock_def = config.into_mock_definitions().await.unwrap();
  assert_eq!(mock_def[0].response.delay, Some(std::time::Duration::from_micros(500)));
}
...
"#;
  ...
  let config: MockCollectionConfig = serde_yaml::from_str(yaml).unwrap();
  let mock_def = config.into_mock_definitions().await.unwrap();
  assert_eq!(mock_def[0].response.delay, Some(std::time::Duration::from_millis(250)));
}
...
"#;
  ...
  let config: MockCollectionConfig = serde_yaml::from_str(yaml).unwrap();
  let mock_def = config.into_mock_definitions().await.unwrap();
  assert_eq!(mock_def[0].response.delay, Some(std::time::Duration::from_secs(2)));
}
...
"#;
  ...
  let config: MockCollectionConfig = serde_yaml::from_str(yaml).unwrap();
  let mock_def = config.into_mock_definitions().await.unwrap();
  assert_eq!(mock_def[0].response.delay, Some(std::time::Duration::from_millis(0)));
}
...
async fn test_body_file_with_config_dir() {
  ...
  let result = config.into_mock_definition_with_dir(Some(temp_dir.path())).await;
  assert!(result.is_ok());
}
...
"#;
  ...
  let request = mock.request.as_ref().expect("request transforms should exist");

  assert_eq!(request.delay.as_deref(), Some("500ms"));
  assert_eq!(request.timeout.as_deref(), Some("10s"));
  assert_eq!(request.forward_to.as_deref(), Some("https://staging.example.com"));
  assert_eq!(request.rewrite_path.as_deref(), Some("/v2/users/{{ captures.id }}"));
...
"#;
  ...
  assert_eq!(request.forward_to.as_deref(), Some("https://internal.example.com"));
...
"#;
  ...
  let config = MockCollectionConfig::from_yaml(yaml).unwrap();
  let result = config.into_mock_definitions().await;
  assert!(result.is_ok(), "Should succeed for top-level patch without body");
}
