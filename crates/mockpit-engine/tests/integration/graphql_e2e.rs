...
use mockpit_engine::{
  BodySource, MockDefinition, MockMatcher, MockRegistry, RequestMatcher, ResponseGenerator, UrlPattern,
};
    ...
    request: RequestMatcher {
      ...
      graphql_matcher: Some(
        GraphQLMatchConfig::Structured {
          ...
          subscription: None,
          introspection: None,
          variables: vec![("input.role".to_string(), json!("admin"))].into_iter().collect(),
        }
        .into_graphql_matcher()
        .unwrap(),
      ),
...
fn test_e2e_introspection_query() {
  ...
  let mock = MockDefinition {
    ...
    request: RequestMatcher {
      ...
      header_matchers: smallvec![],
      body_matcher: None,
      graphql_matcher: Some(GraphQLMatchConfig::Boolean(true).into_graphql_matcher().unwrap()),
      query_matchers: smallvec![],
    },
    response: ResponseGenerator::new(
      StatusCode::OK,
...
fn test_e2e_priority_based_error_handling() {
  ...
  let body = graphql_request_with_operation_name(query, Some(invalid_variables), Some("CreateUser"));
  let headers = HeaderMap::new();
  ...
  let body = graphql_request_with_operation_name(query, Some(valid_variables), Some("CreateUser"));
  let result = matcher.find_match(&Method::POST, "/graphql", None, &headers, Some(&body));
  assert!(result.is_some());
  let matched = result.unwrap();
...
fn test_e2e_apollo_client_headers() {
  ...
  let mock = MockDefinition {
    ...
    request: RequestMatcher {
      methods: smallvec![Method::POST],
      url_patterns: smallvec![UrlPattern::exact("/graphql")],
      header_matchers: smallvec![mockpit_engine::HeaderMatcher::present(HeaderName::from_static(
        "content-type",
      ))],
      body_matcher: None,
      graphql_matcher: Some(
    ...
    response: ResponseGenerator::new(StatusCode::OK, BodySource::inline(r#"{"data": {"items": []}}"#)),
    vars: None,
  };
...
fn test_e2e_variable_based_routing() {
  ...
  let default_mock = MockDefinition {
    ...
    response: ResponseGenerator::new(StatusCode::OK, BodySource::inline(r#"{"data": {"users": []}}"#)),
    vars: None,
  };
  ...
  let result = matcher.find_match(&Method::POST, "/graphql", None, &HeaderMap::new(), Some(&body));
  assert!(result.is_some());
