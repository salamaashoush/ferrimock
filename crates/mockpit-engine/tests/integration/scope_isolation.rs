...
use mockpit_engine::{BodySource, MockDefinition, MockRegistry, RequestMatcher, ResponseGenerator, UrlPattern};
...
fn test_scope_isolation_between_tests() {
  ...
  let mock1 = MockDefinition {
    ...
    response: ResponseGenerator::new(StatusCode::OK, BodySource::inline(r#"{"token":"suite1-token"}"#)),
    vars: None,
  };
  ...
  let mock3 = MockDefinition {
    ...
    response: ResponseGenerator::new(StatusCode::OK, BodySource::inline(r#"{"token":"suite2-token"}"#)),
    vars: None,
  };
  ...
  let deleted = registry.delete_scope("test-suite-1").expect("Failed to delete scope");
  ...
  registry.delete_scope("test-suite-2").expect("Failed to delete scope");
...
fn test_scope_ttl_expiration() {
  ...
  let permanent_mock = MockDefinition {
    ...
    response: ResponseGenerator::new(StatusCode::OK, BodySource::inline(r#"{"permanent":true}"#)),
    vars: None,
  };
...
fn test_scope_info_queries() {
  ...
  for i in 0..3 {
    let mock = MockDefinition {
      ...
      response: ResponseGenerator::new(StatusCode::OK, BodySource::inline(r#"{"test":"api"}"#)),
      vars: None,
    };
    registry.add_mock(mock);
  }
  ...
  for i in 0..2 {
    let mock = MockDefinition {
      ...
      response: ResponseGenerator::new(StatusCode::OK, BodySource::inline(r#"{"test":"ui"}"#)),
      vars: None,
    };
    registry.add_mock(mock);
  }
  ...
  // Test get_scope_info
  let api_info = registry.get_scope_info("api-tests").expect("Should get api-tests info");
  assert_eq!(api_info.id, "api-tests");
  assert_eq!(api_info.mock_count, 3);
  assert!(api_info.expires_at.is_some());

  let ui_info = registry.get_scope_info("ui-tests").expect("Should get ui-tests info");
...
fn test_realistic_test_suite_workflow() {
  ...
  // Test 1: User login flow
  {
    ...
    registry.add_mock(MockDefinition {
      ...
      response: ResponseGenerator::new(StatusCode::OK, BodySource::inline(r#"{"id":"123","name":"Test User"}"#)),
      vars: None,
    });
  ...
  // Test 2: File upload flow (isolated from test 1)
  {
    ...
    registry.add_mock(MockDefinition {
      ...
      response: ResponseGenerator::new(StatusCode::CREATED, BodySource::inline(r#"{"file_id":"file-456"}"#)),
      vars: None,
    });
  ...
  {
    registry.create_scope("parallel-test-1".into(), None).unwrap();
    registry.create_scope("parallel-test-2".into(), None).unwrap();
    registry.create_scope("parallel-test-3".into(), None).unwrap();
    ...
    for i in 1..=3 {
      ...
      registry.add_mock(MockDefinition {
        ...
        response: ResponseGenerator::new(StatusCode::OK, BodySource::inline(format!(r#"{{"test":{}}}"#, i))),
        vars: None,
      });
    }
