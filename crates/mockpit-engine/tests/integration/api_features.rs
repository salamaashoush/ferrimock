...
use mockpit_engine::matcher::MockMatcher;
...
fn create_test_mock(id: &str, path: &str) -> MockDefinition {
  MockDefinition {
    ...
    response: ResponseGenerator::new(StatusCode::OK, BodySource::inline("{\"status\": \"ok\"}")),
    vars: None,
  }
}
...
fn test_call_tracking_with_different_methods() {
  ...
  let methods = vec![Method::GET, Method::POST, Method::PUT, Method::GET, Method::POST];
...
fn test_call_tracking_with_query_params() {
  ...
  matcher.find_match(&Method::GET, "/api/search", Some("q=test"), &HeaderMap::new(), None);
  matcher.find_match(
    &Method::GET,
    "/api/search",
    Some("q=foo&limit=10"),
...
fn test_call_tracking_with_headers() {
  ...
  assert_eq!(call_headers.get("authorization"), Some(&"Bearer token123".to_string()));
  assert_eq!(call_headers.get("content-type"), Some(&"application/json".to_string()));
}
...
fn test_reset_with_call_tracking() {
  ...
  // Generate some calls
  for i in 1..=3 {
    matcher.find_match(&Method::GET, &format!("/api/{}", i), None, &HeaderMap::new(), None);
  }
