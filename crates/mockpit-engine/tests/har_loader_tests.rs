...
use mockpit_engine::har_export::export_mocks_to_har;
use mockpit_engine::{BodySource, MockDefinition, RequestMatcher, ResponseGenerator, UrlPattern};
...
async fn test_url_with_fragment() {
  let har = Har {
    log: Spec::V1_2(v1_2::Log {
      ...
      browser: None,
      pages: None,
      entries: vec![create_test_entry("GET", "https://api.example.com/page#section", 200)],
      comment: None,
    }),
  };
...
async fn test_various_2xx_status_codes() {
  ...
  assert_eq!(mocks[0].response_config.as_ref().unwrap().status().unwrap(), 200);
  assert_eq!(mocks[1].response_config.as_ref().unwrap().status().unwrap(), 201);
  assert_eq!(mocks[2].response_config.as_ref().unwrap().status().unwrap(), 202);
  assert_eq!(mocks[3].response_config.as_ref().unwrap().status().unwrap(), 204);
}
...
async fn test_3xx_redirect_codes() {
  ...
  assert_eq!(mocks.len(), 4);
  assert_eq!(mocks[0].response_config.as_ref().unwrap().status().unwrap(), 301);
  assert_eq!(mocks[1].response_config.as_ref().unwrap().status().unwrap(), 302);
  assert_eq!(mocks[2].response_config.as_ref().unwrap().status().unwrap(), 304);
  assert_eq!(mocks[3].response_config.as_ref().unwrap().status().unwrap(), 307);
}
...
async fn test_4xx_client_error_codes() {
  ...
  assert_eq!(mocks[0].response_config.as_ref().unwrap().status().unwrap(), 400);
  assert_eq!(mocks[1].response_config.as_ref().unwrap().status().unwrap(), 401);
  assert_eq!(mocks[2].response_config.as_ref().unwrap().status().unwrap(), 403);
  assert_eq!(mocks[3].response_config.as_ref().unwrap().status().unwrap(), 404);
  assert_eq!(mocks[4].response_config.as_ref().unwrap().status().unwrap(), 429);
}
...
async fn test_5xx_server_error_codes() {
  ...
  assert_eq!(mocks.len(), 4);
  assert_eq!(mocks[0].response_config.as_ref().unwrap().status().unwrap(), 500);
  assert_eq!(mocks[1].response_config.as_ref().unwrap().status().unwrap(), 502);
  assert_eq!(mocks[2].response_config.as_ref().unwrap().status().unwrap(), 503);
  assert_eq!(mocks[3].response_config.as_ref().unwrap().status().unwrap(), 504);
}
...
async fn test_export_single_mock_to_har() {
  let mock = MockDefinition {
    ...
    response: ResponseGenerator::new(StatusCode::OK, BodySource::inline(r#"{"id":"123","name":"Test"}"#)),
    vars: None,
  };
