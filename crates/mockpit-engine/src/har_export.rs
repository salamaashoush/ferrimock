  ...
  Ok(Har {
    log: Spec::V1_2(v1_2::Log {
      creator: v1_2::Creator {
        ...
        comment: Some("Exported from the internal gateway mock definitions".to_string()),
      },
      browser: None,
      pages: None,
      entries,
      comment: Some("Static snapshot of mock responses. Templates/dynamic features not preserved.".to_string()),
    }),
  })
}
...
fn mock_to_har_entry(mock: &MockDefinition, _index: usize) -> v1_2::Entries {
  ...
  let response_body = match &mock.response.body {
    BodySource::Inline(content) => {
      String::from_utf8(content.to_vec()).unwrap_or_else(|_| format!("<binary data: {} bytes>", content.len()))
    },
    BodySource::File(path) => format!("<file: {}>", path.display()),
    BodySource::FileCached(content) => {
      String::from_utf8(content.to_vec()).unwrap_or_else(|_| format!("<binary data: {} bytes>", content.len()))
    },
