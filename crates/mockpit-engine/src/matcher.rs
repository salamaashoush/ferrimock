...
use crate::types::{MockDefinition, PatchOperation, RequestPatch, ResponseGeneratorExt, UpstreamOptions};
...
impl MockMatcher {
  ...
  pub fn find_match(
    ...
    let parsed_query = if query.is_some() && all_mocks.iter().any(|m| !m.request.query_matchers.is_empty()) {
  ...
  fn matches_url(&self, mock: &MockDefinition, path: &str, query: Option<&str>) -> bool {
    ...
    if mock.request.url_patterns.iter().any(|pattern| pattern.matches(path)) {
      return true;
    }
  ...
  fn matches_introspection(&self, matcher: &crate::types::IntrospectionMatcher, json: &serde_json::Value) -> bool {
  ...
  fn get_nested_json_value<'a>(json: Option<&'a serde_json::Value>, path: &str) -> Option<&'a serde_json::Value> {
  ...
  pub async fn try_match_parts(
    ...
    let has_response_patches = matches!(&mock_def.response.mode, crate::types::ResponseMode::Patch { .. });
    ...
    if has_request_transforms || has_response_patches {
      ...
      let (request_patches, pre_delay, upstream_options, rewrite_path) = if let Some(rt) = &mock_def.request_transforms
      {
    ...
    let dynamic_result = if matches!(response_generator.body, crate::types::BodySource::Template { .. }) {
    ...
    let dynamic_response = match dynamic_result {
      ...
      Err(e) => {
        ...
        let error_message = format!("Mock '{}' failed to generate response: {}", mock_def.id, e);
  ...
  pub async fn apply_patches(
    ...
    request_context: Option<mockpit_types::RequestContext>,
    ...
    let patched_response = patcher.apply(patchable_response, patch_context.as_ref()).await?;
...
mod tests {
  ...
  fn test_match_exact_url() {
    ...
    assert!(
      matcher
        .find_match(&Method::GET, "/api/users/123", None, &HeaderMap::new(), None)
        .is_none()
    );
  }
  ...
  fn test_match_regex_url() {
    ...
    assert!(
      matcher
        .find_match(&Method::GET, "/api/users/456", None, &HeaderMap::new(), None)
        .is_some()
    );
    assert!(
      matcher
        .find_match(&Method::GET, "/api/users/abc", None, &HeaderMap::new(), None)
        .is_none()
    );
  }
  ...
  fn test_match_multiple_url_patterns() {
    ...
    registry.add_mock(create_mock(
      ...
      100,
      vec![],
      vec![UrlPattern::exact("/api/users"), UrlPattern::exact("/api/files")],
      vec![],
    ));
  ...
  fn test_match_header_presence() {
    ...
    registry.add_mock(create_mock(
      ...
      vec![],
      vec![UrlPattern::exact("/test")],
      vec![HeaderMatcher::present(HeaderName::from_static("authorization"))],
    ));
  ...
  fn test_disabled_mock_not_matched() {
    let registry = MockRegistry::new();
    let mut mock = create_mock("test", 100, vec![], vec![UrlPattern::exact("/test")], vec![]);
    mock.enabled = false;
  ...
  fn test_graphql_query_by_type() {
    ...
    let mutation_body =
      r#"{"query":"mutation CreateUser { createUser(name: \"John\") { id } }","operationName":"CreateUser"}"#;
    assert!(
      matcher
        .find_match(
          &Method::POST,
  ...
  fn test_graphql_introspection_any() {
    ...
    let schema_body =
      r#"{"query":"query IntrospectionQuery { __schema { types { name } } }","operationName":"IntrospectionQuery"}"#;
    assert!(
      matcher
        .find_match(
          &Method::POST,
    ...
    let regular_body = r#"{"query":"query GetUser { user { name } }","operationName":"GetUser"}"#;
    assert!(
      matcher
        .find_match(
          &Method::POST,
  ...
  fn test_graphql_wildcard_match() {
    ...
    let mutation_body = r#"{"query":"mutation CreateUser { createUser { id } }","operationName":"CreateUser"}"#;
    assert!(
      matcher
        .find_match(
          &Method::POST,
