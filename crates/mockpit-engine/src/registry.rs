...
use crate::scope::{ScopeInfo, ScopeManager};
use crate::types::MockDefinition;
use mockpit_core::PersistenceStore;
use mockpit_recorder::RecordedInteraction;
use mockpit_types::LeanString;
...
impl SortedMocksCache {
  ...
  fn update(&self, mocks: Vec<Arc<MockDefinition>>) {
    ...
    self.cached_version.store(current_version, Ordering::Release);
  }
}
...
impl MockRegistry {
  ...
  pub fn new() -> Self {
    ...
    let persistence_store = mockpit_template::get_global_persistence_store();
  ...
  pub async fn load_from_directory(&self, dir_path: &str) -> Result<usize, String> {
    ...
    let har_tasks: Vec<_> = har_files.iter().map(|path| self.load_har_file(path)).collect();
    ...
    for (i, result) in har_results.into_iter().enumerate() {
      match result {
        ...
        Err(e) => {
          eprintln!("Warning: Failed to load HAR file from {:?}: {}", har_files[i], e);
        },
      }
    }
  ...
  fn ensure_exact_index(&self) {
    ...
    let sorted_cached = self.sorted_mocks_cache.cached_version.load(Ordering::Acquire);
    ...
    for mock in &enabled {
      ...
      if !is_conditional && mock.request.url_patterns.len() == 1 && mock.request_transforms.is_none() {
    ...
    self.has_conditional_mocks.store(has_conditional, Ordering::Release);
    *self.exact_match_index.write() = index;
    ...
    self.exact_index_version.store(current_sorted, Ordering::Release);
  }

  /// Try to find an exact match via the index. O(1) lookup, zero allocation.
  /// Returns None if no indexed mock matches.
  pub fn try_exact_match(&self, method: &http::Method, path: &str) -> Option<Arc<MockDefinition>> {
  ...
  pub fn create_scope(&self, id: LeanString, ttl: Option<std::time::Duration>) -> Result<ScopeInfo, String> {
  ...
  pub fn enable_call_tracking(&self, mock_id: &str, max_calls: Option<usize>) {
    ...
    self.call_tracking.insert(mock_id.into(), VecDeque::with_capacity(max));
  }
  ...
  pub fn get_call_count(&self, mock_id: &str) -> usize {
    self.call_tracking.get(mock_id).map(|v| v.len()).unwrap_or(0)
  }
  ...
  pub fn get_tracked_mock_ids(&self) -> Vec<String> {
    self.call_tracking.iter().map(|entry| entry.key().to_string()).collect()
  }
  ...
  fn validate_mock_templates(mock: &crate::MockDefinition) -> Result<(), String> {
    // Check if the response body is a template
    if let crate::BodySource::Template { source: template, .. } = &mock.response.body {
      if let Err(e) = mockpit_template::validate_template(template) {
        return Err(format!("Mock '{}': Template validation failed: {}", mock.id, e));
      }
    }
...
mod tests {
  ...
  fn create_test_mock_with_scope(id: &str, priority: u32, enabled: bool, scope: Option<LeanString>) -> MockDefinition {
    MockDefinition {
      id: id.into(),
      priority,
      enabled,
  ...
  fn test_create_scope_with_ttl() {
    ...
    let info = registry.create_scope("test-scope".into(), Some(ttl)).unwrap();
  ...
  fn test_get_mocks_by_scope() {
    ...
    registry.create_scope("scope1".into(), None).unwrap();
    registry.add_mock(create_test_mock_with_scope("mock1", 100, true, Some("scope1".into())));
    registry.add_mock(create_test_mock_with_scope("mock2", 200, true, Some("scope1".into())));
  ...
  fn test_cleanup_expired_scopes() {
    ...
    registry
      ...
      .unwrap();
    registry.add_mock(create_test_mock_with_scope("mock1", 100, true, Some("expired".into())));
