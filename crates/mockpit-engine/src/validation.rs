...
use mockpit_config::MockCollectionConfig;
use mockpit_template as template;
use http::Method;
...
impl MockValidator {
  ...
  pub async fn validate_file(&self, path: &Path) -> ValidationResult {
    ...
    let config_result = match extension {
      Some("json") => {
        serde_json::from_str::<MockCollectionConfig>(&content).map_err(|e| format!("JSON parse error: {}", e))
      },
      Some("yaml") | Some("yml") => {
        serde_yaml::from_str::<MockCollectionConfig>(&content).map_err(|e| format!("YAML parse error: {}", e))
      },
      _ => {
        return ValidationResult {
          file_path,
    ...
    match config_result {
      Ok(config) => {
        ...
        self
          .validate_config_internal(&config, path.parent(), &content, file_path)
          .await
      },
      Err(e) => {
  ...
  pub async fn validate_config(&self, config: &MockCollectionConfig, config_dir: Option<&Path>) -> ValidationResult {
    self.validate_config_internal(config, config_dir, "", None).await
  }
  ...
  pub async fn validate_content(&self, content: &str, extension: &str) -> ValidationResult {
    let config_result = match extension {
      "json" => serde_json::from_str::<MockCollectionConfig>(content).map_err(|e| format!("JSON parse error: {}", e)),
      "yaml" | "yml" => {
        serde_yaml::from_str::<MockCollectionConfig>(content).map_err(|e| format!("YAML parse error: {}", e))
      },
      _ => {
    ...
    match config_result {
      Ok(config) => self.validate_config_internal(&config, None, content, None).await,
      Err(e) => {
        let (line_number, snippet) = Self::extract_parse_error_info(&e, content);
        ValidationResult {
          file_path: None,
  ...
  async fn validate_config_internal(
    ...
    for mock in &config.mocks {
      ...
      if !seen_ids.insert(&mock.id) {
        ...
        warnings.push(ValidationWarning {
          ...
          line_number,
          snippet: line_number.and_then(|line| Self::extract_snippet(file_content, line, &mock.id)),
      ...
      if let Some(ref match_config) = mock.match_config {
        ...
        errors.push(ValidationError {
          ...
          snippet: None,
          suggestion: Some("Add match configuration: match.url or match.methods".to_string()),
          line_number: None,
        });
      }
      ...
      if let Some(ref response_config) = mock.response_config {
        ...
        if let Some(status) = response_config.status() {
          if !(100..=599).contains(&status) {
            ...
            errors.push(ValidationError {
              ...
              snippet: line_number.and_then(|line| Self::extract_snippet(file_content, line, &status_str)),
          ...
          if let Some(dir) = config_dir {
            ...
            if !full_path.exists() {
              ...
              errors.push(ValidationError {
                ...
                snippet: line_number.and_then(|line| Self::extract_snippet(file_content, line, tf_ref)),
              ...
              if let Ok(template_content) = std::fs::read_to_string(&full_path) {
                if let Err(e) = template::validate_template(&template_content) {
                  errors.push(ValidationError {
                    mock_id: mock_id.clone(),
                    error_type: ErrorType::TemplateError,
                    message: format!("Template file '{}' validation failed: {}", tf_ref, e),
                    snippet: None,
          ...
          if let Some(dir) = config_dir {
            ...
            if !full_path.exists() {
              ...
              errors.push(ValidationError {
                ...
                snippet: line_number.and_then(|line| Self::extract_snippet(file_content, line, file_ref)),
                suggestion: Some(format!("Expected file at: {}", full_path.display())),
                line_number,
              });
            }
        ...
        if !has_request_transforms && !has_patch && !has_delay {
          errors.push(ValidationError {
            ...
            snippet: None,
            suggestion: Some("Add response, patch, delay, or request transform configuration".to_string()),
            line_number: None,
          });
        }
      }
      ...
      if let Some(ref delay) = mock.delay {
        if let Err(_e) = mockpit_config::response::parse_duration(delay) {
          ...
          errors.push(ValidationError {
            ...
            snippet: line_number.and_then(|line| Self::extract_snippet(file_content, line, delay)),
            suggestion: Some("Duration must have a unit suffix: '100ms', '2s', '500us'".to_string()),
            line_number,
          });
        }
      }
      ...
      if let Some(ref patches_config) = mock.patch {
        ...
        for regex_config in &patches_config.regex {
          ...
          if let Err(e) = Regex::new(&regex_config.pattern) {
            let line_number = Self::find_line_number(file_content, &regex_config.pattern);
            errors.push(ValidationError {
              mock_id: mock_id.clone(),
          ...
          if regex_config.replacement.contains("{{") || regex_config.replacement.contains("{%") {
            if let Err(e) = template::validate_template(&regex_config.replacement) {
              let line_number = Self::find_line_number(file_content, &regex_config.replacement);
              errors.push(ValidationError {
                mock_id: mock_id.clone(),
                error_type: ErrorType::TemplateError,
      ...
      if let Some(ref request_config) = mock.request {
        if request_config.is_empty() {
          ...
          warnings.push(ValidationWarning {
            ...
            line_number,
            snippet: None,
            suggestion: Some("Add request transform fields or remove the [request] section".to_string()),
          });
        }
        ...
        if let Some(ref timeout) = request_config.timeout {
          if let Err(_e) = mockpit_config::response::parse_duration(timeout) {
            ...
            errors.push(ValidationError {
              ...
              snippet: line_number.and_then(|line| Self::extract_snippet(file_content, line, timeout)),
              suggestion: Some("Duration must have a unit suffix: '100ms', '2s', '500us'".to_string()),
              line_number,
            });
          }
    ...
    match config.clone().into_mock_definitions_with_dir(config_dir, None).await {
      Ok(mock_defs) => {
  ...
  fn extract_parse_error_info(error_msg: &str, content: &str) -> (Option<usize>, Option<CodeSnippet>) {
    // Try to extract line number from common error formats
    // JSON: "... at line 10 column 5"
    // YAML: "... at line 3"

    let line_number = if let Some(captures) = Regex::new(r"line (\d+)").ok().and_then(|re| re.captures(error_msg)) {
      captures.get(1).and_then(|m| m.as_str().parse::<usize>().ok())
    } else {
      None
    };
  ...
  fn mocks_may_overlap(mock1: &MockDefinition, mock2: &MockDefinition) -> bool {
    ...
    if !mock1.request.methods.is_empty() && !mock2.request.methods.is_empty() {
      let methods_overlap = mock1.request.methods.iter().any(|m| mock2.request.methods.contains(m));
      if !methods_overlap {
        return false;
      }
    }
    ...
    let urls_overlap = if !mock1.request.url_patterns.is_empty() && !mock2.request.url_patterns.is_empty() {
    ...
    if let (Some(gql1), Some(gql2)) = (&mock1.request.graphql_matcher, &mock2.request.graphql_matcher) {
    ...
    if let (Some(body1), Some(body2)) = (&mock1.request.body_matcher, &mock2.request.body_matcher) {
    ...
    if Self::header_matchers_discriminate(&mock1.request.header_matchers, &mock2.request.header_matchers) {
      return false;
    }

    // Mocks with different required query param matchers discriminate
    if Self::query_matchers_discriminate(&mock1.request.query_matchers, &mock2.request.query_matchers) {
      return false;
    }
  ...
  fn url_patterns_may_overlap(
    patterns1: &[mockpit_types::UrlPattern],
    patterns2: &[mockpit_types::UrlPattern],
  ) -> bool {
    ...
    for pattern1 in patterns1 {
      for pattern2 in patterns2 {
        ...
        match (pattern1, pattern2) {
          ...
          (UrlPattern::Exact(s), UrlPattern::Prefix(prefix)) | (UrlPattern::Prefix(prefix), UrlPattern::Exact(s)) => {
          ...
          (UrlPattern::Exact(s), UrlPattern::Suffix(suffix)) | (UrlPattern::Suffix(suffix), UrlPattern::Exact(s)) => {
          ...
          (UrlPattern::Exact(s), UrlPattern::Regex(re)) | (UrlPattern::Regex(re), UrlPattern::Exact(s)) => {
            if re.is_match(s) {
              return true;
            }
          },
          ...
          (UrlPattern::Exact(s), UrlPattern::Glob(g)) | (UrlPattern::Glob(g), UrlPattern::Exact(s)) => {
            if g.is_match(s) {
              return true;
            }
          },
  ...
  fn graphql_matchers_discriminate(
    gql1: &mockpit_types::GraphQLMatcher,
    gql2: &mockpit_types::GraphQLMatcher,
  ) -> bool {
    ...
    if gql1.introspection_matcher.is_some() {
      if let Some(t) = &gql2.operation_type {
        if matches!(
          t,
          mockpit_types::GraphQLOperationType::Mutation | mockpit_types::GraphQLOperationType::Subscription
        ) {
          return true;
        }
      }
  ...
  fn body_matchers_discriminate(body1: &mockpit_types::BodyMatcher, body2: &mockpit_types::BodyMatcher) -> bool {
    use mockpit_types::BodyMatcher;
    // JSONPath matchers on different paths discriminate
    if let (BodyMatcher::JsonPath { path: p1, .. }, BodyMatcher::JsonPath { path: p2, .. }) = (body1, body2) {
      if p1 != p2 {
        return true;
      }
      ...
      for h2 in headers2 {
        if h1.name == h2.name {
          match (&h1.pattern, &h2.pattern) {
            (mockpit_types::HeaderMatchPattern::Exact(v1), mockpit_types::HeaderMatchPattern::Exact(v2))
              if v1 != v2 =>
            {
              return true;
            },
            (mockpit_types::HeaderMatchPattern::Exact(_), mockpit_types::HeaderMatchPattern::Exact(_)) => {},
            (mockpit_types::HeaderMatchPattern::Present, mockpit_types::HeaderMatchPattern::Absent)
            | (mockpit_types::HeaderMatchPattern::Absent, mockpit_types::HeaderMatchPattern::Present) => {
              return true;
            },
            _ => {},
          }
      ...
      for q2 in queries2 {
        if q1.name == q2.name {
          match (&q1.pattern, &q2.pattern) {
            (mockpit_types::QueryMatchPattern::Exact(v1), mockpit_types::QueryMatchPattern::Exact(v2))
              if v1 != v2 =>
            {
              return true;
            },
            (mockpit_types::QueryMatchPattern::Exact(_), mockpit_types::QueryMatchPattern::Exact(_)) => {},
            (mockpit_types::QueryMatchPattern::Present, mockpit_types::QueryMatchPattern::Absent)
            | (mockpit_types::QueryMatchPattern::Absent, mockpit_types::QueryMatchPattern::Present) => {
              return true;
            },
            _ => {},
          }
...
mod tests {
  ...
  async fn test_validate_missing_file() {
    let validator = MockValidator::new();
    let result = validator.validate_file(Path::new("/nonexistent.yaml")).await;

    assert!(result.has_errors());
    assert_eq!(result.error_count(), 1);
    assert!(matches!(result.errors[0].error_type, ErrorType::FileReadError));
  }
  ...
  fn test_url_patterns_different_methods_and_paths() {
    use mockpit_types::{BodySource, RequestMatcher, ResponseGenerator, ResponseMode, UrlPattern};
    ...
    let mock2 = MockDefinition {
      ...
      request: RequestMatcher {
        ...
        url_patterns: smallvec![UrlPattern::regex("^/api/users/(?P<id>[^/]+)$").expect("valid regex")],
        header_matchers: smallvec![],
        query_matchers: smallvec![],
        body_matcher: None,
        graphql_matcher: None,
  ...
  fn test_mocks_with_different_query_params() {
    use mockpit_types::{BodySource, QueryMatcher, RequestMatcher, ResponseGenerator, ResponseMode, UrlPattern};
    use http::{Method, StatusCode};
    use std::sync::Arc;
  ...
  fn test_mocks_with_different_headers() {
    use mockpit_types::{BodySource, HeaderMatcher, RequestMatcher, ResponseGenerator, ResponseMode, UrlPattern};
    use http::header::HeaderName;
    use http::{Method, StatusCode};
    use std::sync::Arc;
