...
fn try_decompress_gzip(data: &Bytes, content_encoding: Option<&str>) -> Option<Bytes> {
  let is_gzipped = content_encoding.map(|e| e.contains("gzip")).unwrap_or(false);
  ...
  pub async fn apply(
    ...
    while i < self.operations.len() {
      match &self.operations[i] {
          ...
          while i < self.operations.len() {
            match &self.operations[i] {
              ...
              PatchOperation::JsonPath { path, value } => {
                ...
                let rendered_value = render_json_value_templates(value, patch_context);
                apply_jsonpath_patch(&mut json, &rendered_path, rendered_value)
                  .map_err(|e| anyhow::anyhow!("JSONPath patch failed for '{}': {}", rendered_path, e))?;
                i += 1;
              },
        ...
        PatchOperation::RegexReplace { pattern, replacement } => {
    ...
    let final_body = if was_compressed {
      match recompress_gzip(&body) {
        ...
        Err(e) => {
          tracing::warn!("Failed to re-compress patched body, returning uncompressed: {}", e);
...
fn render_if_template<'a>(
  ...
  match mockpit_template::render_patch_template(value, ctx, None) {
    ...
    Err(e) => {
      tracing::warn!("Template render failed in patch value, using literal: {}", e);
...
fn render_json_value_templates(
  ...
  match value {
    serde_json::Value::String(s) if patch_context.is_some() && (s.contains("{{") || s.contains("{%")) => {
      let ctx = patch_context.unwrap();
      match mockpit_template::render_patch_template(s, ctx, None) {
        Ok(rendered) => {
          // Try to parse as JSON (number, bool, object, array)
          serde_json::from_str(&rendered).unwrap_or_else(|_| serde_json::Value::String(rendered))
        },
        Err(e) => {
          tracing::warn!("Template render failed in patch JSON value, using literal: {}", e);
          value.clone()
        },
      }
    },
...
fn parse_path_segment(segment: &str) -> Result<PathSegment<'_>, anyhow::Error> {
  if let Some(bracket_pos) = segment.find('[') {
    if !segment.ends_with(']') {
      return Err(anyhow::anyhow!("Invalid array notation in path segment: {}", segment));
    }
    ...
    if index_str.contains(',') {
      let indices: Result<Vec<usize>, _> = index_str.split(',').map(|s| s.trim().parse::<usize>()).collect();
      let indices =
        indices.map_err(|_| anyhow::anyhow!("Invalid multi-index '{}' in path segment: {}", index_str, segment))?;
      return Ok(PathSegment::ArrayMultiIndex(field, indices));
    }

    // Check for inclusive range (..=)
    if let Some(range_pos) = index_str.find("..=") {
      let start: usize = index_str[..range_pos]
        .trim()
        .parse()
        .map_err(|_| anyhow::anyhow!("Invalid range start in '{}' for segment: {}", index_str, segment))?;
      let end: usize = index_str[range_pos + 3..]
        .trim()
        .parse()
        .map_err(|_| anyhow::anyhow!("Invalid range end in '{}' for segment: {}", index_str, segment))?;
    ...
    if let Some(range_pos) = index_str.find("..") {
      let start: usize = index_str[..range_pos]
        ...
        .parse()
        .map_err(|_| anyhow::anyhow!("Invalid range start in '{}' for segment: {}", index_str, segment))?;
      let end: usize = index_str[range_pos + 2..]
        .trim()
        .parse()
        .map_err(|_| anyhow::anyhow!("Invalid range end in '{}' for segment: {}", index_str, segment))?;
    ...
    let index: usize = index_str
      .parse()
      .map_err(|_| anyhow::anyhow!("Invalid array index '{}' in path segment: {}", index_str, segment))?;
...
fn is_multi_element_selector(segment: &PathSegment<'_>) -> bool {
  matches!(
    segment,
    PathSegment::ArrayWildcard(_) | PathSegment::ArrayMultiIndex(_, _) | PathSegment::ArrayRange(_, _, _, _)
  )
}
...
fn format_available_keys(obj: &serde_json::Map<String, serde_json::Value>) -> String {
  ...
  if keys.is_empty() {
    ...
    format!("[{}, ... and {} more]", keys.join(", "), obj.len() - keys.len())
  } else {
    format!("[{}]", keys.join(", "))
  }
}
    ...
    PathSegment::ArrayIndex(field, index) => {
      ...
      if let Some(obj) = current.as_object_mut() {
        ...
        if let Some(arr) = arr_val.as_array_mut() {
          ...
          arr
            .get_mut(*index)
            .ok_or_else(|| anyhow::anyhow!("Array index {} out of bounds for '{}' (length: {})", index, field, len))
        } else {
          Err(anyhow::anyhow!("Field '{}' is {} (expected array)", field, arr_type))
        }
      } else {
        Err(anyhow::anyhow!(
      ...
      match segment {
        ...
        PathSegment::ArrayIndex(field, index) => {
          if let Some(obj) = current.as_object_mut() {
            ...
            let arr = obj
              .get_mut(*field)
              .ok_or_else(|| anyhow::anyhow!("Field '{}' not found. Available keys: {}", field, available))?;
...
mod tests {
  ...
  async fn test_jsonpath_patch_array_wildcard_nested() {
    let json_body = r#"{"doc": {"signers": [{"meta": {"active": false}}, {"meta": {"active": false}}]}}"#;
    let response = Response::builder()
      .status(StatusCode::OK)
      .body(Bytes::from(json_body))
      .unwrap();
  ...
  fn test_jsonpath_error_shows_available_keys_at_correct_level() {
    ...
    let json_body = r#"{"signrequest": {"id": "123", "status": "pending"}, "doc": {"name": "test.pdf"}}"#;

    let mut json: serde_json::Value = serde_json::from_str(json_body).unwrap();
    let result = apply_jsonpath_patch(&mut json, "$.signrequest.signers[*].language", serde_json::json!("ja"));
  ...
  fn test_jsonpath_error_shows_type_mismatch() {
    ...
    let mut json: serde_json::Value = serde_json::from_str(json_body).unwrap();
    let result = apply_jsonpath_patch(&mut json, "$.user.name[0].foo", serde_json::json!("bar"));
  ...
  fn make_patch_context() -> mockpit_types::PatchContext {
    ...
    mockpit_types::PatchContext {
      request: mockpit_types::RequestContext {
  ...
  async fn test_no_context_means_no_rendering() {
    ...
    assert_eq!(patched.headers().get("x-test").unwrap(), "{{ captures.id }}");
  }
  ...
  async fn test_template_response_headers_access() {
    ...
    let patched = patcher.apply(response, Some(&ctx)).await.unwrap();
    assert_eq!(patched.headers().get("x-echo-request-id").unwrap(), "req-abc-123");
  }
}
