...
impl RequestPatcher {
  ...
  pub fn apply(
    ...
    for op in &self.operations {
      match op {
        RequestPatch::HeaderAdd { name, value } => {
          let header_name =
            HeaderName::try_from(name.as_str()).with_context(|| format!("Invalid header name: {}", name))?;
        ...
        RequestPatch::QueryAdd { name, value } => {
          let params = query_params.get_or_insert_with(|| parse_query_string(query.unwrap_or("")));
        ...
        RequestPatch::JsonPath { path, value } => {
          ...
          set_jsonpath_value(body_val, path, value.clone())
            .with_context(|| format!("Failed to set JSONPath '{}' on request body", path))?;
        },
        RequestPatch::JsonPatch(patch) => {
          ...
          json_patch::patch(body_val, patch).context("Failed to apply JSON Patch to request body")?;
        },
        RequestPatch::RegexReplace { pattern, replacement } => {
          // If we have a pending json_body modification, serialize it first
          if let Some(json_val) = json_body.take() {
            let serialized = serde_json::to_string(&json_val).context("Failed to serialize JSON body")?;
    ...
    let final_body = if let Some(json_val) = json_body {
      let serialized = serde_json::to_vec(&json_val).context("Failed to serialize patched request body")?;
      // Update Content-Length header
      headers.insert(http::header::CONTENT_LENGTH, HeaderValue::from(serialized.len()));
...
fn set_jsonpath_value(root: &mut serde_json::Value, path: &str, value: serde_json::Value) -> Result<()> {
...
fn set_value_at_path(current: &mut serde_json::Value, segments: &[&str], value: serde_json::Value) -> Result<()> {
...
mod tests {
  ...
  fn test_query_add() {
    ...
    let (_, _, query) = patcher.apply(HeaderMap::new(), None, Some("existing=yes")).unwrap();
  ...
  fn test_query_remove() {
    ...
    let (_, _, query) = patcher
      .apply(HeaderMap::new(), None, Some("keep=yes&sensitive=secret&also_keep=true"))
      .unwrap();
    let query = query.unwrap();
