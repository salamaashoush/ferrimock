        ...
        if let Some((block_content, end_pos)) = tera_block {
          ...
          if let Some(keyword) = extract_tera_keyword(&block_content) {
            if is_always_structural(keyword) {
              ...
              let at_line_start = output_ends_with_indent(&output, depth + tera_block_depth, indent_str)
                || output.ends_with('\n')
                || output.is_empty();
...
\n}",
  ...
  fn test_offset_pagination_pattern() {
    ...
    let set_lines: Vec<&str> = lines
      ...
      .copied()
      .collect();
    assert_eq!(set_lines.len(), 3, "Should have 3 set lines. Got:\n{}", result);
...
\n}",
  ...
  fn test_normalize_complex_expression() {
    assert_eq!(normalize_tera_block("{{fake_name()|upper}}"), "{{ fake_name()|upper }}");
  }
  ...
  fn test_extract_keyword_all_types() {
    assert_eq!(extract_tera_keyword("{% for i in range(end=3) %}"), Some("for"));
  ...
  fn test_classification_tera_blocks_then_json() {
    assert!(looks_like_json_with_tera("{%- set x = 1 -%}{\"key\": \"value\"}"));
  }
  ...
  fn test_classification_block_then_expression_not_json() {
    assert!(!looks_like_json_with_tera("{%- set x = 1 -%}{{ fake_name() }}"));
  }
  ...
  fn test_classification_tera_blocks_only_not_json() {
    assert!(!looks_like_json_with_tera("{%- set x = 1 -%}{%- set y = 2 -%}"));
  }

  #[test]
  fn test_classification_array_with_tera() {
    assert!(looks_like_json_with_tera("[{% for i in items %}{{ i }}{% endfor %}]"));
  }
  ...
  fn test_nested_for_loops() {
    ...
    let lines: Vec<&str> = result.lines().collect();
    let for_lines: Vec<&&str> = lines.iter().filter(|l| l.trim().starts_with("{% for")).collect();
    assert_eq!(for_lines.len(), 2, "Should have 2 for lines. Got:\n{}", result);

    let endfor_lines: Vec<&&str> = lines.iter().filter(|l| l.trim().starts_with("{% endfor")).collect();
    assert_eq!(endfor_lines.len(), 2, "Should have 2 endfor lines. Got:\n{}", result);
  ...
  fn test_set_for_if_combined() {
    ...
    assert_eq!(set_lines.len(), 2, "Should have 2 set lines. Got:\n{}", result);
...
\n}",
  ...
  fn test_search_results_nested_objects_and_arrays() {
    ...
    assert!(result.contains("\"facets\": {"), "facets object. Got:\n{}", result);
  ...
  fn test_if_block_indents_content() {
    ...
    let if_indent = lines[if_line].chars().take_while(|c| c.is_whitespace()).count();
    let set_indent = lines[set_line].chars().take_while(|c| c.is_whitespace()).count();
    let endif_indent = lines[endif_line].chars().take_while(|c| c.is_whitespace()).count();
    ...
    assert_eq!(if_indent, endif_indent, "endif should align with if. Got:\n{}", result);
  ...
  fn test_for_loop_indents_content() {
    ...
    let for_indent = lines[for_line_idx].chars().take_while(|c| c.is_whitespace()).count();
    let id_indent = lines[id_line_idx].chars().take_while(|c| c.is_whitespace()).count();
    let endfor_indent = lines[endfor_line_idx].chars().take_while(|c| c.is_whitespace()).count();
  ...
  fn test_if_else_endif_alignment() {
    ...
    assert_eq!(if_indent, else_indent, "else should align with if. Got:\n{}", result);
    assert_eq!(if_indent, endif_indent, "endif should align with if. Got:\n{}", result);
    ...
    let set_a_lines: Vec<&str> = lines
      ...
      .copied()
      .collect();
    assert_eq!(set_a_lines.len(), 2, "Should have 2 set statements. Got:\n{}", result);

    for set_line in set_a_lines {
      let set_indent = set_line.chars().take_while(|c| c.is_whitespace()).count();
      assert!(set_indent > if_indent, "Content should be indented. Got:\n{}", result);
    }
  ...
  fn test_elif_alignment() {
    ...
    assert_eq!(if_indent, elif_indent, "elif should align with if. Got:\n{}", result);
    assert_eq!(if_indent, else_indent, "else should align with if. Got:\n{}", result);
    assert_eq!(if_indent, endif_indent, "endif should align with if. Got:\n{}", result);
  ...
  fn test_complex_nesting_for_with_if() {
    ...
    let if_indent = if_lines[0].chars().take_while(|c| c.is_whitespace()).count();
    assert!(
  ...
  fn test_user_example_inbox_pagination() {
    ...
    assert_eq!(if_blocks.len(), 2, "Should have 2 if blocks. Got:\n{}", result);
