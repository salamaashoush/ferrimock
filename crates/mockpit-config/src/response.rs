  ...
  match value {
    ...
    Some(other) => {
      ...
      let json_str = serde_json::to_string(&other)
        .map_err(|e| serde::de::Error::custom(format!("Failed to serialize body value: {e}")))?;
      Ok(Some(json_str))
    },
  }
}
...
impl ResponseConfig {
  ...
  pub fn is_full_mock(&self) -> bool {
    match self {
      ...
      ResponseConfig::Structured {
        ...
        json,
        ..
      } => body.is_some() || template.is_some() || file.is_some() || template_file.is_some() || !json.is_null(),
    }
  }
  ...
  pub fn into_resolved_response(self) -> ResolvedResponse {
    match self {
      ...
      // Always treated as static inline body (no template detection)
      ResponseConfig::StatusShortcuts(shortcuts) => {
        let (status, body_str) = shortcuts.into_iter().next().unwrap_or((200, String::new()));
      ...
      ResponseConfig::Structured {
        ...
        let body = if let Some(template_str) = template {
          ...
          BodyConfig::Template { template: template_str }
        } else if let Some(tf_path) = template_file {
          // Template loaded from file (processed by Tera)
          BodyConfig::TemplateFile { template_file: tf_path }
          ...
          if let Some(json_str) = json.as_str() {
            ...
            BodyConfig::Inline { inline: json_string }
          }
        ...
        ResolvedResponse { status, headers, body }
      },
    }
  }
}
...
impl ResolvedResponse {
  ...
  pub async fn into_response_generator_with_dir(
    ...
    self.into_response_generator_with_caching(true, config_dir).await
  }
  ...
  async fn into_response_generator_with_caching(
    ...
    let status =
      StatusCode::from_u16(self.status).map_err(|e| format!("Invalid status code {}: {}", self.status, e))?;
...
pub fn parse_duration(s: &str) -> Result<Duration, String> {
  ...
  if let Some(us) = s.strip_suffix("us") {
    let value: u64 = us.trim().parse().map_err(|_| format!("Invalid duration: {}", s))?;
    Ok(Duration::from_micros(value))
  } else if let Some(ms) = s.strip_suffix("ms") {
    let value: u64 = ms.trim().parse().map_err(|_| format!("Invalid duration: {}", s))?;
    Ok(Duration::from_millis(value))
  } else if let Some(s_val) = s.strip_suffix('s') {
    let value: u64 = s_val.trim().parse().map_err(|_| format!("Invalid duration: {}", s))?;
...
"#;
    ...
    let config: ResolvedResponse = serde_yaml::from_str(yaml).expect("Failed to parse YAML config");
...
"#;
    ...
    assert_eq!(
      generator.headers.get("x-custom").expect("x-custom header should exist"),
      "value"
    );
  }
...
"#;
    ...
    let config: ResponseConfig = serde_yaml::from_str(yaml).expect("Failed to parse YAML config");
