  ...
  fn normalize_url(&self, raw_url: &str) -> Option<String> {
    ...
    if let Ok(parsed) = Url::parse(raw_url) {
      if let Some(host) = parsed.host_str() {
        ...
        if self.options.filter_non_box_domains && !is_box_domain(host, &self.options.extra_box_domains) {
          return None;
        }
...
mod tests {
  ...
  fn test_normalize_url_preserves_query() {
    let loader = HarLoader::new();
    assert_eq!(
      loader.normalize_url("https://api.box.com/2.0/folders/0/items?fields=name,id&limit=100"),
  ...
  fn test_normalize_url_strips_access_token() {
    let loader = HarLoader::new();
    assert_eq!(
      loader.normalize_url("https://api.box.com/2.0/users/me?access_token=SECRET&fields=name"),
  ...
  async fn test_urls_normalized_to_relative() {
    let har = make_har(vec![create_test_entry("GET", "https://api.box.com/2.0/users/me", 200)]);
