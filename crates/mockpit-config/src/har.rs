    ...
    fn normalize_url(&self, raw_url: &str) -> Option<String> {
        ...
        if let Ok(parsed) = Url::parse(raw_url) {
            if let Some(host) = parsed.host_str() {
                ...
                if self.options.filter_non_allowed_domains
                    && !is_allowed_domain(host, &self.options.extra_allowed_domains)
                {
                    return None;
                }
...
mod tests {
    ...
    fn test_normalize_url_preserves_query() {
        ...
        assert_eq!(
            loader
                .normalize_url("https://api.example.com/2.0/folders/0/items?fields=name,id&limit=100"),
    ...
    fn test_normalize_url_strips_access_token() {
        ...
        assert_eq!(
            loader
                .normalize_url("https://api.example.com/2.0/users/me?access_token=SECRET&fields=name"),
    ...
    async fn test_urls_normalized_to_relative() {
        let har = make_har(vec![create_test_entry(
            "GET",
            "https://api.example.com/2.0/users/me",
            200,
        )]);
