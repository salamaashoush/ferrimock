...
use crate::{MatchConfig, MockConfig, ResponseConfig};
...
pub fn is_allowed_domain(host: &str, extra_domains: &[String]) -> bool {
  ...
  if lower.ends_with(".example.com")
    ...
    || lower == "example.net"
    || lower.ends_with(".cdn.example.com")
    || lower == "cdn.example.com"
...
pub struct HarLoadOptions {
  ...
  pub filter_non_allowed_domains: bool,
...
impl Default for HarLoadOptions {
  fn default() -> Self {
    Self {
      ...
      normalize_urls: true,
      filter_non_allowed_domains: true,
...
impl HarLoader {
  ...
  fn normalize_url(&self, raw_url: &str) -> Option<String> {
    ...
    if let Ok(parsed) = Url::parse(raw_url) {
      if let Some(host) = parsed.host_str() {
        ...
        if self.options.filter_non_allowed_domains && !is_allowed_domain(host, &self.options.extra_allowed_domains) {
          return None;
        }
  ...
  async fn convert_entry_to_mock(&self, entry: &v1_2::Entries, index: usize) -> Result<Option<MockConfig>> {
      ...
      && should_use_file_body(&body, content_type, self.options.body_size_threshold)
    {
      ...
      if let Some(ref output_dir) = self.options.body_output_dir {
        ...
        tokio::fs::write(&full_path, &body)
          .await
          .with_context(|| format!("Failed to write body file: {}", full_path.display()))?;
      }
      (None, Some(file_path))
    } else {
      (Some(body), None)
  ...
  fn should_strip_header(&self, name: &str) -> bool {
    ...
    if self.options.strip_sensitive_headers
      && matches!(
        lower.as_str(),
        "authorization" | "cookie" | "set-cookie" | "x-auth-token" | "x-csrf-token" | "proxy-authorization"
      )
    {
      return true;
    }
...
mod tests {
  ...
  #[test]
  fn test_is_allowed_domain_standard() {
    assert!(is_allowed_domain("api.example.com", &[]));
    assert!(is_allowed_domain("app.example.com", &[]));
    assert!(is_allowed_domain("upload.example.com", &[]));
    assert!(is_allowed_domain("dl.cdn.example.com", &[]));
    assert!(is_allowed_domain("cdn01.boxcdn.net", &[]));
    assert!(is_allowed_domain("realtime.services.example.net", &[]));
    assert!(is_allowed_domain("example.com", &[]));
  }

  #[test]
  fn test_is_allowed_domain_enterprise() {
    assert!(is_allowed_domain("myorg.app.example.com", &[]));
    assert!(is_allowed_domain("myorg.ent.example.com", &[]));
    assert!(is_allowed_domain("fupload-us1.app.example.com", &[]));
  }

  #[test]
  fn test_is_allowed_domain_regional() {
    assert!(is_allowed_domain("us-east-1.cdn.example.com", &[]));
  }

  #[test]
  fn test_is_allowed_domain_negative() {
    assert!(!is_allowed_domain("google.com", &[]));
    assert!(!is_allowed_domain("api.github.com", &[]));
    assert!(!is_allowed_domain("cdn.jsdelivr.net", &[]));
    assert!(!is_allowed_domain("analytics.google.com", &[]));
  }

  #[test]
  fn test_is_allowed_domain_extra() {
    let extra = vec!["internal.mycompany.com".to_string()];
    assert!(is_allowed_domain("internal.mycompany.com", &extra));
    assert!(is_allowed_domain("api.internal.mycompany.com", &extra));
    assert!(!is_allowed_domain("google.com", &extra));
  }
  ...
  fn test_is_not_static_asset() {
    assert!(!is_static_asset("https://api.example.com/2.0/users/me"));
    assert!(!is_static_asset("/2.0/files/123"));
    assert!(!is_static_asset("https://api.example.com/2.0/folders/0/items"));
  }
  ...
  fn test_normalize_url_absolute_box() {
    ...
    assert_eq!(
      loader.normalize_url("https://api.example.com/2.0/users/me"),
  ...
  fn test_normalize_url_preserves_query() {
    let loader = HarLoader::new();
    assert_eq!(
      loader.normalize_url("https://api.example.com/2.0/folders/0/items?fields=name,id&limit=100"),
  ...
  fn test_normalize_url_strips_access_token() {
    let loader = HarLoader::new();
    assert_eq!(
      loader.normalize_url("https://api.example.com/2.0/users/me?access_token=SECRET&fields=name"),
  ...
  fn test_normalize_url_filters_non_box_domain() {
    let loader = HarLoader::new();
    assert_eq!(loader.normalize_url("https://www.google.com/analytics"), None);
  }

  #[test]
  fn test_normalize_url_already_relative() {
    let loader = HarLoader::new();
    assert_eq!(loader.normalize_url("/2.0/users/me"), Some("/2.0/users/me".to_string()));
  }
  ...
  fn test_normalize_url_enterprise_domain() {
    let loader = HarLoader::new();
    assert_eq!(
      loader.normalize_url("https://myorg.app.example.com/api/oauth2/token"),
  ...
  fn test_normalize_url_disabled() {
    let loader = HarLoader::with_options(HarLoadOptions {
      ...
      filter_non_allowed_domains: false,
      ..Default::default()
    });
    // Should keep the absolute URL but still strip sensitive params
    let result = loader.normalize_url("https://api.example.com/2.0/users/me?access_token=SECRET");
    assert_eq!(result, Some("https://api.example.com/2.0/users/me".to_string()));
  }
  ...
  fn test_keep_api_calls() {
    let loader = HarLoader::new();
    let entry = create_test_entry("GET", "https://api.example.com/2.0/users/me", 200);
    assert!(!loader.should_skip_entry(&entry));
  }
  ...
  #[tokio::test]
  async fn test_filter_non_allowed_domains() {
    let har = make_har(vec![
      create_test_entry("GET", "https://api.example.com/2.0/users/me", 200),
      create_test_entry("GET", "https://www.google.com/analytics", 200),
      create_test_entry("POST", "https://upload.example.com/api/2.0/files/content", 201),
      create_test_entry("GET", "https://cdn.jsdelivr.net/npm/react", 200),
    ]);

    let loader = HarLoader::new();
    let mocks = loader.convert_har_to_mocks(har).await.expect("conversion failed");
    ...
    let urls: Vec<&str> = mocks
      ...
      .map(|s| s.as_str())
      .collect();
    assert!(urls.iter().all(|u| u.contains("/2.0/") || u.contains("/api/")));
  }
  ...
  async fn test_urls_normalized_to_relative() {
    let har = make_har(vec![create_test_entry("GET", "https://api.example.com/2.0/users/me", 200)]);
  ...
  async fn test_absolute_urls_preserved_when_disabled() {
    ...
    assert_eq!(mocks.len(), 1);
    let mc = mocks[0].match_config.as_ref().unwrap();
    assert_eq!(mc.urls[0], "exact:https://api.example.com/2.0/users/me");
  }
  ...
  async fn test_body_extraction_large() {
    ...
    let mut entry = create_test_entry("GET", "https://api.example.com/2.0/files/123", 200);
    ...
    let file_content = tokio::fs::read_to_string(temp_dir.path().join("bodies/har-entry-1.body"))
      .await
      .unwrap();
  ...
  async fn test_end_to_end_clean_conversion() {
    let har = make_har(vec![
      // Filestore API call - should be kept with relative URL
      create_test_entry_with_headers(
        "GET",
        "https://api.example.com/2.0/users/me?access_token=SECRET_TOKEN&fields=name",
        200,
        vec![
      ...
      create_test_entry("OPTIONS", "https://api.example.com/2.0/files", 204),
    ]);
  ...
  async fn test_load_har_file() {
        ...
        "entries": [
          {
            ...
            "request": {
              "method": "GET",
              "url": "https://api.example.com/2.0/users/me",
    ...
    let loader = HarLoader::new();
    let mocks = loader.load_from_file(&har_path).await.expect("Failed to load HAR file");

    assert_eq!(mocks.len(), 1);
    let match_config = mocks[0].match_config.as_ref().expect("match_config should exist");
    let response_config = mocks[0].response_config.as_ref().expect("response_config should exist");
  ...
  #[tokio::test]
  async fn test_extra_allowed_domains() {
    let har = make_har(vec![
      create_test_entry("GET", "https://api.example.com/2.0/users/me", 200),
    ...
    let loader = HarLoader::with_options(HarLoadOptions {
      extra_allowed_domains: vec!["internal.mycompany.com".to_string()],
      ..Default::default()
    });
    let mocks = loader.convert_har_to_mocks(har).await.expect("conversion failed");
