...
use super::response::{ResponseConfig, ResponsePatchesConfig, parse_duration, parse_patches_config};
use anyhow::Result;
use mockpit_types::MockDefinition;
...
impl MockCollectionConfig {
  ...
  pub async fn from_har(content: &str) -> Result<Self, anyhow::Error> {
    ...
    Ok(Self {
      ...
      description: Some("Auto-converted from HAR file - all entries loaded as static mocks".to_string()),
      enabled: true,
      vars: None,
      mocks,
    })
...
impl MockConfig {
  ...
  pub async fn into_mock_definition_with_dir(
    ...
    if is_full_mock && self.patch.is_some() {
      return Err(
        "Cannot combine top-level `patch` with full mock response. \
    ...
    let mut response = resolved_response.into_response_generator_with_dir(config_dir).await?;

    // Apply top-level delay
    if let Some(ref delay_str) = self.delay {
      let delay = parse_duration(delay_str).map_err(|e| format!("Invalid top-level delay: {}", e))?;
    ...
    if let Some(patches_config) = self.patch {
      ...
      if !patch_ops.is_empty() {
        response = response.with_mode(mockpit_types::ResponseMode::Patch { operations: patch_ops });
      }
    }
    ...
    let entered_patch_mode = matches!(response.mode, mockpit_types::ResponseMode::Patch { .. });
    if self.delay.is_some() && !is_full_mock && !entered_patch_mode && !has_request_transforms {
      response = response.with_mode(mockpit_types::ResponseMode::Patch { operations: vec![] });
    }
...
fn build_request_transforms(
...
) -> Result<mockpit_types::ResolvedRequestTransforms, String> {
  use mockpit_types::{RequestPatch, ResolvedRequestTransforms, UpstreamOptions};
  ...
  if !config.body.operations.is_empty() {
    ...
    let json_patch: json_patch::Patch =
      serde_json::from_str(&json_patch_str).map_err(|e| format!("Failed to parse JSON Patch operations: {}", e))?;
...
"#;
    ...
    assert_eq!(mock_def.response.delay, Some(std::time::Duration::from_millis(100)));
  }
...
"#;
    ...
    let config = MockCollectionConfig::from_yaml(yaml).expect("Failed to parse YAML");
    let defs = config.into_mock_definitions().await.expect("Failed to convert");
...
"#;
    ...
    assert_eq!(vars.get("from_collection").unwrap(), &serde_json::json!(true));
