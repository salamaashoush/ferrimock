...
pub fn parse_url_pattern(pattern: &str) -> Result<UrlPattern, String> {
  ...
  if pattern.contains("/:") || pattern.contains("/{") {
    ...
    return UrlPattern::regex(&regex_pattern)
      .map_err(|e| format!("Failed to convert Express-style pattern '{}' to regex: {}", pattern, e));
  }

  // Check for glob patterns (**/ or *)
  if pattern.contains("**") || (pattern.contains('*') && !pattern.starts_with('^')) {
    return UrlPattern::glob(pattern).map_err(|e| format!("Invalid glob pattern '{}': {}", pattern, e));
  }
  ...
  if pattern.starts_with('^')
    ...
    || pattern.contains('|')
  {
    return UrlPattern::regex(pattern).map_err(|e| format!("Invalid regex pattern '{}': {}", pattern, e));
  }
      ...
      '{' => {
        ...
        while let Some(&next_ch) = chars.peek() {
          if next_ch == '}' {
            ...
            param_name.push(chars.next().expect("Character should exist after peek confirmed it"));
          } else {
            break;
          }
        }
...
mod tests {
  ...
  fn test_parse_url_pattern_express_style() {
    ...
    if let UrlPattern::Regex(regex) = pattern {
      ...
      assert_eq!(caps.name("id").expect("id capture should exist").as_str(), "456");
    }
  }

  #[test]
  fn test_parse_url_pattern_express_multiple_params() {
    let pattern = parse_url_pattern("/api/users/:user_id/files/:file_id").expect("Failed to parse URL pattern");
    assert!(matches!(pattern, UrlPattern::Regex(_)));
    ...
    if let UrlPattern::Regex(regex) = pattern {
      ...
      let caps = regex
        ...
        .expect("Failed to capture regex groups");
      assert_eq!(
        caps.name("user_id").expect("user_id capture should exist").as_str(),
        "abc"
      );
      assert_eq!(
        caps.name("file_id").expect("file_id capture should exist").as_str(),
        "xyz"
      );
    }
  }
  ...
  fn test_parse_url_pattern_escaped_dot() {
    ...
    let pattern = parse_url_pattern("/api/images/avatar\\.png").expect("Failed to parse URL pattern");
    assert!(matches!(pattern, UrlPattern::Regex(_)));
  ...
  fn test_parse_url_pattern_php_array_brackets_in_query() {
    ...
    let pattern =
      parse_url_pattern("/index.php?fileIds[]=10000000001&rm=preview_get_files_metadata").expect("Failed to parse");
    assert!(
  ...
  fn test_parse_url_pattern_nested_brackets_in_query() {
    ...
    let pattern = parse_url_pattern("/api/items?filter[status]=active&filter[type]=doc").expect("Failed to parse");
    assert!(
      matches!(pattern, UrlPattern::Exact(_)),
  ...
  fn test_parse_url_pattern_multi_value_brackets_in_query() {
    ...
    let pattern = parse_url_pattern("/api/users?fields[]=name&fields[]=id&fields[]=email").expect("Failed to parse");
    assert!(
      matches!(pattern, UrlPattern::Exact(_)),
  ...
  fn test_parse_url_pattern_parentheses_in_query() {
    ...
    let pattern = parse_url_pattern("/api/search?$filter=(name eq 'test')&$top=10").expect("Failed to parse");
    assert!(
      matches!(pattern, UrlPattern::Exact(_)),
  ...
  fn test_parse_url_pattern_complex_recorded_url_with_query() {
    ...
    let urls = [
      "/api/attributes:bulk?fileIds=10000000001&template=review",
      "/index.php?rm=gen204_batch_record",
      "/api/onboarding/experiences?context=all_files",
      "/api/v2/files/123?fields=name,size,modified_at",
    ];

    for url in &urls {
      let pattern = parse_url_pattern(url).unwrap_or_else(|e| panic!("Failed to parse '{}': {}", url, e));
      assert!(
        matches!(pattern, UrlPattern::Exact(_)),
