...
use ferrimock::services::create::{CreateInput, MockKind, create, generate_template_body};
...
fn run_streaming_wizard(
    ...
    crate::say!(
        "{}",
        ui::dim("Tip: edit the scaffolded rules/events, then `ferrimock mock serve` to try it")
    );
    Ok(())
}
...
fn step_review_and_save(state: &WizardState) -> anyhow::Result<()> {
    ...
    println!(
        "{}",
        ui::dim(&format!(
            "Tip: Test with: ferrimock mock test -m {} {}",
