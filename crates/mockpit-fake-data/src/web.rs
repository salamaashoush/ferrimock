...
pub fn fake_file_extension() -> String {
  let exts = [
    "pdf", "doc", "docx", "xls", "xlsx", "ppt", "pptx", "txt", "jpg", "png", "gif", "mp4", "mp3", "zip",
  ];
  exts.choose(&mut rand::rng()).copied().unwrap_or("pdf").to_string()
}
...
pub fn fake_status_message() -> String {
  ...
  messages.choose(&mut rand::rng()).copied().unwrap_or("OK").to_string()
}
...
pub fn fake_locale() -> String {
  let locales = [
    "en-US", "en-GB", "en-CA", "en-AU", "en-NZ", "en-IE", "fr-FR", "fr-CA", "fr-BE", "fr-CH", "de-DE", "de-AT",
    "de-CH", "es-ES", "es-MX", "es-AR", "es-CL", "es-CO", "it-IT", "pt-BR", "pt-PT", "ja-JP", "zh-CN", "zh-TW",
    "zh-HK", "ko-KR", "ru-RU", "nl-NL", "nl-BE", "pl-PL", "tr-TR", "ar-SA", "ar-EG", "hi-IN", "th-TH", "vi-VN",
    "id-ID", "sv-SE", "no-NO", "da-DK", "fi-FI",
  ];
  locales.choose(&mut rand::rng()).copied().unwrap_or("en-US").to_string()
}
