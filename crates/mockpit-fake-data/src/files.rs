...
pub fn fake_pdf(text: Option<&str>, pages: Option<u32>) -> String {
  ...
  let content_text = text.unwrap_or("This is a fake PDF document generated for testing purposes.");

  // Add footer with generation timestamp
  let footer = format!("Generated: {}", chrono::Utc::now().format("%Y-%m-%d %H:%M:%S UTC"));
  ...
  for page_num in 1..=page_count {
    ...
    for (i, line) in lines.iter().enumerate() {
      ...
      operations.push(Operation::new("Tj", vec![Object::string_literal((*line).to_string())]));
    }
    ...
    operations.push(Operation::new(
      "Tj",
      vec![Object::string_literal(format!("Page {page_num} of {page_count}"))],
    ));
    ...
    operations.push(Operation::new("Tj", vec![Object::string_literal(footer.clone())]));
...
/// # Panics
/// Panics if image encoding fails or if dimensions are invalid
pub fn fake_jpeg(width: Option<u32>, height: Option<u32>, color: Option<&str>, quality: Option<u8>) -> String {
  use image::{ImageBuffer, Rgb};
...
pub fn fake_jpeg_data_uri(width: Option<u32>, height: Option<u32>, color: Option<&str>, quality: Option<u8>) -> String {
...
pub fn png_to_jpeg(png_base64: &str, quality: Option<u8>) -> String {
  ...
  // Decode base64 PNG
  let Ok(png_bytes) = base64::Engine::decode(&base64::engine::general_purpose::STANDARD, png_base64) else {
...
pub fn fake_image_with_text(
  ...
  draw_text_mut(&mut img, text_color, dim_x, dim_y, dim_scale, &font, &dim_text);
...
pub fn fake_image_stripes(
  ...
  let img = ImageBuffer::from_fn(width, height, |x, y| {
    ...
    if (pos / stripe_width) % 2 == 0 { color1 } else { color2 }
  });
...
pub fn fake_placeholder(
  ...
  let width = width.unwrap_or(400);
  let height = height.unwrap_or(300);
  let text = text.map_or_else(|| format!("{width}x{height}"), std::string::ToString::to_string);

  fake_image_with_text(Some(&text), Some(width), Some(height), bg_color, text_color, Some(32.0))
}
  ...
  match color {
    Some(c) => {
      if let Some(hex) = c.strip_prefix('#') {
        if hex.len() == 6 {
          let r = u8::from_str_radix(hex.get(0..2).unwrap_or_default(), 16).unwrap_or(128);
          let g = u8::from_str_radix(hex.get(2..4).unwrap_or_default(), 16).unwrap_or(128);
          let b = u8::from_str_radix(hex.get(4..6).unwrap_or_default(), 16).unwrap_or(128);
          return [r, g, b];
        } else if hex.len() == 3 {
          let r = u8::from_str_radix(&hex.get(0..1).unwrap_or_default().repeat(2), 16).unwrap_or(128);
          let g = u8::from_str_radix(&hex.get(1..2).unwrap_or_default().repeat(2), 16).unwrap_or(128);
          let b = u8::from_str_radix(&hex.get(2..3).unwrap_or_default().repeat(2), 16).unwrap_or(128);
          return [r, g, b];
        }
      }
      random_color_rgb()
...
mod tests {
  ...
  fn test_embedded_font_loads() {
    ...
    let font_data =
      EmbeddedAssets::get("NotoSans-Regular.ttf").expect("NotoSans-Regular.ttf not found in embedded assets");
  ...
  fn test_fake_image_with_text() {
    ...
    let bytes =
      base64::Engine::decode(&base64::engine::general_purpose::STANDARD, &result).expect("Should be valid base64");
    assert_eq!(
      &bytes[0..8],
  ...
  fn test_fake_avatar() {
    ...
    assert!(!result.is_empty(), "fake_avatar should return non-empty base64");
  ...
  fn test_fake_placeholder() {
    ...
    assert!(!result.is_empty(), "fake_placeholder should return non-empty base64");
