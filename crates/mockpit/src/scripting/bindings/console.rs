//! `console.*` — forwarded to `tracing` under the `mockpit::script`
...
macro_rules! console_fn {
    ($name:ident, $level:ident, $level_const:ident) => {
        fn $name<'js>(ctx: Ctx<'js>, args: Rest<Value<'js>>) {
            ...
            if tracing::enabled!(target: "mockpit::script", tracing::Level::$level_const) {
                tracing::$level!(target: "mockpit::script", "{}", render_args(&ctx, &args.0));
            }
        }
    };
}
