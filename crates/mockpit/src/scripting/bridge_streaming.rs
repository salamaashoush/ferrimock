...
fn poisoned_error() -> MockpitError {
    MockpitError::Script(
...
async fn dispatch_sse_event(
    ...
    vm_with!(vm => |ctx| {
        ...
        if let Some(frame) = &frame {
            ...
            if let Err(e) = result {
                return Err(MockpitError::Script(e.to_string()));
            }
        }
        for listener in listeners {
            let func = match listener.restore(&ctx) {
                Ok(f) => f,
                Err(e) => return Err(MockpitError::Script(format!("restore listener: {e}"))),
            };
    ...
    Arc::new(
        move |request: RequestContext, tx: mpsc::UnboundedSender<SseSinkMsg>| {
            ...
            Box::pin(async move {
                ...
                let init: Result<u64, MockpitError> = vm_with!(vm => |ctx| {
                    let conn_id =
                    ...
                    let Some(info_obj) = info.as_object() else {
                        return Err(MockpitError::Script("resolver info is not an object".into()));
                    };
                ...
                let result: Result<(), MockpitError> = loop {
                    tokio::select! {
                        () = tx.closed() => break Ok(()),
