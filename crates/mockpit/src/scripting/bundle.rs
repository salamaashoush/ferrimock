...
//! `mockpit` native module, kept external so the bytecode re-links by
...
use crate::{MockpitError, Result};

use super::bridge::caught_to_error;
use super::loader::MOCKPIT_MODULE;
...
#[derive(Debug)]
struct MockpitRuntimePlugin;

impl Plugin for MockpitRuntimePlugin {
    fn name(&self) -> Cow<'static, str> {
        "mockpit-runtime".into()
    }
    ...
    async fn resolve_id(
        ...
        if args.specifier == MOCKPIT_MODULE {
...
pub async fn bundle_source(entry: &Path, cwd: &Path) -> Result<(String, Option<String>)> {
    ...
    let mut bundler = Bundler::with_plugins(options, vec![Arc::new(MockpitRuntimePlugin)])
        .map_err(|e| MockpitError::Script(format!("rolldown init: {e:?}")))?;
    // rolldown's generate future is large; box it so it doesn't bloat
    // the enclosing future.
    let out = Box::pin(bundler.generate())
        .await
        .map_err(|e| MockpitError::Script(format!("{}: bundle: {e:?}", entry.display())))?;
...
pub async fn bundle_and_compile(entry: &Path, cwd: &Path) -> Result<CompiledBundle> {
    ...
    // bundle keeps the `mockpit` specifier external — so even this
    ...
    let runtime =
        AsyncRuntime::new().map_err(|e| MockpitError::Script(format!("bytecode runtime: {e}")))?;
    runtime
        .set_loader(
    ...
    let ctx = AsyncContext::full(&runtime)
        .await
        .map_err(|e| MockpitError::Script(format!("bytecode context: {e}")))?;
    ...
    let bytecode: Vec<u8> = ctx
        .async_with(async |ctx| {
            ...
            module
                ...
                .map_err(|e| MockpitError::Script(format!("module write: {e}")))
        })
        .await?;
...
pub fn remap_error(err: MockpitError, bundle: &CompiledBundle) -> MockpitError {
    let MockpitError::Script(message) = err else {
        return err;
    };
