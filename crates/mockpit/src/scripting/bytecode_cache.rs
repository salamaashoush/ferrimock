...
//! compile on the next `mockpit mock serve` start.
//!
//! ## Soundness
//!
...
//! version (proxy for the pinned rolldown/oxc bundler and the `mockpit`
...
fn disabled() -> bool {
    std::env::var_os("MOCKPIT_NO_BYTECODE_CACHE").is_some()
}
...
/// `<cache>/mockpit/bytecode/<abi_tag>/`, created on demand. Honors
/// `MOCKPIT_CACHE_DIR`, else the platform user cache dir, else the
/// system temp dir. Returns `None` if no writable base exists.
fn cache_dir() -> Option<&'static Path> {
    static DIR: OnceLock<Option<PathBuf>> = OnceLock::new();
    DIR.get_or_init(|| {
        let base = std::env::var_os("MOCKPIT_CACHE_DIR")
            .map(PathBuf::from)
            .or_else(user_cache_base)
            .unwrap_or_else(std::env::temp_dir);
        let dir = base.join("mockpit").join("bytecode").join(abi_tag());
