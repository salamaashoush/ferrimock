...
use mockpit::core::PersistenceStore;
use mockpit::template::{TemplateError, render_template, validate_template};
...
fn test_mock_avatar_url_with_params() {
    let ctx = RequestContext::new();
    let result =
        render_template(r#"{{ mock_avatar_url(initials="SA", size=128) }}"#, &ctx).unwrap();
