...
use mockpit::config::{MatchConfig, MockCollectionConfig, MockConfig, ReturnConfig};
use mockpit::engine::validation::{ErrorType, MockValidator, WarningType};
...
";
    ...
    assert!(
        result
            .errors
            .iter()
            .any(|e| matches!(e.error_type, ErrorType::MissingField)
...
",
    ...
    assert!(
        results
            .iter()
            .any(mockpit::engine::ValidationResult::has_errors)
    );
}
...
async fn test_validate_directory_nonexistent() {
    ...
    let results = validator
        .validate_directory(std::path::Path::new("/nonexistent"))
        .await;

    assert_eq!(results.len(), 1);
    assert!(results[0].has_errors());
    assert!(matches!(
        results[0].errors[0].error_type,
...
async fn test_validate_config_directly() {
    use mockpit::config::{MatchConfig, MockConfig, ReturnConfig};
...
async fn test_validation_result_formatting() {
    use mockpit::engine::validation::{
        CodeSnippet, ValidationError, ValidationResult, ValidationWarning,
    };
...
"#;
    ...
    assert!(
        result.is_ok(),
        "Should succeed with valid request transforms"
    );
}
...
"#;
    ...
    assert!(
        result.has_errors(),
        "Should have errors for invalid duration"
    );
    assert!(
        result
            .errors
...
";
    ...
    assert!(
        result.has_errors(),
        "Should have errors for invalid forward_to URL"
    );
    assert!(
        result
            .errors
...
"#;
    ...
    assert!(
        result.has_errors(),
        "Should have errors for invalid body regex"
    );
    assert!(
        result
            .errors
...
"#;
    ...
    assert!(
        result.has_errors(),
        "Should have errors for conflicting modes"
    );
    assert!(
        result
            .errors
...
";
    ...
    assert!(
        result.has_warnings(),
        "Should have warnings for empty request section"
    );
    assert!(
        result
            .warnings
...
"#;
    ...
    assert!(
        result.has_errors(),
        "Should have errors for invalid patch header name"
    );
    assert!(
        result
            .errors
