...
impl RequestContext {
  ...
  pub fn from_request(method: &str, uri: &str, query: Option<&str>, headers: &HeaderMap, body: Option<&[u8]>) -> Self {
    // Parse query string
  ...
  pub fn extract_captures_from_path(pattern: &regex::Regex, path: &str) -> FxHashMap<String, String> {
...
impl QueryMatcher {
  ...
  pub fn matches_parsed(&self, query_params: &FxHashMap<String, String>) -> bool {
    match &self.pattern {
      QueryMatchPattern::Exact(expected) => query_params.get(&self.name).map(|v| v == expected).unwrap_or(false),
      QueryMatchPattern::Regex(re) => query_params.get(&self.name).map(|v| re.is_match(v)).unwrap_or(false),
  ...
  pub fn from_json(json: serde_json::Value) -> Result<Self, anyhow::Error> {
    if let Some(obj) = json.as_object() {
      ...
      if has_status || has_headers || has_body {
        ...
        let headers = obj.get("headers").and_then(|v| v.as_object()).map(|header_obj| {
          header_obj
            .iter()
        ...
        let body_value = if has_body { obj.get("body").unwrap() } else { &json };
...
pub enum BodySource {
  ...
  #[serde(serialize_with = "serialize_inline", deserialize_with = "deserialize_inline")]
  ...
  pub fn may_produce_structured_response(&self) -> bool {
    match self {
      BodySource::Template { source, .. } => {
        source.contains("\"status\"") || source.contains("\"headers\"") || source.contains("\"body\"")
      },
      _ => false,
    }
  }
...
mod tests {
  ...
  fn test_header_matcher_exact() {
    let matcher = HeaderMatcher::exact(HeaderName::from_static("content-type"), "application/json");
  ...
  fn test_header_matcher_regex() {
    let matcher = HeaderMatcher::regex(HeaderName::from_static("content-type"), r"^application/(json|xml)$").unwrap();
  ...
  fn test_response_generator_with_header() {
    let response =
      ResponseGenerator::new(StatusCode::OK, BodySource::inline("{}")).with_header("content-type", "application/json");
  ...
  fn test_response_generator_with_delay() {
    let response =
      ResponseGenerator::new(StatusCode::OK, BodySource::inline("{}")).with_delay(Duration::from_millis(100));
  ...
  fn test_response_generator_build_headers() {
    ...
    assert_eq!(headers.get("x-custom").unwrap(), HeaderValue::from_static("value"));
  }
  ...
  fn test_jsonpath_deep_nesting() {
    ...
    let matcher = BodyMatcher::json_path("$.response.users[0].addresses[0].city", serde_json::json!("New York"));
    assert!(matcher.matches(serde_json::to_string(&json).unwrap().as_bytes()));

    let matcher = BodyMatcher::json_path("$.response.users[0].addresses[1].zip", serde_json::json!("02101"));
    assert!(matcher.matches(serde_json::to_string(&json).unwrap().as_bytes()));
  }
