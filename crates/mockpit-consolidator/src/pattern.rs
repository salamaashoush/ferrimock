...
impl PatternDetector {
  ...
  fn extract_graphql_grouping_key(graphql_config: Option<&mockpit_config::GraphQLMatchConfig>) -> String {
  ...
  pub fn normalize_path_for_grouping(path: &str) -> String {
    ...
    normalized = ISO_DATE_PATTERN.replace_all(&normalized, "/{date}$2").to_string();
  ...
  pub fn generate_smart_url_pattern(group: &[MockConfig]) -> String {
    ...
    if Self::all_urls_identical(group) {
      ...
      first_url.strip_prefix("exact:").unwrap_or(first_url).to_string()
    } else {
    ...
    for mock in group {
      if let Some(ref match_config) = mock.match_config {
        ...
        for url_pattern in url_patterns {
          ...
          let parsed = Url::parse(url).or_else(|_| Url::parse(&format!("http://dummy{}", url)));
          if let Ok(parsed) = parsed {
            base_paths.insert(parsed.path().to_string());

            for (key, value) in parsed.query_pairs() {
              all_params.entry(key.to_string()).or_default().insert(value.to_string());
            }
          }
        }
      }
  ...
  /// Handles both absolute URLs (`https://api.example.com/2.0/users/me`)
  ...
  pub fn are_duplicates(group: &[MockConfig]) -> bool {
    ...
    let first_body = group[0].response_config.as_ref().and_then(|rc| rc.body().cloned());
    ...
    group.iter().skip(1).all(|mock| {
      ...
      let mock_body = mock.response_config.as_ref().and_then(|rc| rc.body().cloned());
...
mod tests {
  ...
  fn test_normalize_path_uuid() {
    ...
    assert_eq!(PatternDetector::normalize_path_for_grouping(path), "/files/{uuid}");
  }
  ...
  fn test_graphql_grouping_simple_syntax() {
    let mock = create_graphql_mock("get-user-simple", GraphQLMatchConfig::Simple("GetUser".to_string()));
  ...
  fn test_graphql_vs_rest_grouping() {
    let graphql_mock = create_graphql_mock("graphql-user", GraphQLMatchConfig::Simple("GetUser".to_string()));
  ...
  fn test_graphql_grouping_same_operation_grouped_together() {
    ...
    let mock1 = create_graphql_mock(
      ...
      GraphQLMatchConfig::Structured {
        ...
        variables: {
          ...
          vars.insert("id".to_string(), serde_json::Value::String("123".to_string()));
          vars
        },
      },
    );
    ...
    let mock2 = create_graphql_mock(
      ...
      GraphQLMatchConfig::Structured {
        ...
        variables: {
          let mut vars = FxHashMap::default();
          vars.insert("id".to_string(), serde_json::Value::String("456".to_string()));
          vars
        },
      },
    );
  ...
  fn test_extract_base_path_absolute_url() {
    let mock = create_rest_mock("abs-1", "GET", "exact:https://api.example.com/2.0/users/me");
  ...
  fn test_extract_base_path_absolute_url_with_query() {
    let mock = create_rest_mock(
      "abs-2",
      "GET",
      "exact:https://api.example.com/2.0/folders/0/items?fields=name&limit=100",
    );
    assert_eq!(PatternDetector::extract_base_path(&mock), "/2.0/folders/0/items");
  }
  ...
  fn test_grouping_mixed_absolute_and_relative() {
    ...
    let mock_abs = create_rest_mock("abs", "GET", "exact:https://api.example.com/2.0/users/123");
  ...
  fn test_analyze_query_params_absolute_urls() {
    let mocks = vec![
      create_rest_mock(
        "q1",
        "GET",
        "exact:https://api.example.com/2.0/folders/0/items?fields=name&offset=0",
      ),
      create_rest_mock(
        "q2",
        "GET",
        "exact:https://api.example.com/2.0/folders/0/items?fields=name&offset=100",
      ),
    ];
