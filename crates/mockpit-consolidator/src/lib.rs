...
use mockpit_codegen::TemplateGenerator;
use mockpit_config::{MockCollectionConfig, MockConfig, ReturnConfig};
...
impl MockConsolidator {
  ...
  pub fn with_options(options: ConsolidatorOptions) -> Self {
    ...
    let template_generator = TemplateGenerator::new(options.pagination_storage_key_template.clone());
  ...
  pub async fn consolidate_file(&mut self, input_path: impl AsRef<Path>) -> Result<MockCollectionConfig> {
  ...
  pub async fn consolidate(&mut self, collection: MockCollectionConfig) -> Result<MockCollectionConfig> {
    self.stats.original_count = collection.mocks.len();

    println!("Analyzing {} mocks for consolidation...", self.stats.original_count);
    ...
    self.stats.reduction_ratio = 1.0 - (self.stats.consolidated_count as f64 / self.stats.original_count.max(1) as f64);
  ...
  async fn process_mock_group(&mut self, group_id: usize, group: &[MockConfig]) -> Result<Vec<MockConfig>> {
    if group.len() == 1 {
    ...
    if response_analysis.varying_fields.is_empty() {
      ...
      self
        .create_smart_template_mock(group, &url_pattern, &response_analysis, &graphql_analysis)
        .await
    } else {
      println!(
  ...
  async fn create_smart_template_mock(
    ...
    let response_structure: mockpit_codegen::ResponseStructure = analysis.into();
    let graphql_info: mockpit_codegen::GraphQLVariableInfo = graphql_analysis.into();

    let template_body = self
      .template_generator
      .generate_tera_template(&response_structure, &base_path, &graphql_info);
    ...
    if let Err(e) = mockpit_template::validate_template(&template_body) {
      ...
      println!("      ↳ Falling back to keeping mocks separate due to template validation error");
      return Ok(group.to_vec());
    }
