...
use crate::pattern::QueryParamAnalysis;
use anyhow::Result;
use mockpit_config::MockConfig;
use mockpit_type_detector::{FieldType, TypeDetector};
use regex::Regex;
...
impl From<&PaginationPattern> for mockpit_codegen::PaginationInfo {
  fn from(pattern: &PaginationPattern) -> Self {
    mockpit_codegen::PaginationInfo {
      ...
      pagination_type: match pattern.pagination_type {
        PaginationType::Offset => mockpit_codegen::PaginationType::Offset,
        PaginationType::Cursor => mockpit_codegen::PaginationType::Cursor,
        PaginationType::Page => mockpit_codegen::PaginationType::Page,
      },
...
impl From<&ResponseAnalysis> for mockpit_codegen::ResponseStructure {
  fn from(analysis: &ResponseAnalysis) -> Self {
    mockpit_codegen::ResponseStructure {
...
impl From<&GraphQLVariableAnalysis> for mockpit_codegen::GraphQLVariableInfo {
  fn from(analysis: &GraphQLVariableAnalysis) -> Self {
    mockpit_codegen::GraphQLVariableInfo {
...
impl ResponseAnalyzer {
  ...
  fn analyze_top_level_array_responses(&self, responses: &[JsonValue]) -> Result<ResponseAnalysis> {
  ...
  pub fn analyze_object_fields(&self, objects: &[&JsonValue]) -> (Vec<(String, FieldType)>, Vec<(String, JsonValue)>) {
    ...
    for (mock, response) in group.iter().zip(responses.iter()) {
      ...
      if let Some(url_pattern) = url_pattern {
        ...
        if let Some(caps) = path_id_regex.captures(url) {
          if let Some(path_id) = caps.get(1).and_then(|m: regex::Match| m.as_str().parse::<i64>().ok()) {
  ...
  pub fn detect_pagination_pattern(&self, responses: &[JsonValue], group: &[MockConfig]) -> Option<PaginationPattern> {
    if responses.is_empty() || !self.enable_stateful_pagination {
      return None;
    }

    let objects: Vec<&serde_json::Map<String, JsonValue>> = responses.iter().filter_map(|r| r.as_object()).collect();
    ...
    let total_field = Self::find_field_fuzzy(objects[0], &total_fields, &["total", "count", "num"]);
    let offset_field = Self::find_field_fuzzy(objects[0], &offset_fields, &["offset", "skip", "start"]);
    let limit_field = Self::find_field_fuzzy(objects[0], &limit_fields, &["limit", "page", "size"]);
      ...
      if let Some(ref next_f) = next_field {
        if let Some(val) = objects[0].get(next_f) {
          if let Some(next_str) = val.as_str() {
            ...
            if next_str.starts_with("http://") || next_str.starts_with("https://") || next_str.contains("?page=") {
              PaginationType::Page
            } else {
    ...
    let query_analysis = crate::pattern::PatternDetector::analyze_query_param_variations(group);
    let has_pagination_query_params = Self::has_pagination_params(&query_analysis);

    if has_pagination_query_params || (total_field.is_some() && (offset_field.is_some() || next_field.is_some())) {
      // Extract static query params from next/previous URLs
      let static_query_params = Self::extract_static_query_params(&objects, &next_field, &prev_field, &query_analysis);
  ...
  fn has_pagination_params(query_analysis: &QueryParamAnalysis) -> bool {
    query_analysis
      .varying_params
      .iter()
      .any(|p| p == "offset" || p == "limit" || p == "page" || p == "per_page" || p == "marker" || p == "cursor")
  }
  ...
  fn extract_static_query_params(
    ...
    let sample_url = next_field
      .as_ref()
      .and_then(|field| objects.iter().find_map(|obj| obj.get(field).and_then(|v| v.as_str())))
      .or_else(|| {
        prev_field
          .as_ref()
          .and_then(|field| objects.iter().find_map(|obj| obj.get(field).and_then(|v| v.as_str())))
  ...
  fn extract_graphql_variables(graphql: &mockpit_config::matcher::GraphQLMatchConfig) -> FxHashMap<String, JsonValue> {
    use mockpit_config::matcher::GraphQLMatchConfig;
...
mod tests {
    ...
    let mocks = vec![
      MockConfig {
        ...
        response_config: Some(ReturnConfig::Structured {
          ...
          body: Some(r#"{"total_count": 100, "offset": 0, "limit": 10, "items": []}"#.to_string()),
          template: None,
          file: None,
          template_file: None,
      ...
      MockConfig {
        ...
        response_config: Some(ReturnConfig::Structured {
          status: Some(200),
          headers: FxHashMap::default(),
          body: Some(r#"{"total_count": 100, "offset": 10, "limit": 10, "items": []}"#.to_string()),
          template: None,
          file: None,
          template_file: None,
          json: Box::new(serde_json::Value::Null),
  ...
  fn test_extract_static_query_params_from_next_url() {
    ...
    let query_analysis = crate::pattern::QueryParamAnalysis {
      has_variations: true,
    ...
    let static_params =
      ResponseAnalyzer::extract_static_query_params(&objects, &next_field, &prev_field, &query_analysis);
  ...
  fn test_pagination_pattern_has_static_query_params() {
    ...
    let mocks = vec![
      MockConfig {
        ...
        match_config: Some(MatchConfig {
          methods: vec!["GET".to_string()],
          urls: vec!["exact:/api/search?q=test&status=active&page=1&limit=10".to_string()],
          ..Default::default()
        }),
        request: None,
        response_config: Some(ReturnConfig::Structured {
      ...
      MockConfig {
        ...
        match_config: Some(MatchConfig {
          methods: vec!["GET".to_string()],
          urls: vec!["exact:/api/search?q=test&status=active&page=2&limit=10".to_string()],
          ..Default::default()
        }),
        request: None,
        response_config: Some(ReturnConfig::Structured {
  ...
  fn test_graphql_variable_analysis_non_graphql_mocks() {
    use mockpit_config::MatchConfig;
