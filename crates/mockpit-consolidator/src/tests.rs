...
use crate::{ConsolidatorOptions, MockConsolidator};
use mockpit_config::{MatchConfig, MockCollectionConfig, MockConfig, ReturnConfig};
use mockpit_template::{render_template, validate_template};
...
async fn test_consolidation_creates_valid_mocks() {
  ...
  for mock in &consolidated.mocks {
    ...
    assert!(mock.match_config.is_some(), "Mock {} missing match_config", mock.id);
...
async fn test_template_generation_for_varying_fields() {
  ...
  let template_mock = consolidated
    .mocks
    .iter()
    .find(|m| m.response_config.as_ref().and_then(|r| r.template()).is_some())
    .expect("Should have at least one template mock");

  let tmpl = template_mock.response_config.as_ref().unwrap().template().unwrap();
  ...
  assert!(tmpl.contains("total"), "Template should include 'total' field");
  assert!(
...
async fn test_express_style_pattern_generation() {
  let mocks = vec![
    ...
    create_test_mock("m3", "GET", "/users/789", r#"{"id": 789, "name": "Charlie"}"#),
  ];
...
async fn test_consolidation_with_disabled_templates() {
  let mocks = vec![
    ...
    create_test_mock("m3", "GET", "/api/users/3", r#"{"id": 3, "name": "Charlie"}"#),
  ];
...
async fn test_consolidation_preserves_mock_properties() {
  ...
  assert!(mock.response_config.is_some(), "Mock should have response_config");
}
...
async fn test_consolidation_statistics_accuracy() {
  ...
  assert!(stats.reduction_ratio > 0.0, "Should have positive reduction ratio");
...
async fn test_non_json_responses_not_templated() {
  let mocks = vec![
    create_test_mock("m1", "GET", "/api/html/1", "<html><body>Page 1</body></html>"),
    create_test_mock("m2", "GET", "/api/html/2", "<html><body>Page 2</body></html>"),
    create_test_mock("m3", "GET", "/api/html/3", "<html><body>Page 3</body></html>"),
  ];
...
async fn test_semantic_penalty_prevents_false_positives() {
  ...
  let url_values = [json!("https://example.com/user1"), json!("https://example.com/user2")];

  let (field_type, confidence) = detector.detect_type("email_url", &url_values.iter().collect::<Vec<_>>());
  ...
  assert!(confidence >= 0.7, "Should have reasonable confidence: {}", confidence);
}
...
async fn test_consolidation_groups_by_priority() {
  ...
  let mut mocks = vec![];
  for i in 1..=3 {
    let mut mock = create_test_mock(&format!("normal-{}", i), "GET", "/api/data", r#"{"value": 1}"#);
    mock.priority = 100; // Normal priority
    mocks.push(mock);
  }

  for i in 1..=3 {
    let mut mock = create_test_mock(&format!("high-{}", i), "GET", "/api/data", r#"{"value": 2}"#);
  ...
  let normal_mocks: Vec<_> = consolidated
    ...
    .collect();
  let high_mocks: Vec<_> = consolidated.mocks.iter().filter(|m| m.priority >= 500).collect();

  assert!(!normal_mocks.is_empty(), "Should have normal priority mocks preserved");
  assert!(!high_mocks.is_empty(), "Should have high priority mocks preserved");
}
...
async fn test_consolidation_single_mock() {
  let collection = MockCollectionConfig {
    ...
    enabled: true,
    vars: None,
    mocks: vec![create_test_mock("solo", "GET", "/api/solo", r#"{"ok": true}"#)],
  };
...
async fn test_consolidation_special_characters_in_body() {
  ...
  assert!(result.is_ok(), "Consolidation should handle special characters");
...
async fn test_consolidation_preserves_collection_metadata() {
  let collection = MockCollectionConfig {
    ...
    enabled: true,
    vars: None,
    mocks: vec![create_test_mock("m1", "GET", "/api/test", r#"{"ok": true}"#)],
  };
