...
async fn test_pagination_pattern_detection() {
  ...
  let mocks = vec![
    create_mock("1", "GET", "/api/users?page=1", 200, r#"{"users":[{"id":1}]}"#),
    create_mock("2", "GET", "/api/users?page=2", 200, r#"{"users":[{"id":2}]}"#),
    create_mock("3", "GET", "/api/users?page=3", 200, r#"{"users":[{"id":3}]}"#),
    create_mock("4", "GET", "/api/users?page=4", 200, r#"{"users":[{"id":4}]}"#),
  ];
  ...
  println!("Patterns detected: {}", consolidator.stats().patterns_detected);

  // Should consolidate pagination into fewer mocks
  assert!(result.mocks.len() < 4, "Should consolidate pagination patterns");
  assert!(
  ...
  let has_prefix_pattern = match_config.url.as_ref().is_some_and(|u| u.contains("prefix:"))
...
async fn test_id_based_pattern_detection() {
  ...
  let mocks = vec![
    create_mock("1", "GET", "/api/users/123", 200, r#"{"id":123,"name":"Alice"}"#),
    create_mock("2", "GET", "/api/users/456", 200, r#"{"id":456,"name":"Bob"}"#),
    create_mock("3", "GET", "/api/users/789", 200, r#"{"id":789,"name":"Charlie"}"#),
    create_mock("4", "GET", "/api/users/999", 200, r#"{"id":999,"name":"David"}"#),
  ];
  ...
  println!("Original mocks: 4");
  println!("Consolidated mocks: {}", result.mocks.len());
  println!("ID patterns detected: {}", consolidator.stats().patterns_detected);

  // Should consolidate ID-based patterns
  assert!(result.mocks.len() < 4, "Should consolidate ID patterns");
  assert!(consolidator.stats().patterns_detected > 0, "Should detect ID pattern");
  ...
  let has_regex_pattern = match_config.url.as_ref().is_some_and(|u| u.contains("regex:"))
...
async fn test_uuid_pattern_detection() {
  ...
  assert!(consolidator.stats().patterns_detected > 0, "Should detect UUID pattern");
...
async fn test_search_query_pattern() {
  ...
  let mocks = vec![
    create_mock("1", "GET", "/api/search?q=foo", 200, r#"{"results":["foo1","foo2"]}"#),
    create_mock("2", "GET", "/api/search?q=bar", 200, r#"{"results":["bar1","bar2"]}"#),
    create_mock("3", "GET", "/api/search?q=baz", 200, r#"{"results":["baz1","baz2"]}"#),
  ];
  ...
  assert!(result.mocks.len() < 3, "Should consolidate search query patterns");
...
async fn test_duplicate_removal() {
  ...
  println!("Original mocks: 4");
  println!("Consolidated mocks: {}", result.mocks.len());
  println!("Duplicates removed: {}", consolidator.stats().duplicates_removed);

  // Should remove all duplicates, keeping only one
  assert_eq!(result.mocks.len(), 1, "Should keep only one mock after deduplication");
  assert!(
...
async fn test_variable_response_template() {
  ...
  println!("Original mocks: 3");
  println!("Consolidated mocks: {}", result.mocks.len());
  println!("Templates created: {}", consolidator.stats().templates_created);

  // Should create a template for variable responses
  assert!(result.mocks.len() <= 1, "Should consolidate into template");
  assert!(consolidator.stats().templates_created > 0, "Should create template");
...
async fn test_mixed_patterns() {
  ...
  let mocks = vec![
    ...
    create_mock("u2", "POST", "/api/unique/endpoint2", 201, r#"{"data":"b"}"#),
  ];
  ...
  assert!(result.mocks.len() >= 4, "Should keep unique requests separate");
  assert!(
...
async fn test_no_consolidation_for_single_mock() {
  ...
  let mocks = vec![create_mock("1", "GET", "/api/unique", 200, r#"{"status":"ok"}"#)];
  ...
  assert_eq!(consolidator.stats().patterns_detected, 0, "Should not detect patterns");
...
async fn test_min_pattern_threshold() {
  ...
  assert_eq!(result.mocks.len(), 2, "Should not consolidate below threshold");
...
async fn test_disable_consolidation() {
  ...
  assert_eq!(result.mocks.len(), 3, "Should not consolidate when disabled");
...
async fn test_complex_real_world_scenario() {
  ...
  println!("Simulating real-world recording with {} mocks:", original_count);
  ...
  assert!(stats.patterns_detected >= 3, "Should detect multiple patterns");
  assert!(stats.duplicates_removed >= 4, "Should remove health check duplicates");
...
async fn test_preserves_different_methods() {
  ...
  assert_eq!(result.mocks.len(), 4, "Should not consolidate different HTTP methods");
...
async fn test_path_normalization_via_grouping() {
  ...
  assert!(result.mocks.len() < 3, "Should group similar paths together");
...
async fn test_cursor_based_pagination() {
  ...
  assert!(result.mocks.len() < 3, "Should consolidate cursor pagination");
  assert!(
    consolidator.stats().patterns_detected > 0,
...
async fn test_complex_query_params() {
  ...
  assert!(result.mocks.len() < 3, "Should consolidate complex query params");
  assert!(consolidator.stats().patterns_detected > 0, "Should detect pattern");
