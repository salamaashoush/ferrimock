...
use crate::types::{PaginationType, ResponseStructure};
use mockpit_type_detector::FieldType;
use rustc_hash::FxHashSet;

use crate::array_object::{generate_tera_array_with_limit, generate_tera_object_with_extension};
use crate::field_converter::{field_type_to_tera_expr, field_type_to_tera_expr_with_context};
use crate::file_detection::extract_file_extension_from_response;
use crate::helpers::detect_results_array_field;
use crate::pagination::{
  generate_cursor_pagination_preamble, generate_offset_pagination_preamble, generate_page_pagination_preamble,
  generate_pagination_fields,
};
...
impl TemplateGenerator {
  ...
  pub fn generate_tera_template(
    ...
    if let Some(ref pagination) = analysis.pagination {
      let storage_path = base_path.trim_start_matches('/').replace('/', ".").replace('-', "_");
      ...
      let pagination_field_strs = generate_pagination_fields(pagination, base_path, &mut pagination_fields);
    ...
    for (field, field_type) in &analysis.varying_fields {
      if !pagination_fields.contains(field) {
        ...
        let graphql_var_expr = Self::try_graphql_variable_expression(field, graphql_analysis);
        ...
        let expr = if let Some(gql_expr) = graphql_var_expr {
          ...
          Self::field_type_to_tera_expr_with_limit(field, field_type, analysis.has_matching_path_ids)
        } else if matches!(field_type, FieldType::DownloadUrl { .. }) && file_extension.is_some() {
  ...
  fn try_graphql_variable_expression(
    ...
    if let Some(last_part) = field_parts.last() {
      ...
      if graphql_analysis.varying_variables.contains(&clean_part.to_string()) {
    ...
    if field_parts.len() == 1 {
      ...
      if graphql_analysis.varying_variables.contains(&clean_field.to_string()) {
  ...
  fn field_type_to_tera_expr_with_extension(
    ...
    match field_type {
      FieldType::Object(analysis) => {
        generate_tera_object_with_extension(analysis, has_matching_path_ids, extension, graphql_analysis)
      },
      _ => field_type_to_tera_expr(field_name, field_type, has_matching_path_ids),
    }
  }
...
mod tests {
  ...
  fn test_box_file_object_with_pdf_download_url() {
    ...
    let sign_object = FieldType::Object(Box::new(mockpit_type_detector::ObjectAnalysis {
      varying_fields: vec![(
        "download_url".to_string(),
        FieldType::DownloadUrl {
          sample_url: Some("https://dl.cdn.example.com/d/1/b0!6mFruo9M1-NhU-3a1Ou3mh...".to_string()),
        },
      )],
      constant_fields: vec![(
    ...
    let analysis = ResponseStructure {
      ...
      constant_fields: vec![
        ("type".to_string(), serde_json::Value::String("file".to_string())),
        ("extension".to_string(), serde_json::Value::String("pdf".to_string())),
      ],
    ...
    let graphql_analysis = crate::types::GraphQLVariableInfo::empty();
    let template = generator.generate_tera_template(&analysis, "/api/file-info", &graphql_analysis);

    // Verify template is valid
    assert!(
      mockpit_template::validate_template(&template).is_ok(),
  ...
  fn test_box_file_object_with_png_download_url() {
    ...
    let sign_object = FieldType::Object(Box::new(mockpit_type_detector::ObjectAnalysis {
      varying_fields: vec![(
        "download_url".to_string(),
        FieldType::DownloadUrl {
          sample_url: Some("https://dl.cdn.example.com/d/1/abc123...".to_string()),
        },
      )],
      constant_fields: vec![(
        "download_status".to_string(),
    ...
    let analysis = ResponseStructure {
      ...
      constant_fields: vec![
        ("type".to_string(), serde_json::Value::String("file".to_string())),
        ("extension".to_string(), serde_json::Value::String("png".to_string())),
      ],
      has_matching_path_ids: false,
      is_json: true,
      top_level_type: "object".to_string(),
    ...
    assert!(mockpit_template::validate_template(&template).is_ok());
  ...
  fn test_box_file_object_with_jpeg_download_url() {
    ...
    let sign_object = FieldType::Object(Box::new(mockpit_type_detector::ObjectAnalysis {
      varying_fields: vec![(
        "download_url".to_string(),
        FieldType::DownloadUrl {
          sample_url: Some("https://dl.cdn.example.com/d/1/xyz789...".to_string()),
        },
      )],
      constant_fields: vec![(
        "download_status".to_string(),
    ...
    let analysis = ResponseStructure {
      ...
      constant_fields: vec![
        ("type".to_string(), serde_json::Value::String("file".to_string())),
        ("extension".to_string(), serde_json::Value::String("jpg".to_string())),
      ],
      has_matching_path_ids: false,
      is_json: true,
      top_level_type: "object".to_string(),
  ...
  #[test]
  fn test_file_with_authenticated_download_url() {
    let generator = TemplateGenerator::new("api.{path}.total".to_string());
    ...
    let sign_object = FieldType::Object(Box::new(mockpit_type_detector::ObjectAnalysis {
      varying_fields: vec![(
        "download_url".to_string(),
        FieldType::DownloadUrl {
          sample_url: Some("https://dl.cdn.example.com/d/1/long_url...".to_string()),
        },
      )],
      constant_fields: vec![(
        "download_status".to_string(),
    ...
    for (ext, expected_generator) in &[
      ...
      let sign_object = FieldType::Object(Box::new(mockpit_type_detector::ObjectAnalysis {
        varying_fields: vec![(
          "download_url".to_string(),
          FieldType::DownloadUrl {
            sample_url: Some("https://dl.cdn.example.com/d/1/test...".to_string()),
          },
        )],
        constant_fields: vec![],
      }));
      ...
      let analysis = ResponseStructure {
        ...
        constant_fields: vec![
          ("type".to_string(), serde_json::Value::String("file".to_string())),
          ("extension".to_string(), serde_json::Value::String((*ext).to_string())),
        ],
        has_matching_path_ids: false,
        is_json: true,
        top_level_type: "object".to_string(),
  ...
  fn test_generate_tera_template_simple_fields() {
    ...
    let analysis = ResponseStructure {
      varying_fields: vec![
        ("id".to_string(), FieldType::RandomNumber { min: None, max: None }),
        ("name".to_string(), FieldType::Name),
        ("email".to_string(), FieldType::Email),
      ],
      constant_fields: vec![("type".to_string(), serde_json::Value::String("user".to_string()))],
      has_matching_path_ids: false,
      is_json: true,
      top_level_type: "object".to_string(),
      pagination: None,
      ...
      varying_fields: vec![(
        ...
        FieldType::Array(Box::new(ArrayPattern {
          ...
          element_type: FieldType::Object(Box::new(mockpit_type_detector::ObjectAnalysis {
            varying_fields: vec![("id".to_string(), FieldType::RandomNumber { min: None, max: None })],
            constant_fields: vec![],
          })),
  ...
  fn test_page_based_pagination_without_static_params() {
    ...
    let analysis = ResponseStructure {
      varying_fields: vec![(
        ...
        FieldType::Array(Box::new(ArrayPattern {
          is_homogeneous: true,
          element_type: FieldType::RandomNumber { min: None, max: None },
  ...
  fn test_offset_based_pagination_with_static_params() {
    ...
    let graphql_analysis = crate::types::GraphQLVariableInfo::empty();
    let template = generator.generate_tera_template(&analysis, "/api/records", &graphql_analysis);
  ...
  fn test_cursor_based_pagination() {
    ...
    let graphql_analysis = crate::types::GraphQLVariableInfo::empty();
    let template = generator.generate_tera_template(&analysis, "/api/stream", &graphql_analysis);
  ...
  fn test_results_array_uses_limit_variable() {
    ...
    let graphql_analysis = crate::types::GraphQLVariableInfo::empty();
    let template = generator.generate_tera_template(&analysis, "/api/search", &graphql_analysis);
  ...
  fn test_graphql_template_with_variable_extraction() {
    ...
    let analysis = ResponseStructure {
      varying_fields: vec![
        (
          ...
          FieldType::RandomNumber { min: None, max: None },
        ),
    ...
    let graphql_analysis = crate::types::GraphQLVariableInfo {
  ...
  fn test_cursor_pagination_without_total_field() {
    ...
    let graphql_analysis = crate::types::GraphQLVariableInfo::empty();
    let template = generator.generate_tera_template(&analysis, "/api/templates", &graphql_analysis);
