...
pub(super) fn to_har_entry(interaction: &RecordedInteraction) -> v1_2::Entries {
  ...
  let request_body_size = interaction.request.body.as_ref().map(|b| b.len() as i64).unwrap_or(0);

  let response_body_size = interaction.response.body.len() as i64;

  // Detect if binary data (contains our sentinel string)
  let (response_text, response_encoding) = if interaction.response.body.starts_with("<binary data:") {
  ...
  v1_2::Entries {
    ...
    request: v1_2::Request {
      ...
      query_string,
      post_data: interaction.request.body.as_ref().map(|body| v1_2::PostData {
