...
pub use types::{RecordedInteraction, RecordingSession};
...
impl MockRecorder {
  ...
  pub fn with_format(
    ...
    Self::with_filters(session_name, storage_dir, format, RecordingFilterOptions::default())
  }
  ...
  pub async fn init_file(&self) -> Result<PathBuf> {
    ...
    match self.format {
      RecordingFormat::Json => {
        ...
        let description_value = format!("Auto-generated from recording session {}", self.session_id);
        ...
        let har = Har {
          log: Spec::V1_2(v1_2::Log {
            creator: v1_2::Creator {
              name: "the internal gateway".to_string(),
  ...
  pub async fn record(
    ...
    let (decompressed_response, was_decompressed) = Self::decompress_if_gzipped(response_body, response_headers);
    ...
    // Store in memory
    self.interactions.insert(interaction_id.clone(), interaction.clone());

    // Handle error context tracking for auto-export
    if self.filter_options.auto_export_on_error && self.filter_options.error_context_requests > 0 {
  ...
  fn detect_graphql_request(
  ...
  ) -> Option<mockpit_config::GraphQLMatchConfig> {
    use mockpit_config::GraphQLMatchConfig;
    ...
    let operation_type = if query_trimmed.starts_with("query") || query_trimmed.starts_with("query ") {
      Some("query")
    } else if query_trimmed.starts_with("mutation") || query_trimmed.starts_with("mutation ") {
      Some("mutation")
    } else if query_trimmed.starts_with("subscription") || query_trimmed.starts_with("subscription ") {
      Some("subscription")
  ...
  async fn append_interaction_to_file(
    ...
    if let Some(file) = handle_guard.as_mut() {
      ...
      let use_file = Self::should_use_file_body(&interaction.response.body, &interaction.response.headers);
  ...
  pub fn get_all(&self) -> Vec<RecordedInteraction> {
    self.interactions.iter().map(|entry| entry.value().clone()).collect()
  }
  ...
  async fn write_body_files(&self, collection: &mockpit_config::MockCollectionConfig) -> Result<()> {
    // Create bodies directory
  ...
  fn create_mock_collection(&self) -> mockpit_config::MockCollectionConfig {
  ...
  async fn export_error_context(&self, context_interactions: Vec<RecordedInteraction>) -> Result<()> {
    // Add context interactions to this export instance
    for interaction in context_interactions {
      self.interactions.insert(interaction.id.clone(), interaction);
    }
...
mod tests {
  ...
  fn test_json_header_serialization() {
    ...
    assert!(json_str.contains("\"name\""), "JSON should contain 'name' field");
    assert!(
    ...
    assert!(json_str.rfind(']').is_some(), "JSON should contain ']' bracket");
  }
  ...
  async fn test_init_and_finalize_file() {
    ...
    let recorder = MockRecorder::with_format("test-session", temp_dir.path(), RecordingFormat::Json);
    ...
    assert!(json_value.get("name").is_some(), "JSON should have 'name' field");
  ...
  async fn test_save_and_load_yaml() {
    ...
    // Load as mock collection
    let content = tokio::fs::read_to_string(&file_path).await.unwrap();
    let collection: mockpit_config::MockCollectionConfig = serde_yaml::from_str(&content).unwrap();
  ...
  async fn test_har_format_save() {
    ...
    assert!(get_entry.response.content.text.as_ref().unwrap().contains("Test User"));
  ...
  async fn test_har_streaming_format() {
    let temp_dir = TempDir::new().unwrap();
    let recorder = MockRecorder::with_format("har-stream", temp_dir.path(), RecordingFormat::Har);
  ...
  async fn test_har_with_headers() {
    ...
    assert!(entry.request.headers.iter().any(|h| h.name == "authorization"));
    assert!(entry.request.headers.iter().any(|h| h.name == "content-type"));

    // Check response headers
    assert!(entry.response.headers.iter().any(|h| h.name == "content-type"));
    assert!(entry.response.headers.iter().any(|h| h.name == "x-request-id"));
  }
  ...
  async fn test_gzip_decompression_header_fix() {
    ...
    resp_headers.insert("content-length", gzipped_body.len().to_string().parse().unwrap());
    resp_headers.insert("content-type", "application/json".parse().unwrap());
  ...
  fn test_detect_graphql_query_with_operation_name() {
    ...
    let method = "POST";
    let body =
      r#"{"query":"query GetUser { user(id: \"123\") { name } }","operationName":"GetUser","variables":{"id":"123"}}"#;

    let result = MockRecorder::detect_graphql_request(uri, method, Some(body));
    assert!(result.is_some());

    if let Some(mockpit_config::GraphQLMatchConfig::Structured {
      query,
      mutation,
      subscription,
      variables,
  ...
  fn test_detect_graphql_mutation() {
    ...
    let method = "POST";
    let body =
      r#"{"query":"mutation CreateUser { createUser(input: {name: \"John\"}) { id } }","operationName":"CreateUser"}"#;

    let result = MockRecorder::detect_graphql_request(uri, method, Some(body));
    assert!(result.is_some());

    if let Some(mockpit_config::GraphQLMatchConfig::Structured { query, mutation, .. }) = result {
  ...
  fn test_detect_graphql_introspection() {
    ...
    if let Some(mockpit_config::GraphQLMatchConfig::Structured { introspection, .. }) = result {
      assert_eq!(
        introspection,
        Some(IntrospectionMatchConfig::String("schema".to_string()))
      );
  ...
  fn test_detect_graphql_no_operation_name() {
    ...
    assert!(matches!(
      result,
      Some(mockpit_config::GraphQLMatchConfig::Boolean(true))
    ));
  }
  ...
  fn test_detect_graphql_with_variables() {
    ...
    if let Some(mockpit_config::GraphQLMatchConfig::Structured { variables, .. }) = result {
      assert_eq!(variables.get("id").and_then(|v| v.as_str()), Some("123"));
  ...
  async fn test_record_graphql_query() {
    ...
    if let Some(mockpit_config::GraphQLMatchConfig::Structured { query, variables, .. }) = &match_config.graphql {
      assert_eq!(query, &Some("GetUser".to_string()));
      assert_eq!(variables.get("id").and_then(|v| v.as_str()), Some("123"));
    } else {
  ...
  async fn test_streaming_single_mock_produces_valid_json() {
    let temp_dir = TempDir::new().unwrap();
    let recorder = MockRecorder::with_format("stream-single", temp_dir.path(), RecordingFormat::Json);
    ...
    let parsed: serde_json::Value =
      serde_json::from_str(&content).unwrap_or_else(|e| panic!("Invalid JSON: {}\nContent:\n{}", e, content));

    // Must parse as MockCollectionConfig
    let collection: mockpit_config::MockCollectionConfig = serde_json::from_str(&content)
      .unwrap_or_else(|e| panic!("Failed to parse as MockCollectionConfig: {}\nContent:\n{}", e, content));
  ...
  async fn test_streaming_multiple_mocks_produces_valid_json() {
    let temp_dir = TempDir::new().unwrap();
    let recorder = MockRecorder::with_format("stream-multi", temp_dir.path(), RecordingFormat::Json);
    ...
    assert_eq!(collection.mocks.len(), 5, "Should have 5 mocks in the collection");

    // Verify all mock IDs are unique
    let ids: rustc_hash::FxHashSet<&str> = collection.mocks.iter().map(|m| m.id.as_str()).collect();
  ...
  async fn test_streaming_concurrent_records_produce_valid_json() {
    ...
    let collection: mockpit_config::MockCollectionConfig = serde_json::from_str(&content)
      .unwrap_or_else(|e| panic!("Invalid JSON from concurrent writes: {}\nContent:\n{}", e, content));
  ...
  async fn test_streaming_empty_recording_produces_valid_json() {
    let temp_dir = TempDir::new().unwrap();
    let recorder = MockRecorder::with_format("stream-empty", temp_dir.path(), RecordingFormat::Json);
    ...
    let collection: mockpit_config::MockCollectionConfig = serde_json::from_str(&content)
      .unwrap_or_else(|e| panic!("Empty recording should be valid JSON: {}\nContent:\n{}", e, content));
  ...
  async fn test_streaming_with_filtered_requests_valid_json() {
    ...
    let collection: mockpit_config::MockCollectionConfig = serde_json::from_str(&content)
      .unwrap_or_else(|e| panic!("Filtered recording should be valid JSON: {}\nContent:\n{}", e, content));

    assert_eq!(collection.mocks.len(), 1, "Should only have the successful request");
  }

  #[tokio::test]
  async fn test_streaming_yaml_multiple_mocks() {
    let temp_dir = TempDir::new().unwrap();
    let recorder = MockRecorder::with_format("stream-yaml", temp_dir.path(), RecordingFormat::Yaml);
    ...
    let collection: mockpit_config::MockCollectionConfig = serde_yaml::from_str(&content)
      .unwrap_or_else(|e| panic!("Streaming YAML should be valid: {}\nContent:\n{}", e, content));
  ...
  async fn test_streaming_output_loadable_by_consolidator() {
    ...
    let temp_dir = TempDir::new().unwrap();
    let recorder = MockRecorder::with_format("stream-e2e", temp_dir.path(), RecordingFormat::Json);
