use mockpit_template::{RequestContext, hash_template, render_template, render_template_with_hash, validate_template};
...
fn bench_render_fake_data(c: &mut Criterion) {
  ...
  group.bench_function("single_fake_name", |b| {
    b.iter(|| render_template(black_box(r#"{"name": "{{ fake_name() }}"}"#), black_box(&ctx)))
  });
...
fn bench_render_cache_miss(c: &mut Criterion) {
  ...
  group.bench_function("unique_template_compile", |b| {
    b.iter(|| {
      counter += 1;
      let template = format!(r#"{{"id": "unique_{}", "name": "{{{{ fake_name() }}}}"}}"#, counter);
...
fn bench_new_features(c: &mut Criterion) {
  ...
  group.bench_function("now_plus_days", |b| {
    b.iter(|| render_template(black_box(r#"{"expires": "{{ now_plus(days=30) }}"}"#), black_box(&ctx)))
  });

  group.bench_function("now_minus_hours", |b| {
    b.iter(|| render_template(black_box(r#"{"created": "{{ now_minus(hours=2) }}"}"#), black_box(&ctx)))
  });
...
fn bench_precomputed_hash(c: &mut Criterion) {
  ...
  group.bench_function("precomputed_hash", |b| {
    b.iter(|| render_template_with_hash(black_box(template), black_box(hash), black_box(&ctx), None))
  });
