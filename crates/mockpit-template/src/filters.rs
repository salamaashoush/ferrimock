...
pub fn b64decode(value: &TeraValue, _args: &HashMap<String, TeraValue>) -> TeraResult<TeraValue> {
  ...
  let result = String::from_utf8(decoded).map_err(|e| tera::Error::msg(format!("UTF-8 decode error: {}", e)))?;
  Ok(to_value(result)?)
}
...
/// ```
pub fn b64encode_urlsafe(value: &TeraValue, _args: &HashMap<String, TeraValue>) -> TeraResult<TeraValue> {
...
/// ```
pub fn b64decode_urlsafe(value: &TeraValue, _args: &HashMap<String, TeraValue>) -> TeraResult<TeraValue> {
...
pub fn json_parse(value: &TeraValue, _args: &HashMap<String, TeraValue>) -> TeraResult<TeraValue> {
  ...
  let parsed: Value = serde_json::from_str(&s).map_err(|e| tera::Error::msg(format!("JSON parse error: {}", e)))?;
  Ok(to_value(parsed)?)
}
...
pub fn urldecode(value: &TeraValue, _args: &HashMap<String, TeraValue>) -> TeraResult<TeraValue> {
  ...
  let decoded = urlencoding::decode(&s).map_err(|e| tera::Error::msg(format!("URL decode error: {}", e)))?;
...
/// ```
pub fn random_choice(value: &TeraValue, _args: &HashMap<String, TeraValue>) -> TeraResult<TeraValue> {
  use rand::RngExt;
  ...
  if values.is_empty() {
    return Err(tera::Error::msg("random_choice filter requires a non-empty array"));
  }
...
mod tests {
  ...
  fn test_b64encode() {
    ...
    assert_eq!(result.as_str().expect("Result should be a string"), "SGVsbG8gV29ybGQ=");
  }
  ...
  fn test_b64decode() {
    ...
    assert_eq!(result.as_str().expect("Result should be a string"), "Hello World");
  }

  #[test]
  fn test_json_parse() {
    let value = to_value(r#"{"name":"John","age":30}"#).expect("Failed to convert JSON string to Tera value");
