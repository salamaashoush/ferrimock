...
static LINE_REGEX: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"line (\d+)").expect("valid regex"));
static COLUMN_REGEX: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"column (\d+)").expect("valid regex"));
...
fn generate_template_suggestions(message: &str) -> Vec<String> {
  ...
  if message.contains("cannot sample empty range") || message.contains("sample empty range") {
    suggestions.push("The get_random() function requires start < end (e.g., get_random(start=1, end=5))".to_string());
    suggestions.push("Check if you're using get_random(start=X, end=X) which creates an empty range".to_string());
    suggestions.push("If generated by consolidator, this might be a bug - please report".to_string());
  }

  // Specific error: division by zero
  if message.contains("division by zero") || message.contains("divide by zero") {
    suggestions.push("Check for division operations where the denominator might be 0".to_string());
    suggestions.push(
  ...
  if message.contains("null") && (message.contains("access") || message.contains("index")) {
    ...
    suggestions.push("Use the default filter: {{ variable | default(value=\"fallback\") }}".to_string());
    suggestions.push("Or use conditional: {% if variable %}{{ variable.field }}{% endif %}".to_string());
  }
  ...
  if message.contains("unknown variable") || message.contains("Variable") {
    suggestions.push("Check available variables: method, path, captures, query, headers, body, body_json".to_string());
  ...
  if message.contains("parse") || message.contains("syntax") {
    ...
    suggestions.push("Common syntax: {{ variable }}, {% if condition %}, {% for item in array %}".to_string());
  }

  if message.contains("filter") {
    suggestions.push("Common filters: json_encode, lower, upper, trim, length, default".to_string());
    suggestions.push("Custom filters: base64_encode, base64_decode, urldecode, random_choice".to_string());
    ...
    && (message.contains("number") || message.contains("string") || message.contains("array"))
  {
    ...
    suggestions.push("Use type conversion: {{ value | int }} or {{ value | string }}".to_string());
  }
...
mod tests {
  ...
  fn test_error_suggestions_empty_range() {
    ...
    assert!(!suggestions.is_empty(), "Should have suggestions for empty range error");
    assert!(
  ...
  fn test_error_suggestions_unknown_variable() {
    ...
    assert!(!suggestions.is_empty());
    assert!(
      suggestions.iter().any(|s| s.contains("method") || s.contains("path")),
