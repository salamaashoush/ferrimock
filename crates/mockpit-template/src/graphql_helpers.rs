...
pub fn graphql_error(args: &HashMap<String, Value>) -> Result<Value> {
  ...
  result.insert("errors".to_string(), Value::Array(vec![Value::Object(error)]));
...
pub fn graphql_field_error(args: &HashMap<String, Value>) -> Result<Value> {
  ...
  let path: Vec<Value> = field.split('.').map(|s| Value::String(s.to_string())).collect();
...
pub fn graphql_type(args: &HashMap<String, Value>) -> Result<Value> {
  ...
  let kind = args.get("kind").and_then(|v| v.as_str()).unwrap_or("OBJECT");
...
pub fn graphql_schema(args: &HashMap<String, Value>) -> Result<Value> {
  let query_type = args.get("queryType").and_then(|v| v.as_str()).unwrap_or("Query");
...
mod tests {
  ...
  fn test_graphql_error() {
    ...
    args.insert("message".to_string(), Value::String("Not found".to_string()));
  ...
  fn test_graphql_field_error() {
    ...
    args.insert("message".to_string(), Value::String("Invalid email".to_string()));
    args.insert("code".to_string(), Value::String("VALIDATION_ERROR".to_string()));
    ...
    assert_eq!(result["errors"][0]["extensions"]["code"], "VALIDATION_ERROR");
  }
  ...
  fn test_graphql_schema() {
    ...
    args.insert("mutationType".to_string(), Value::String("Mutation".to_string()));

    let result = graphql_schema(&args).expect("Failed to create GraphQL schema");
    assert_eq!(result["data"]["__schema"]["queryType"]["name"], "Query");
    assert_eq!(result["data"]["__schema"]["mutationType"]["name"], "Mutation");
