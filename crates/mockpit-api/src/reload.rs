...
pub async fn reload_mocks(
  ...
  if let Some(dir) = payload.dir {
    ...
    if let Some(collections_dir) = &app_state.config.collections_dir {
      ...
      match app_state.mock.mock_registry.load_from_directory(&collections_str).await {
        Ok(count) => {
          debug!("Reloaded {} mocks from collections: {}", count, collections_str);
          total_count += count;
    ...
    if let Some(recordings_dir) = &app_state.config.recordings_dir {
      if recordings_dir.exists() {
        ...
        match app_state.mock.mock_registry.load_from_directory(&recordings_str).await {
          Ok(count) => {
            debug!("Reloaded {} recordings as mocks from: {}", count, recordings_str);
            total_count += count;
  ...
  let response = ReloadResponse {
    success: true,
    message: format!("Reloaded {} mock(s) from: {}", total_count, loaded_dirs.join(", ")),
    count: total_count,
  };
