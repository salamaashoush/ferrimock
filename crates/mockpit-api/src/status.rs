...
pub async fn get_status(State(app_state): State<MockApiState>) -> impl IntoResponse {
  ...
  let total_calls: usize = tracked_mocks.iter().map(|id| registry.get_call_count(id)).sum();
...
pub async fn get_recordings(State(app_state): State<MockApiState>) -> impl IntoResponse {
  ...
  response.insert("count".to_string(), serde_json::Value::Number(recordings.len().into()));
...
pub async fn finalize_recordings(State(app_state): State<MockApiState>) -> impl IntoResponse {
  ...
  (StatusCode::CREATED, Json(serde_json::Value::Object(success_response))).into_response()
}
...
pub async fn set_mode(State(app_state): State<MockApiState>, Json(request): Json<SetModeRequest>) -> impl IntoResponse {
  ...
  if !["hybrid", "selective", "full"].contains(&mode.as_str()) {
    ...
    return (StatusCode::BAD_REQUEST, Json(serde_json::Value::Object(response))).into_response();
  }
...
pub async fn reset_scenario() -> impl IntoResponse {
  ...
  response.insert(
    ...
    serde_json::Value::String(
      "Scenarios are deprecated and no longer supported. Use state management instead.".to_string(),
    ),
  );
...
pub async fn start_recording(
  ...
  let result: Result<String, String> = async {
    ...
    let session = options.session.unwrap_or_else(|| {
    ...
    let output_dir = app_state.config.recordings_dir
      .clone()
  ...
  match result {
    Ok(session_name) => {
      ...
      response.insert("session".to_string(), serde_json::Value::String(session_name));
    ...
    Err(e) => {
      ...
      (StatusCode::BAD_REQUEST, Json(serde_json::Value::Object(response))).into_response()
    },
  }
}
...
pub async fn stop_recording(
  ...
  let Some(recorder) = recorder_guard.take() else {
    let mut response = serde_json::Map::new();
    response.insert("success".to_string(), serde_json::Value::Bool(false));
    response.insert("error".to_string(), serde_json::Value::String("No recording in progress".to_string()));
    return (StatusCode::BAD_REQUEST, Json(serde_json::Value::Object(response))).into_response();
  };
  drop(recorder_guard);
  ...
  if let Err(e) = recorder.finalize_file().await {
    let mut response = serde_json::Map::new();
    response.insert("success".to_string(), serde_json::Value::Bool(false));
    response.insert("error".to_string(), serde_json::Value::String(format!("Failed to finalize: {e}")));
    return (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::Value::Object(response))).into_response();
  }
          ...
          Ok(_collection) => {
            ...
            stats_obj.insert(
              ...
              serde_json::Value::Number(
                serde_json::Number::from_f64(stats.reduction_ratio * 100.0).unwrap_or(serde_json::Number::from(0)),
              ),
            );
            response.insert("consolidation".to_string(), serde_json::Value::Object(stats_obj));
          },
          Err(e) => {
