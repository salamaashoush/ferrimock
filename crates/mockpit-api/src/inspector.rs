...
use crate::types::{EvaluatedMock, InspectRequest, InspectResponse, MatchDetails, MatchedMock};
use axum::{Json, extract::State, http::StatusCode, response::IntoResponse};
use std::time::Instant;

/// Inspect how a request would match against mocks
///
/// POST /__mockpit/inspect
pub async fn inspect_request(
  ...
  let method = match request.method.parse::<axum::http::Method>() {
    ...
    Err(e) => {
      ...
      return (StatusCode::BAD_REQUEST, Json(serde_json::Value::Object(error_response))).into_response();
    },
  };
  ...
  let mock_match =
    app_state
      .mock
      .mock_matcher
      .find_match(&method, &request.path, request.query.as_deref(), &headers, body);
