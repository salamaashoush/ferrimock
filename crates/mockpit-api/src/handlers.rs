...
use crate::query::{apply_filters, parse_query};
use crate::types::{MockOperationResponse, MockRequest, PatchMockRequest};
use axum::{
  Json,
...
pub async fn create_mock(State(app_state): State<MockApiState>, Json(request): Json<MockRequest>) -> impl IntoResponse {
  ...
  let mock_collection = mockpit_config::MockCollectionConfig {
    name: None,
    description: None,
    enabled: true,
    vars: None,
    ...
    if let Some(mock) = registry.get_mock(&id) {
      ...
      let url = if mock.request.url_patterns.is_empty() {
        ...
        match mock.request.url_patterns.first() {
          Some(mockpit_engine::types::UrlPattern::Exact(path)) => path.clone(),
          Some(mockpit_engine::types::UrlPattern::Prefix(prefix)) => format!("{prefix}*"),
          Some(mockpit_engine::types::UrlPattern::Suffix(suffix)) => format!("*{suffix}"),
          Some(mockpit_engine::types::UrlPattern::Regex(pattern)) => format!("~{pattern}"),
          Some(mockpit_engine::types::UrlPattern::Glob(_)) => "[glob pattern]".to_string(),
          None => "/*".to_string(),
        }
      };
    ...
    .map(|m| {
      ...
      let url = if m.request.url_patterns.is_empty() {
        ...
        match m.request.url_patterns.first() {
          ...
          Some(mockpit_engine::types::UrlPattern::Suffix(suffix)) => format!("*{suffix}"),
          Some(mockpit_engine::types::UrlPattern::Regex(pattern)) => format!("~{pattern}"),
          Some(mockpit_engine::types::UrlPattern::Glob(_)) => "[glob]".to_string(),
          None => "/*".to_string(),
        }
      };
