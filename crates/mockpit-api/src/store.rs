  ...
  for key in &keys {
    if let Some(value) = persistence_store.get(key) {
      ...
      if let Some(ttl_secs) = persistence_store.ttl_seconds(key) {
        ...
        let expires_at = chrono::Utc::now() + chrono::Duration::seconds(i64::try_from(ttl_secs).unwrap_or(i64::MAX));
...
pub async fn get_store_key(Path(key): Path<String>, State(app_state): State<MockApiState>) -> impl IntoResponse {
  let persistence_store = app_state.mock.mock_registry.get_persistence_store();
  ...
  if let Some(value) = persistence_store.get(&key) {
    ...
    if let Some(ttl) = ttl_seconds {
      response.insert("ttl_seconds".to_string(), serde_json::Value::Number(ttl.into()));
    } else {
    ...
    (StatusCode::NOT_FOUND, Json(serde_json::Value::Object(response))).into_response()
  }
}
...
pub async fn delete_store_key(Path(key): Path<String>, State(app_state): State<MockApiState>) -> impl IntoResponse {
  let persistence_store = app_state.mock.mock_registry.get_persistence_store();
