...
use crate::types::{FilterOperator, QueryFilter};
use mockpit_engine::types::MockDefinition;
use regex::Regex;
use std::borrow::Cow;
use std::sync::Arc;
...
pub fn parse_query(query: &str) -> Result<Vec<QueryFilter>, String> {
  ...
  let parts: Vec<&str> = query.split(" AND ").flat_map(|part| part.split(" OR ")).collect();
...
fn parse_single_filter(expr: &str) -> Result<QueryFilter, String> {
  ...
  for op_str in &operators {
    if let Some(pos) = expr.find(op_str) {
      ...
      return Ok(QueryFilter { field, operator, value });
    }
  }
...
pub fn apply_filters(mocks: Vec<Arc<MockDefinition>>, filters: &[QueryFilter]) -> Vec<Arc<MockDefinition>> {
  if filters.is_empty() {
    return mocks;
  }
...
fn get_field_value<'a>(mock: &'a MockDefinition, field: &str) -> Cow<'a, str> {
  match field {
    ...
    "scope" => mock.scope.as_deref().map_or(Cow::Borrowed(""), Cow::Borrowed),
...
mod tests {
  ...
  use mockpit_engine::types::{BodySource, RequestMatcher, ResponseGenerator};
  use smallvec::smallvec;

  fn create_test_mock(id: &str, priority: u32, enabled: bool, scope: Option<String>) -> MockDefinition {
    MockDefinition {
      id: id.into(),
      priority,
      enabled,
