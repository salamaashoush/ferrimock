...
use crate::types::{BulkOpResult, BulkOperation, BulkOperationRequest, BulkOperationResponse};
...
pub async fn bulk_operations(
  ...
  let snapshot = if atomic { Some(registry.get_all_mocks()) } else { None };
  ...
  for operation in request.operations {
    ...
    if atomic && !result.success {
      ...
      response.insert(
        "error".to_string(),
        serde_json::Value::String("Atomic operation failed, all changes rolled back".to_string()),
      );
      response.insert(
  ...
  Json(BulkOperationResponse { success: true, results }).into_response()
}
