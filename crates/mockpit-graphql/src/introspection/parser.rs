...
impl SchemaParser {
  ...
  pub fn parse(response: IntrospectionResponse) -> Result<ParsedSchema> {
    ...
    // Parse directives
    let directives = schema_intro.directives.into_iter().map(Self::parse_directive).collect();
  ...
  fn parse_type(full_type: FullType) -> Result<Option<TypeDefinition>> {
    ...
    // Parse type kind
    let kind = TypeKind::parse(&full_type.kind).with_context(|| format!("Unknown type kind: {}", full_type.kind))?;
  ...
  fn parse_field(field: FieldIntrospection) -> FieldDefinition {
    FieldDefinition {
      ...
      args: field.args.into_iter().map(Self::parse_input_value).collect(),
  ...
  fn parse_directive(directive: DirectiveIntrospection) -> DirectiveDefinition {
    DirectiveDefinition {
      ...
      args: directive.args.into_iter().map(Self::parse_input_value).collect(),
    }
  }
}
...
mod tests {
  ...
  fn test_parse_simple_schema() {
    ...
    let response: IntrospectionResponse =
      serde_json::from_value(response_json).expect("Failed to deserialize introspection response");
    ...
    // Check Query type
    let query_type = schema.get_type("Query").expect("Failed to get Query type from schema");
    assert_eq!(query_type.kind, TypeKind::Object);
    assert_eq!(query_type.fields.len(), 1);
    assert_eq!(query_type.fields[0].name, "user");

    // Check User type
    let user_type = schema.get_type("User").expect("Failed to get User type from schema");
