...
impl SdlWriter {
  ...
  fn write_schema(&mut self, schema: &ParsedSchema) {
    ...
    let mut types: Vec<&TypeDefinition> = schema.types.values().filter(|t| !t.name.starts_with("__")).collect();
  ...
  fn write_union(&mut self, type_def: &TypeDefinition) {
    ...
    self.buf.push_str(" = ");
    let members: Vec<&str> = type_def.possible_types.iter().map(|t| t.name.as_str()).collect();
...
mod tests {
  ...
  fn generate_sdl_from_json(json: serde_json::Value) -> String {
    let response: crate::introspection::IntrospectionResponse =
