...
use crate::type_mapper::TypeToFakeMapper;
use anyhow::{Context, Result};
use crate::introspection::{FieldDefinition, OperationType, ParsedSchema, TypeDefinition, TypeKind, TypeRef};
use mockpit_config::{GraphQLMatchConfig, MatchConfig, MockCollectionConfig, MockConfig, ReturnConfig};
...
pub struct GeneratorOptions {
  ...
  pub max_depth: usize,
  /// GraphQL endpoint URL path (e.g., "/graphql" or "/api/graphql")
...
impl MockGenerator {
  ...
  fn has_pagination_variable(&self, variables: &[String], param_names: &[&str]) -> Option<String> {
  ...
  fn generate_operation_mock(&self, operation: &FieldDefinition, op_type: OperationType) -> Result<MockConfig> {
    ...
    let response_template = self.generate_response_template_with_variables(operation, &variables)?;
  ...
  fn generate_type_template_with_variables(
    ...
    if let Some(name) = field_name {
      if let Some(var_expr) = self.matches_variable(name, variables) {
        ...
        if unwrapped.is_list {
          ...
          return Ok(format!("[\n{}{}\n{}]", "  ".repeat(depth + 1), var_expr, indent));
        }
        return Ok(var_expr);
      }
    }
    ...
    let single_value = match type_def.kind {
      TypeKind::Scalar => {
        ...
        use mockpit_type_detector::{detect_from_field_name_only, detect_from_semantic_context};
          ...
          if self.is_pagination_field(name) {
            match name {
              "hasNextPage" => {
                ...
                if self.has_pagination_variable(variables, &["after", "cursor"]).is_some() {
      ...
      TypeKind::Enum => {
        let enum_values: Vec<String> = type_def.enum_values.iter().map(|v| v.name.clone()).collect();
        self.type_mapper.enum_to_fake(&enum_values)
      },
      TypeKind::Object => {
        visited.insert(unwrapped.name.clone());
        let result = self.generate_object_template_with_variables(type_def, depth, visited, variables)?;
      ...
      TypeKind::Interface | TypeKind::Union => {
        ...
        let result = self.generate_union_or_interface_template(type_def, depth, visited, variables)?;
    ...
    if unwrapped.is_list {
      ...
      if let Some(param) = self.has_pagination_variable(variables, &["first", "limit"]) {
        ...
        array_template.push_str(&indent);
        array_template.push_str("{% set __array_length = body_json.variables.last | default(value=");
        array_template.push_str(&self.options.default_list_length.to_string());
        array_template.push_str(") %}\n");
        array_template.push_str(&indent);
        array_template.push_str("[\n");
  ...
  fn should_include_field(&self, field: &FieldDefinition, depth: usize) -> bool {
    ...
    if matches!(
      field.name.as_str(),
      "name" | "title" | "description" | "email" | "status" | "type" | "kind" | "createdAt" | "updatedAt"
    ) {
      return true;
    }
  ...
  fn is_complex_type(&self, type_ref: &TypeRef) -> bool {
    ...
    if let Some(type_def) = self.schema.get_type(&unwrapped.name) {
      matches!(type_def.kind, TypeKind::Object | TypeKind::Interface | TypeKind::Union)
    } else {
      false
    }
  }
  ...
  fn generate_union_or_interface_template(
    ...
    if possible_types.len() == 1 {
      ...
      return self.generate_object_with_typename(concrete_type, &type_ref.name, depth, visited, variables);
    }
    ...
    let type_names: Vec<String> = possible_types.iter().map(|t| format!("\"{}\"", t.name)).collect();
    ...
    for (i, type_ref) in possible_types.iter().enumerate() {
      ...
      let obj_template =
        self.generate_object_with_typename(concrete_type, &type_ref.name, depth, visited, variables)?;
