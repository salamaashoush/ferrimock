...
impl TypeToFakeMapper {
  ...
  pub fn load_mappings(&mut self, path: &Path) -> Result<()> {
    ...
    let content =
      fs::read_to_string(path).with_context(|| format!("Failed to read scalar mappings file: {}", path.display()))?;
    ...
    let mappings: FxHashMap<String, String> = match extension {
      "json" => {
        let config: serde_json::Value =
          serde_json::from_str(&content).with_context(|| format!("Failed to parse JSON file: {}", path.display()))?;
...
mod tests {
  ...
  fn test_builtin_scalars() {
    ...
    assert_eq!(mapper.scalar_to_fake("String"), "\"{{ fake_sentence(word_count=8) }}\"");
    assert_eq!(mapper.scalar_to_fake("Int"), "{{ get_random(start=1, end=1000) }}");
    assert_eq!(
  ...
  fn test_common_custom_scalars() {
    ...
    assert_eq!(mapper.scalar_to_fake("PhoneNumber"), "\"{{ fake_phone() }}\"");
  }
  ...
  fn test_fallback() {
    ...
    assert_eq!(mapper.scalar_to_fake("UnknownType"), "\"{{ fake_word() }}\"");
  }
...
"#;
    ...
    let mut temp_file = NamedTempFile::with_suffix(".toml").expect("Failed to create temporary file with suffix");
    temp_file
...
"#;
    ...
    let mut temp_file = NamedTempFile::with_suffix(".json").expect("Failed to create temporary file with suffix");
    temp_file
...
"#;
    ...
    assert_eq!(mapper.scalar_to_fake("UUID"), "\"{{ fake_alphanumeric(length=32) }}\"");
  }
  ...
  fn test_load_mappings_unsupported_extension() {
    ...
    let mut temp_file = NamedTempFile::with_suffix(".txt").expect("Failed to create temporary file with suffix");
    temp_file
