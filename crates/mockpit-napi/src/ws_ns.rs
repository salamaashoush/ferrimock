...
//! `mockpit::streaming::drive_ws_connection` — the same driver behind
//! the QuickJS lane — so `MockpitServer.listen()` serves JS-defined
//! `ws.link` handlers natively. The JS side (`@mockpit/core`) wraps this
//! into MSW's `ws.link(...).addEventListener("connection", ...)` shape.

use crate::http_ns::{RequestHandler, as_regexp, compile_js_regex};
use mockpit::streaming::{WsDispatchFn, WsDriverEvent, WsUpstreamCmd, drive_ws_connection};
use mockpit::types::{
        ...
        Box::pin(async move {
            ...
            match tsfn.call_async(bridge_event).await {
                Ok(promise) => promise.await.map_err(|e| {
                    mockpit::MockpitError::msg(format!("ws dispatch callback failed: {e}"))
                }),
...
pub fn handler(
    ...
    mock.streaming = Some(mockpit::types::StreamingResponse::WsHandler(handler_fn));
