...
fn create_complex_mock(id: &str) -> MockDefinition {
    use ferrimock::engine::types::{BodyMatcher, HeaderMatcher};
...
fn bench_context_construction(c: &mut Criterion) {
    ...
    for (k, v) in [
        ...
        ("user-agent", "ferrimock-bench/1.0"),
...
fn bench_template_response(c: &mut Criterion) {
    use ferrimock::engine::RequestContext;
...
fn bench_response_patcher(c: &mut Criterion) {
    use bytes::Bytes;
    use ferrimock::engine::ResponsePatcher;
    use ferrimock::engine::types::PatchOperation;
    use http::Response;
