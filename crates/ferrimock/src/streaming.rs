...
pub type WsDispatchFn = Arc<
    dyn Fn(WsDriverEvent) -> Pin<Box<dyn Future<Output = Result<bool, FerrimockError>> + Send>>
        + Send
        + Sync,
>;
...
/// `FerrimockEventSource`): once a connection has opened, a dropped or
