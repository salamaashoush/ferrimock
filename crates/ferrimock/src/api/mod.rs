...
//! Routes are prefixed with a configurable prefix (defaults to `/__ferrimock`).
...
pub const DEFAULT_PREFIX: &str = "/__ferrimock";

/// Create the mock management API router with the default prefix (`/__ferrimock`).
///
...
pub fn create_mock_router_with_prefix(prefix: &str) -> Router<MockApiState> {
    ...
    Router::new()
        ...
        .route(
            &format!("{p}/recording/start"),
            post(status::start_recording),
        )
