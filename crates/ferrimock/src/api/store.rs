...
use super::types::{StoreDeleteResponse, StoreGetAllResponse, StoreMetadata, StoreSetRequest};
use axum::{
    Json,
    extract::{Path, State},
    http::StatusCode,
    ...
    for key in &keys {
        if let Some(value) = persistence_store.get(key) {
            ...
            if let Some(ttl_secs) = persistence_store.ttl_seconds(key) {
                ...
                keys_with_ttl.push(super::types::KeyTtlInfo {
                    key: key.clone(),
