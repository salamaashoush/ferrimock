...
pub trait ResponseGeneratorExt {
    ...
    fn generate(
        &self,
    ) -> impl std::future::Future<Output = Result<bytes::Bytes, crate::FerrimockError>> + Send;
    ...
    ) -> impl std::future::Future<Output = Result<DynamicResponse, crate::FerrimockError>> + Send;
...
impl ResponseGeneratorExt for ResponseGenerator {
    async fn generate(&self) -> Result<bytes::Bytes, crate::FerrimockError> {
    ...
    async fn generate_with_context(
        &self,
        context: &RequestContext,
    ) -> Result<bytes::Bytes, crate::FerrimockError> {
        ...
        match &self.body {
            ...
            BodySource::Handler { handler, .. } | BodySource::ForeignHandler(handler) => {
                let dynamic = handler(context.clone()).await?;
                Ok(dynamic.body)
            }
        }
    }
    ...
    async fn generate_dynamic(
        ...
        match &self.body {
            ...
            BodySource::Handler { handler, needs } => {
                let mut context = RequestContext::from_request_for_handler_selective(
                    method, uri, query, headers, body, *needs,
                );
                context.captures = captures;
                context.vars = vars.cloned();
                handler(context).await
            }
            BodySource::ForeignHandler(handler) => {
                let mut context =
    ...
    fn generate_dynamic_sync(
        ...
        match &self.body {
            BodySource::File(_) | BodySource::Handler { .. } | BodySource::ForeignHandler(_) => {
