...
impl ScriptHost {
    ...
    pub async fn load_file(&self, path: &Path, root: Option<&Path>) -> Result<Vec<MockDefinition>> {
        ...
        let canonical = dunce::canonicalize(path)
            .map_err(|e| FerrimockError::Script(format!("{}: {e}", path.display())))?;
        let root = match root {
            Some(r) => dunce::canonicalize(r)
                .map_err(|e| FerrimockError::Script(format!("{}: {e}", r.display())))?,
            None => canonical.parent().map(Path::to_path_buf).ok_or_else(|| {
                FerrimockError::Script("script file has no parent dir".to_string())
            })?,
        };
        ...
        ScriptMockKind::Ws { ref path } => {
            ...
            let upgrade =
                crate::types::HeaderMatcher::regex(http::header::UPGRADE, "(?i)^websocket$")
                    .map_err(|e| FerrimockError::Script(format!("upgrade matcher: {e}")))?;
