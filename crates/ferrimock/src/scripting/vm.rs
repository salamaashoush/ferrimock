...
impl VmHandle {
    ...
    pub async fn with<R, F>(&self, f: F) -> Result<R, FerrimockError>
    where
        R: Send + 'static,
            ...
            + 'static,
    {
        ...
        self.tx
            .send(job)
            .map_err(|_| FerrimockError::Script("script VM loop is gone".to_string()))?;
        rx.await
            .map_err(|_| FerrimockError::Script("script VM loop dropped the job".to_string()))
    }
}
