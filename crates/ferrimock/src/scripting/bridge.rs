...
pub fn caught_to_error(caught: &CaughtError<'_>) -> FerrimockError {
    match caught {
        CaughtError::Exception(ex) => {
            let message = ex.message().unwrap_or_else(|| "exception".to_string());
            match ex.stack() {
                Some(stack) if !stack.is_empty() => {
                    FerrimockError::Script(format!("{message}\n{stack}"))
                }
                _ => FerrimockError::Script(message),
            }
        }
        CaughtError::Value(v) => FerrimockError::Script(format!("script threw: {v:?}")),
        CaughtError::Error(e) => FerrimockError::Script(e.to_string()),
    }
}
...
pub(super) fn restore_handler<'js>(
    ctx: &Ctx<'js>,
    slot: u64,
) -> Result<Function<'js>, FerrimockError> {
    let persistent = with_slots(ctx, |slots| slots.get(slot))
        .map_err(|e| FerrimockError::Script(e.to_string()))?
        .ok_or_else(|| FerrimockError::Script(format!("script handler slot {slot} is gone")))?;
    persistent
        .restore(ctx)
        .map_err(|e| FerrimockError::Script(format!("restore handler: {e}")))
}
...
) -> Result<Value<'js>, FerrimockError> {
...
fn begin_generator_step<'js>(
    ...
    let iterator = if let Some(persistent) = existing {
        persistent
            .restore(ctx)
            .map_err(|e| FerrimockError::Script(format!("restore iterator: {e}")))?
    } else {
    ...
    let next: Function<'js> = iterator
        .get("next")
        .map_err(|e| FerrimockError::Script(format!("generator iterator has no next(): {e}")))?;
...
fn finish_generator_step<'js>(
    ...
    let step_obj = step
        .into_object()
        .ok_or_else(|| FerrimockError::Script("generator step is not an object".to_string()))?;
    let done: bool = step_obj.get("done").unwrap_or(false);
    let value: Value<'js> = step_obj
        .get("value")
        .map_err(|e| FerrimockError::Script(format!("generator step value: {e}")))?;
    ...
    if value.is_undefined() && done {
            ...
            .map_err(|e| FerrimockError::Script(e.to_string()))?
        {
            return last
                .restore(ctx)
                .map_err(|e| FerrimockError::Script(format!("restore generator value: {e}")));
        }
        return Ok(value);
    }
    ...
    Arc::new(move |request: RequestContext| {
        ...
        Box::pin(async move {
            if poisoned.load(Ordering::Relaxed) {
                return Err(FerrimockError::Script(
            ...
            let eval = vm_with!(vm => |ctx| {
                ...
                let stream_value = match stream.restore(&ctx) {
                    Ok(v) => v,
                    Err(e) => return Err(FerrimockError::Script(format!("restore stream: {e}"))),
                };
                ...
                if let Some(start) = handle.start_result {
                    let start_value = match start.restore(&ctx) {
                        Ok(v) => v,
                        Err(e) => {
                            return Err(FerrimockError::Script(format!("restore stream start: {e}")));
                        }
                    };
            ...
            let outcome: Result<DynamicResponse, FerrimockError> =
                match tokio::time::timeout(backstop, eval).await {
                    Ok(result) => result.and_then(|inner| inner),
                    Err(_) => Err(FerrimockError::Script(format!(
