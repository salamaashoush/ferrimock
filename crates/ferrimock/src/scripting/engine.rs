...
impl ScriptEngine {
    ...
    /// `ferrimock` native-module loader, install the host bindings, and
    ...
    pub async fn new(config: ScriptEngineConfig) -> Result<Self> {
        let runtime = AsyncRuntime::new()
            .map_err(|e| FerrimockError::Script(format!("QuickJS runtime init: {e}")))?;
        ...
        vm_with!(vm => |ctx| {
            super::bindings::install_all(&ctx)
                .map_err(|e| FerrimockError::Script(format!("install bindings: {e}")))
        })
        .await??;
