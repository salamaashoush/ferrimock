//! Ferrimock configuration file parsing.
//!
...
///
/// Can be defined in `ferrimock.config.yaml`, `ferrimock.config.json`,
/// or (via the Node.js layer) `ferrimock.config.ts` / `ferrimock.config.js`.
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
#[serde(rename_all = "camelCase", default)]
pub struct FerrimockConfig {
...
///
/// Searches for `ferrimock.config.{yaml,yml,json,ts,js,mjs,mts}` in order.
...
pub fn discover_config_file(dir: &str) -> Option<String> {
    ...
    let names = [
        "ferrimock.config.ts",
        "ferrimock.config.js",
        "ferrimock.config.mjs",
        "ferrimock.config.mts",
        "ferrimock.config.yaml",
        "ferrimock.config.yml",
        "ferrimock.config.json",
    ];
