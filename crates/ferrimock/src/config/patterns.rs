...
pub fn parse_url_pattern(pattern: &str) -> crate::Result<UrlPattern> {
    // Handle explicit prefixes (generated by HAR loader and consolidator)
    if let Some(url) = pattern.strip_prefix("exact:") {
        return Ok(UrlPattern::exact(url));
    }
    if let Some(url) = pattern.strip_prefix("prefix:") {
        return Ok(UrlPattern::prefix(url));
    }
    if let Some(url) = pattern.strip_prefix("suffix:") {
        return Ok(UrlPattern::suffix(url));
    }
    if let Some(re) = pattern.strip_prefix("regex:") {
        return UrlPattern::regex(re)
            .map_err(|e| crate::mp_err!("Invalid regex pattern '{re}': {e}"));
    }
    if let Some(glob) = pattern.strip_prefix("glob:") {
        return UrlPattern::glob(glob)
            .map_err(|e| crate::mp_err!("Invalid glob pattern '{glob}': {e}"));
    }

    // Auto-detection - no explicit prefix
    ...
    if pattern.contains("/:") || pattern.contains("/{") {
        ...
        return UrlPattern::regex(&regex_pattern).map_err(|e| {
            crate::mp_err!("Failed to convert Express-style pattern '{pattern}' to regex: {e}")
        });
    }
    ...
    if pattern.contains("**") || (pattern.contains('*') && !pattern.starts_with('^')) {
        return UrlPattern::glob(pattern)
            .map_err(|e| crate::mp_err!("Invalid glob pattern '{pattern}': {e}"));
    }
        ...
        || pattern.contains('|')
    {
        return UrlPattern::regex(pattern)
            .map_err(|e| crate::mp_err!("Invalid regex pattern '{pattern}': {e}"));
    }
