...
async fn serve_yaml(yaml: &str) -> (ferrimock::services::serve::ServeHandle, String) {
...
async fn upstream_forwarding_passthrough() {
    // Real upstream: a second ferrimock server in echo mode.
...
async fn href_regex_matches_reconstructed_ws_urls() {
    ...
    use ferrimock::types::{HeaderMatcher, StreamingResponse, UrlPattern, WsScript};

    let mut mock = ferrimock::handler::http::get("*", |_ctx| async {
        Err(ferrimock::FerrimockError::msg("ws stub"))
    });
...
async fn href_regex_respects_a_known_scheme() {
    use ferrimock::engine::{MockMatcher, MockRegistry};
    use ferrimock::types::{StreamingResponse, UrlPattern, WsScript};
