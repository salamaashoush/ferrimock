...
use ferrimock::config::{HarLoadOptions, HarLoader, ReturnConfig};
use ferrimock::engine::har_export::export_mocks_to_har;
use ferrimock::engine::{
...
async fn test_export_single_mock_to_har() {
    ...
    match &har.log {
        Spec::V1_2(log) => {
            ...
            assert_eq!(log.creator.name, ferrimock::core::app_name());
        }
...
async fn load_har_string(content: &str) -> Vec<ferrimock::config::MockConfig> {
...
"#,
    ...
    assert!(
        def.streaming
            .as_ref()
            .is_some_and(ferrimock::types::StreamingResponse::is_ws)
    );
    assert_eq!(
        def.request.methods,
