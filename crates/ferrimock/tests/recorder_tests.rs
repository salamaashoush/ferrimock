...
use anyhow::Result;
use bytes::Bytes;
use ferrimock::engine::MockRecorderConsolidationExt;
use ferrimock::recorder::{MockRecorder, RecordingFilterOptions, RecordingFormat};
...
async fn test_har_with_query_string_parsing() {
    ...
    let file_path = recorder.save(RecordingFormat::Har).await.unwrap();
    let content = tokio::fs::read_to_string(&file_path).await.unwrap();
    let har: har::Har = ferrimock::config::parse_har(&content).unwrap();
