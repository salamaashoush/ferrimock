...
use ferrimock::types::{BodySource, MockDefinition, RequestContext};
...
async fn loads_handlers_from_globals_and_module_import() {
    ...
    let path = write_mock(
        ...
        "users.mjs",
        r"
import { http as mhttp, HttpResponse as HR } from 'ferrimock';
...
async fn syntax_error_fails_load_with_location() {
    ...
    assert!(
        matches!(err, ferrimock::FerrimockError::Script(_)),
        "got: {err}"
    );
}
...
",
    ...
    let host = ScriptHost::new();
    host.set_config(ferrimock::scripting::ScriptEngineConfig {
...
",
    ...
    assert_eq!(
        resp.headers
            .as_ref()
            .and_then(|h| h.get(ferrimock::types::NETWORK_ERROR_HEADER))
            .map(String::as_str),
        Some("1")
    );
    ...
    assert_eq!(
        resp.headers
            .as_ref()
            .and_then(|h| h.get(ferrimock::types::PASSTHROUGH_HEADER))
            .map(String::as_str),
        Some("1")
    );
...
mod serve_markers {
    ...
    async fn passthrough_serves_unmatched_with_signal_header() {
        ...
        let resp = ferrimock::services::serve::respond(
            &matcher,
            &Method::GET,
            "/pt",
            None,
        ...
        assert_eq!(
            resp.headers()
                .get(ferrimock::types::PASSTHROUGH_HEADER)
...
mod streaming_scripts {
    use ferrimock::services::serve::{ServeInput, start};
    use futures::{SinkExt, StreamExt};
    use tokio_tungstenite::tungstenite::Message;

    async fn serve_script(source: &str) -> (ferrimock::services::serve::ServeHandle, String) {
        let dir = tempfile::tempdir().expect("tempdir");
